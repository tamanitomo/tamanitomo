...
**Current status (2026-09-24, supersedes the status line below):** C1 core, C1 integration (Appendix E) and the integration closure with Linux activation evidence (Appendix F) exist on test branches and are **not activated**: only an explicit `build(chat_sends=Options(...))` registers the routes, no UI or release file uses them, and `POST /api/chat` is unchanged. Gate table: [`Phase1B_C1_ActivationReadiness.md`](Phase1B_C1_ActivationReadiness.md) §4. Public-output closure after review (R1–R4): [`Phase1B_C1_PublicOutputClosure.md`](Phase1B_C1_PublicOutputClosure.md). **C2 dispatcher safety** (§7) is implemented on a separate unmerged branch, pending review: [`Phase1B_C2_Results.md`](Phase1B_C2_Results.md). Unlike C1 it changes shipped scripts and has no activation flag.
...
| **C1 integration (R6, implemented, not activated)** | `kit/app/chat_send_routes.py` (new); additions to `chat_sends.py` (links, provenance, read model, installation hold), `send_protocol.py` (schema 3), `runtime.py` (`Operations` ident/claim/allowlist/guard/retention, `Runtime.executor_spec`), `server.py` (opt-in `chat_sends=`), `manage.py` (operation view, console hold), `terminal.py`, `chat_routes.py`/`chat_sources.py`/`chat_projection.py` (provenance, join, O-12 exclusion, disclosure); `docs/CHAT_CONTRACT.md` §8. Not in `release-files.json`; `/api/chat` and `hermes_stream.py` unchanged. | `tests/test_phase1b_c1_integration.py`; `PinnedHttp` in the pinned lane. Appendix E. |
| **C1 activation** (later, separate review; the rows below are the original proposal, partly superseded by the integration row above) | `kit/app/chat_sends.py` (new): ledger, storage checks, states, facts, fencing, lease, liveness, recovery, retention. `kit/app/hermes_stream.py`: go-gate, executor lock with recorded identity, S2 under the shared guard, watchdog, the commit-boundary recorder (§4.10.1), `executor_finished` with `receipts_complete`. `kit/app/runtime.py`: `Runtime.chat()` launch protocol and process group/job; `Operations.submit(ident=)`, persisted-field allowlist, operation retention; installation lock helper. `kit/app/chat_routes.py`: `/api/chat/sends*`, `correlation.send_id` join. `kit/app/manage.py`: legacy `/api/chat` through the ledger, before/after guess removed; installation lock around mutations. `kit/app/chat_sources.py`: workspace sessions from fresh-session receipts (R3); capability table. `docs/CHAT_CONTRACT.md`: "Sending" section, restore limitation. `release-files.json`. | Tests: `tests/test_chat_sends.py`, `tests/test_send_recovery_subprocess.py`, `tests/test_operations_privacy.py`. (`tests/mock_provider.py` already gained `hang` in C0.) |
...
| **C3** minimal client correction (**implemented, not activated**, Appendix G) | `kit/app/static/chat-sends.js` (new, not in `release-files.json`); three inert hooks in `kit/app/static/workspace.js`; `server.py`/`chat_send_routes.Options(client=)` (page signal); `manage.py`/`runtime.feed_page(source_ids=)` (row ids, client build only). | `tests/test_phase1b_c3_browser.py` (real Chromium, M-22), `tests/test_phase1b_c3_disabled.py`; `Phase1B_C3_Results.md` |
...
| O-I | **Decided (R5, PHASE1B_REVIEW_R4 §6):** the continuation note is internal machinery, not owner speech. The source row is preserved; nothing is excluded by text or prefix; no historical row is reclassified. | For C1-controlled turns the executor records causal provenance (§4.10.1 R5), receipts list the row as internal, and the owner turn is the real owner row. **Still an activation gate:** the Phase 1A read adapter must exclude that exact row by this provenance (bound to the committed source identity) while keeping an equal-text owner control; rows written outside a C1 executor (terminal, legacy, history) have no such provenance, so that trusted-read path stays marked incomplete and its activation blocked. **(R6)** The read-side exclusion is implemented for apps built with keyed sends (Appendix E); rows without provenance keep the stated limitation. |
...
## Appendix E. (R6) The C1 integration, answering PHASE1B_REVIEW_R5

| Review item | What was built | Where it differs from the proposal above |
|---|---|---|
| §5.1 routes | bootstrap, keyed POST, receipt, `?key=`, `?open=1`, stop, confirmed reset, over the accepted service; scope captured first; replay/conflict, generation fence and one lease through HTTP | Enabled only by `build(chat_sends=Options(...))`. A process whose in-process slot is taken still replays (`accept(admit=False)`) and refuses new keys (`turn_in_progress` / `installation_busy`); keyed routes are exempt from the middleware busy refusal. Reset needs `{"confirm": "reset send ledger"}`. |
| §5.2 operations | reserved id through `Operations.submit(ident=, claimed=, persist=)`; ledger-built view on `GET /api/operations/{id}`; allowlisted `format: 2` files; transient/reconstructed legacy result under current authorisation; retention of `format: 2` only | `not_started` maps to `failed` for old clients. Historical (pre-1B) records untouched. |
| §5.3 read boundary | fresh-session provenance and O-12 exclusion bound to the receipted fingerprint; `correlation.send_id` computed at read time; late proof withdraws an already projected row; `provenance` table (not pruned, carried across a readable reset; loss recorded and disclosed) | Applied only in an app built with keyed sends (shipped modules import nothing unshipped); a build without them discloses `not_applied` if a ledger exists. |
...
## Appendix G. The C3 client (combined review of 02f2285)

Implemented as `kit/app/static/chat-sends.js`, loaded only on a page served by an app built with `chat_sends=Options(client=True)` (`docs/CHAT_CONTRACT.md` §8.6). Departures from §6, stated: (1) the client is a separate file rather than an edit of the `workspace.js` submit path, so the default page and release contents are unchanged; `workspace.js` gains three hooks that do nothing without the signal. (2) Bootstrap runs before every new intent, not only the first of a page load (cheaper to reason about; same fence). (3) §4.10.4's replacement of provisional bubbles needs a history-row identity; the `/api/feed` rows gain `source_message` for the client build only, and the join is by `(session, Hermes row id)` from the snapshot's `correlation.send_id` or the settled `result.messages[].source`. Because links are recorded at settlement (C1), a reload mid-turn can show the owner row twice until then. Results: `Phase1B_C3_Results.md`.
