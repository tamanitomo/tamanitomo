...
def runtime_lock(root):
    with (root / ".runtime.lock").open("a+b") as f:
        ...
        try:
            ...
            raise ValueError(
                "Close the running Tamanitomo host (Ctrl-C in its launcher window), then launch again to update."
            )
        try:
            yield
        finally:
...
def apply_pending(root):
    if not (root / ".pending-update/SHA256SUMS.json").exists():
        return
...
def _apply_pending(root):
    ...
    manifest = folder / "SHA256SUMS.json"
    if not manifest.exists():
        return
    hashes = json.loads(manifest.read_text(encoding="utf-8"))
    old = json.loads((root / "SHA256SUMS.json").read_text(encoding="utf-8"))
    for name, digest in hashes.items():
        ...
        if (
            ...
            raise ValueError("Staged update changed; reinstall a trusted package")
    ...
    names = set(old) | set(hashes) | {"SHA256SUMS.json"}
    written = []
    try:
        for name in names:
            dest = root / name
        ...
        for name in [*hashes, "SHA256SUMS.json"]:
            ...
            temp = dest.with_name("." + dest.name + ".update")
            shutil.copy2(folder / name, temp)
            os.replace(temp, dest)
            written.append(name)
        for name in set(old) - set(hashes):
...
def _prune_backups(root, keep=KEEP_BACKUPS):
    ...
    try:
        if not folder.is_dir() or folder.is_symlink():
            return []
        saved = sorted(
            (d for d in folder.iterdir() if d.is_dir() and not d.is_symlink()),
        ...
        if removed:
            print(
                f"Removed {len(removed)} older backup(s), keeping the newest {max(int(keep),1)}."
            )
        return removed
    except OSError:
        return []
