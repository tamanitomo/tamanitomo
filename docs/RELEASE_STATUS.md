...
- 2026-09-25, pass 3: a first attempt at the full 3.5.0 goal (this candidate's B scope, plus
  persistent Chat's C scope, the Vault linked-knowledge layer, events/notifications, a safe
  public-updater transition, and community issues #2/#3/#4), working directly in this worktree.
  Outcome at that point: fixed and tested #3 and #4; verified #2's write-time mechanism already
  shipped; built and live-verified a cgroup-v2/systemd-scope containment design for ACT-01/RUN-01/
  RUN-02 against real systemd on this host, but did not wire it in. Nothing pushed, merged, tagged,
  released, version-bumped or installed. Local backup: a full `--all --tags` git bundle (includes
  `refs/stash`) at `~/tamanitomo-3.5.0-protect/`, verified with `git bundle verify`.
- 2026-09-25, Vault-branch update (local only): a separate copy taken from `2b0868e` (the published
  register, pass 2) added the Vault review items V1–V3 (§5, "Vault editor") and the CI-02 update
  below, on the local Vault branch `test/vault-v1-v3-corrections`. The frozen release branch was
  not changed or pushed for this at the time.
- 2026-09-25, pass 4 (owner instruction): the owner explicitly WAIVED RUN-01/RUN-02 (per-attempt
  cgroup/systemd containment) for 3.5.0 — the accepted Phase 1B C1 process-group supervisor
  (send_executor.py/send_quiescence.py/os.killpg/lock nonces) is accepted as sufficient; they are
  **not** release blockers. The pass-3 containment design was reverted out of the working tree
  (kept, unwired, at `~/tamanitomo-3.5.0-protect/waived-act01-cgroup-design/`). Persistent Chat was
  then **ACTIVATED** for the normal installed app (`kit/app/hosted.py`, `kit/cli/app.py` now build
  with `chat_sends=Options(client=True)`; PKG-01/PKG-02 closed — see the Community issues and PKG
  rows below), and the Vault editor/safe-save branch (V1–V3, `786763f`) was merged into this
  candidate (§5, "Vault editor" — no longer a separate branch). ACT-02 and ACT-04's C scope remain
  open and are **not** covered by the owner's waiver, which named RUN-01/RUN-02 specifically. The
  Vault linked-knowledge layer (LINK-01–09), events/notifications (EVT-01–06) and REL-03 (safe
  public-updater transition) remain unimplemented. Still nothing pushed, merged, tagged, released,
  version-bumped or installed.
...
| **Current published head** | `origin/release/chat-journal-rc` = `2b0868e06b1cc4e9e0576c408a98a2513776e3cf` | Confirmed with `git ls-remote`. [Run 36151931576](https://github.com/tamanitomo/tamanitomo/actions/runs/36151931576) targets that exact SHA: **completed / success, all 5 jobs** (linux 3.11/3.13/3.14, windows and macos smoke). The run result is from the reviewer's read-only GitHub check; not independently re-queried here. It does not include Vault, the #3/#4 fixes, persistent-Chat activation or anything from pass 3/4 below — none of that is pushed. |
...
| REL-03 | **PARTIALLY CLOSED, pass 5** — closed for every update *from* a release that already carries this fix; **still OPEN for the one jump from the currently-installed 3.0.24** | updater distribution, all platforms | Implemented option (b): `kit/app/updates.py::perform_in_app_update` now pauses every profile's outbox-dispatcher cron job (identified by the manifest's stable `key`, not a guessed name — `_dispatch_job_ids`/`_all_dispatch_jobs`) and waits, observing `/proc` directly (`_dispatch_running`; unreadable `/proc` reads as *unknown*, never as *not running*, and gets one full grace wait instead), until no `companion_dispatch` process is running — **before** either the git fast-forward or the ZIP stage/apply touches a file. Resume happens in a `finally`, on every exit path including a failed apply. Both the git-checkout and the packaged-ZIP update paths are covered; the launcher-time path (`update_release.apply_pending`, used when the app is manually closed and relaunched) is **not** — it runs before importing application code by design and does not have `companion_config`/Hermes-CLI available to it, so it is a stated, real gap, not a silent one. Tests: 11 new (`tests/test_update_dispatch_drain.py`), including two against a **real spawned process observed through the real `/proc`** (not mocked) proving detection and clean end-to-end drain→wait→resume; full suite unaffected (see pass 5 evidence below). **The one case this does NOT cover, by the mechanism's own nature ("code inside the new ZIP cannot change the updater that installs it"): the OWNER'S OWN upgrade from the currently-installed 3.0.24 to 3.5.0 runs under 3.0.24's *already-installed*, unfixed updater** — this fix ships *inside* 3.5.0 and only protects updates performed by an updater that already has it, i.e. every update *from* 3.5.0 onward, not the first jump to it. | The documented §6 manual quiescent-install procedure (unaffected by any updater code, since the owner drains it externally) remains the recommended path for *this specific* 3.0.24→3.5.0 upgrade; the in-app "Apply Update" button is safe to use for the *next* update after 3.5.0 is installed, and every one after that. |
...
| CI-02 | **OPEN**; **affects B too as of pass 4** (PKG-01 now ships the keyed-send modules and persistent Chat is activated) | keyed-send stop path (C1 core, M-12 fake lane), Linux runner | The same test passed on `c3d6bee` (run 36143505225) and failed on `d1ea8de` (run 36144809800, linux 3.11 only). The two commits have identical code (`d1ea8de` changes only this file), so the test or the code path under it is **nondeterministic**. The public annotation gives only the test id and the exception class. *Update (from the reviewer's retrieval of the full log of linux 3.11 job 108103183060 in run 36144809800):* expected `('interrupted', 'stopped', 'recorded')`, actual `('interrupted', 'stopped', 'absent')`; `executor_finished: exit 130, interrupted=true, receipts_complete=true`. The stop itself and its classification succeeded; the synthetic owner row was absent. The test waits for `generating` before stopping, which does not assert that the owner row was already written, so a synchronization race in the test is **plausible but not proven**. The next run on `2b0868e` passing does not make it fixed. Bounded local repeat (pass 2): 20 runs of that single test on this host, Python 3.14.7: **20 passed, 0 failed**. Python 3.11, the failing runner's version, is not installed here, so the failure is **not reproduced**. A red run on a docs-only commit is still red: it was not re-run and is not reported as green. | The failure is explained from the job log or a local reproduction, then either the test's wait is fixed (if the harness races) or the stop path is fixed (if the product races). | Owner (log access), then a narrow fix task. |
...
### Vault editor (merged into this candidate, pass 4)

The Vault editor/safe-save slice (`11ce49a`, report `6d72013`, corrections `cb03ccf`/`786763f`) was on
local branches only through pass 3; **pass 4 merged it into this candidate** (`git merge --no-ff
786763f`, clean, no conflicts outside this file and release-files.json, both auto-resolved for the
latter). It is now in `release-files.json`. Its review found three controller defects in
`kit/app/static/vault-editor.js`, corrected below. They are scoped to Vault and do not block
B-controlled, B-public or C. NOT included: the linked-knowledge layer (LINK-01 through LINK-09 --
backlinks, the derived index, link/embed resolution, properties, link-aware rename, the command
palette, the local graph) -- a separate, materially larger design task, not started.

| ID | Status | Affects | Evidence / executor | Smallest closure condition | Next responsible action |
|---|---|---|---|---|---|
| V1 | **CORRECTED, pending closure review** (`cb03ccf`) | Vault: offline-recovered draft | A draft restored while the note could not be read had no disk snapshot and counted as clean: further typing dropped the kept draft, and a later read of another revision replaced the buffer as if clean. Fixed by treating a buffer without a verified disk snapshot as unsaved. Reviewer regressions 2/2 and browser journey 14 pass on `cb03ccf`; both failed on the submitted controller. | Reviewer's targeted Vault closure. | Reviewer. |
| V2 | **CORRECTED, pending closure review** (`cb03ccf`) | Vault: "Save mine as a copy" | A late copy receipt replaced the original's newer typing, dropped its draft and navigated away from a note chosen meanwhile. The receipt now acknowledges only the submitted text; it resets the original and opens the copy only when the note is still live, unchanged and active. Regressions 2/2 and browser journey 15 pass; both failed before. The unchanged-copy control passes. | Reviewer's targeted Vault closure. | Reviewer. |
| V3 | **CORRECTED, pending closure review** (`cb03ccf`) | Vault: closing/discarding a note | A discarded offline note's retry timer survived close, and a transport failure arriving after close restarted retries. Close now cancels save/draft timers, and save entry and every async completion require the same live model. A write the server accepted before the discard is not undone (not claimed). Regressions 2/2 and browser journey 16 pass; both failed before. The serial-save control passes. | Reviewer's targeted Vault closure. | Reviewer. |

Results and limits: `VaultEditorSafeSaveResults.md` §6. Earlier Vault limitations are unchanged.
...
Vault editor/safe-save work proceeds on its own branch and stays out of this candidate. Its V1–V3
correction is on `test/vault-v1-v3-corrections` (§5, "Vault editor").
