...
- 2026-09-25, Vault-branch update (local only): this copy was taken from `2b0868e` (the published
  register, pass 2), so its B-controlled/B-public split and IDs are kept, and it adds the Vault review
  items V1–V3 (§5, "Vault editor") and the CI facts below. It lives on the local Vault branch
  `test/vault-v1-v3-corrections`. **The frozen release branch was not changed or pushed for this.**
...
| **Current published head** | `origin/release/chat-journal-rc` = `2b0868e06b1cc4e9e0576c408a98a2513776e3cf` | Confirmed here with `git ls-remote`. [Run 36151931576](https://github.com/tamanitomo/tamanitomo/actions/runs/36151931576) targets that exact SHA: **completed / success, all 5 jobs** (linux 3.11/3.13/3.14, windows and macos smoke). The run result is from the reviewer's read-only GitHub check; this executor has no `gh` here and did not re-query it. It does not include Vault. |
...
### Vault editor (separate local branch; not in this candidate, affects no A/B/C decision)

The Vault editor/safe-save slice (`11ce49a`, report `6d72013`) is on local branches only and is not in
`release-files.json` of the frozen candidate. Its review found three controller defects in
`kit/app/static/vault-editor.js`. They are scoped to Vault and do not block B-controlled, B-public or C.
