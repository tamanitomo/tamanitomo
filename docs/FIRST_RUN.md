...
1. Extract the ZIP into a permanent writable folder and double-click `companion.cmd`. First launch downloads dependencies and opens the local app.
...
4. Run **Test saved model chain**, then send a chat message. These are real provider requests and can cost money. Verify the response appears in conversation history.
5. Review and approve the companion hooks, choose gateway ownership, configure Telegram if wanted, and install/start the gateway. Check **Jobs & health** for installed schedules and errors before activating routines.
...
7. Photos remain optional. Configure an image provider, try one image, and choose a recurring schedule only when comfortable with the cost. Check Photos, Timeline, and the vault after generation.
8. Set relationship pace in **Preferences → Relationship**. The two computed indicators start enabled and can be switched off there.
