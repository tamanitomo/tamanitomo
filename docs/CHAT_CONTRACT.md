...
- **Failures.** `503 source_unavailable` is shown as "cannot be read right now", never as an empty conversation. In an enabled build, that error carries the current scope-checked `projection_id` (or `null` when it cannot be read); it never carries message content. Rows already read stay, marked as read earlier, **only when that id equals the one they were read under**. A changed or missing id, a `401`, or `409 resync_required` / `400 invalid_cursor` on an older page first removes all server-derived history, links and reply copies (`ChatStore.invalidateHistory`). The latter two then cause exactly one fresh snapshot. This is because `Projection.sync` commits a revoked or changed binding's reset before it reads the source, so a failed read can follow a purge (review 5027108, C1). The editable draft and an unsettled pending intent are kept; the intent is shown as request status, not as a sent message. A disabled build's error response is unchanged.
...
- A readable send ledger (`ok`) is applied, and no ledger (`none`, no keyed send ever made) reads as before. Two states raise `EvidenceUnavailable`: an **unreadable** ledger (`unavailable`), and **known lost provenance** (`incomplete`: an explicit reset recorded `provenance_lost_at`). In both, a row once proven to be a Hermes continuation note could be taken as the owner's words (review edfee06, R1). Evidence is refused, never read with a warning. The Chat projection keeps its own disclosed degraded display (§8.4); only these evidence consumers refuse. A missing store (`fn(None)`) means no conversation has happened yet. A store that cannot be read is an outage, never an empty conversation.
...
- An unreadable store, an unreadable send ledger, or known lost provenance raises before an attempt is counted or the model is asked. Nothing is recorded and the watermark does not move.
...
- **Refusal.** An evidence refusal (unreadable store, unreadable ledger, known lost provenance) gives the same "could not be checked this turn" diagnostic and quotes nothing.
- **Known issues, accepted (review edfee06).** N1: the cap bounds the quoted lines only. The header and the omission line come on top, so a full section measured 1,911 characters at a cap of 1,800. N2: when the 400-row scan is used up by newer rows (for example the destination session's own) before it reaches an eligible row from another channel, the handoff is left out **with no omission notice**. An incomplete scan is not proof that nothing happened elsewhere.
