...
def without_snapshot(sidecar,raw,agent):
    ...
    known_end=('Never treat absence of records as proof of never.',
               'Absence of records is not proof of never.')
    if f'[{agent} — continuity' not in candidate or not candidate.endswith(known_end):return None
...
def select_messages(request_messages,conversation_messages,agent):
    ...
    for row in conversation_messages or []:
        ...
        sidecar=row.get('api_content');raw=row.get('content')
        cleaned=without_snapshot(sidecar,raw,agent)
        if cleaned is not None:replacements[sidecar]=cleaned
    candidates=[i for i,row in enumerate(request_messages)
                if row.get('role')=='user' and isinstance(row.get('content'),str)
                and row['content'] in replacements]
    if len(candidates)<2:return None
    keep=candidates[-1];out=list(request_messages)
...
def make_engine(config,agent_name):
    ...
    class CompanionLocalContext(ContextCompressor):
        @property
        def name(self):return 'companion-local'
        def __init__(self,cfg):
            self._initial_config=copy.deepcopy(cfg)
            comp=cfg.get('compression',{});model=cfg.get('model',{})
            super().__init__(model=model.get('default',''),
                threshold_percent=comp.get('threshold',.75),
                summary_target_ratio=comp.get('target_ratio',.4),
                protect_first_n=comp.get('protect_first_n',3),
                protect_last_n=comp.get('protect_last_n',20),
                quiet_mode=True,config_context_length=model.get('context_length'),
                base_url=model.get('base_url',''),provider=model.get('provider',''),
                abort_on_summary_failure=comp.get('abort_on_summary_failure',False),
                max_tokens=cfg.get('max_tokens'),model_thresholds=comp.get('model_thresholds'),
                threshold_tokens_cap=comp.get('threshold_tokens'),
                proactive_prune_tokens=comp.get('proactive_prune_tokens',0),
                proactive_prune_min_result_chars=comp.get('proactive_prune_min_result_chars',8000),
                proactive_prune_min_reclaim_tokens=comp.get('proactive_prune_min_reclaim_tokens',4096),
                min_tail_user_messages=comp.get('min_tail_user_messages',1),
                tail_mode=comp.get('tail_mode','lean'),custom_providers=cfg.get('custom_providers'))
