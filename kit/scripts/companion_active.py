...
HEADER=('<!-- Assembled by companion_active.py from the presence state, the open-loops ledger and\n'
...
def right_now(c,now):
    ...
    scene=current(c)
    if not scene:
        return 'No state recorded yet. Establish one before describing a current scene.\n'
    state=scene['state']
    when=dt.datetime.fromisoformat(scene['recorded_at'])
    outfit=', '.join(item['description'] for item in state.get('outfit',[]))
    lines=[f"_recorded {_age(when,now)}_",'',
...
def _read(path,limit=1500):
    try:return path.read_text(encoding='utf-8')[:limit].strip()
...
def ambient(c,skip=('relationship-thread','bars')):
    ...
    for path in sorted(folder.glob('*.md')):
        ...
        body=_read(path,800)
        if body:out.append(f'### {path.stem}\n{body}')
    return '\n\n'.join(out)
...
def recent_conversation(c,now=None):
    ...
    try:
        ...
        if not thread_data or not thread_data.get('available') or not thread_data.get('tail'):
            return ''
        lines=[]
        ...
        if hours is not None:
            if hours < (1/60):
                ...
                lines.append(f"_Last exchanged messages ~{mins} minute{'s' if mins!=1 else ''} ago._")
            else:
        ...
        lines.append('If the human recently persuaded you, made a suggestion (e.g. taking a bath, relaxing in the hot tub, an outing), or if you agreed on an activity or promised to follow up later, honor that agreement! Conversational reality takes priority over default scheduled routine anchors.')
        return '\n'.join(lines)
    except Exception:
        return ''
...
def intended_plan(c,now=None):
    ...
    try:
        ...
        if plan.get('theme'):lines.append(f"**Theme / Archetype**: {plan.get('theme')}")
        outfit=plan.get('laid_out_outfit')
        if outfit:lines.append(f"**Clothes laid out last night**: {', '.join(outfit)}")
        if plan.get('notes'):lines.append(f"**Notes**: {plan.get('notes')}")
        lines.append('')
        ...
        lines.append('FORGOING / ADAPTING PLANS: Crucially, she is never trapped by her schedule. If conversation with the human suggests an activity, an outing, or de-rails the plan, she gladly adapts or forgoes the scheduled plan to live in the moment with them.')
        return '\n'.join(lines)
    except Exception:
        return ''
...
def recent_photo(c):
    ...
    try:
        ...
        return (f"Latest timeline image: `{info['path']}`{desc}\n"
...
def build(c,now=None):
    ...
    if c.bars:
        ...
        except (OSError,ValueError,KeyError,TypeError):feelings='Contextual feelings unavailable; do not infer intent from silence.'
    sections=[('Right now',right_now(c,now)),
...
def write(c,now=None):
    path=c.soul_dir/'ActiveContext.md';path.parent.mkdir(parents=True,exist_ok=True)
    text=build(c,now)
    ...
    try:
        if path.read_text(encoding='utf-8')==text:return {'written':False,'path':str(path)}
    except OSError:pass
...
def main():
    ...
    p.add_argument('--show',action='store_true',help='print the assembly without writing it')
    a=p.parse_args();c=cc.load(a.home)
    if a.show:print(build(c))
    else:print(json.dumps(write(c),ensure_ascii=False,indent=2))
