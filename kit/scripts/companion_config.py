...
def detected_for_real(how: str) -> bool:
...
AGENT_TYPES = {
    "companion": "A full life: a present, moods, a relationship, and permission to write first.",
...
JOB_TIERS = {
    "loops": "the 15- and 30-minute loops that keep the present moving",
...
def soul_cap(context_tokens: int) -> int:
    ...
    return _clamp(
        context_tokens * HERMES_CHARS_PER_TOKEN * HERMES_CONTEXT_FILE_FRACTION,
        HERMES_CONTEXT_FILE_FLOOR,
        HERMES_CONTEXT_FILE_CEILING,
    )
...
def vault_index_cap(context_tokens: int, setting: int = -1) -> int:
    ...
    if setting < 0:
        tokens = min(
            int(context_tokens * VAULT_INDEX_FRACTION), VAULT_INDEX_AUTO_MAX_TOKENS
        )
    else:
        tokens = setting
...
class Companion:
    ...
    context_mode: str = (
        "auto"  # auto follows Hermes on each load; fixed is an intentional kit override
    )
    ...
    autonomy_windows: list = dataclasses.field(
        default_factory=lambda: ["10:20", "20:20"]
    )
    ...
    def __post_init__(self):
        if self.relationship_progression not in ("off", "subtle", "milestones"):
            raise ValueError(
                "relationship_progression must be off, subtle, or milestones"
            )
        ...
        if self.relationship_started:
            ...
            try:
                _dt.date.fromisoformat(self.relationship_started)
            except (TypeError, ValueError):
                raise ValueError(
                    "relationship_started must be a date as YYYY-MM-DD, or empty"
                )
        if not isinstance(self.peer_interaction, bool):
            raise ValueError("peer_interaction must be true or false")
        if not isinstance(self.adult_images, bool):
            raise ValueError("adult_images must be true or false")
        if self.adult_images and not self.explicit:
            raise ValueError(
                "Adult images require a romantic relationship; enable romance first"
            )
        ...
        if self.explicit:
            ...
            if self.current_age() < 18 or self.boundary in NON_ROMANTIC:
                raise ValueError(
                    "Adult themes require an adult companion and an eligible relationship frame"
                )
        ...
        if not isinstance(self.remote_pin, str) or (
            self.remote_pin
            and (not self.remote_pin.isdigit() or len(self.remote_pin) != 4)
        ):
            raise ValueError("remote_pin must be a 4-digit numeric PIN, or empty")
        if (
            not isinstance(self.sleep_grace_minutes, int)
            or not 0 <= self.sleep_grace_minutes <= 240
        ):
            raise ValueError("sleep_grace_minutes must be between 0 and 240")
        if not isinstance(self.adaptive_quiet, bool):
            raise ValueError("adaptive_quiet must be true or false")
        if not isinstance(self.content_permissions, dict):
            raise ValueError("content_permissions must be an object")
        for key, value in self.content_permissions.items():
            if key not in CONTENT_KINDS:
                raise ValueError(
                    f"unknown content kind {key!r}; expected {sorted(CONTENT_KINDS)}"
                )
            if value not in PERMISSIONS:
                raise ValueError(f"{key} permission must be one of {PERMISSIONS}")
        if self.birthdate:
            import datetime as _dt

            try:
                born = _dt.date.fromisoformat(self.birthdate)
            except (ValueError, TypeError):
                raise ValueError("birthdate must be YYYY-MM-DD")
            years = _dt.date.today().year - born.year
            if not 18 <= years <= 120:
                raise ValueError("birthdate must make the companion an adult under 120")
        if type(
            self.image_interval_minutes
        ) is not int or self.image_interval_minutes not in (
            5,
            10,
            15,
            20,
            ...
            raise ValueError(
                "Choose a supported image interval between 5 minutes and 24 hours"
            )
        if (
            type(self.schedule_offset_minutes) is not int
            or not 0 <= self.schedule_offset_minutes < 15
        ):
            raise ValueError("Schedule offset must be between 0 and 14 minutes")
        if not isinstance(self.bars, bool):
            raise ValueError("bars must be true or false")
        if (
            isinstance(self.timeline_budget_gb, bool)
            or not isinstance(self.timeline_budget_gb, (int, float))
            or not 0 <= self.timeline_budget_gb <= 1000
        ):
            raise ValueError(
                "timeline_budget_gb must be a number of gigabytes between 0 and 1000"
            )
        if self.agent_type not in AGENT_TYPES:
            raise ValueError(f"agent_type must be one of {sorted(AGENT_TYPES)}")
        if (
            not isinstance(self.autonomy_windows, list)
            or len(self.autonomy_windows) > 6
        ):
            raise ValueError(
                "autonomy_windows must be a list of at most six HH:MM times"
            )
        for value in self.autonomy_windows:
            try:
                import datetime as _dt

                _dt.time.fromisoformat(
                    str(value) if len(str(value)) == 5 else str(value) + ":00"
                )
            except (ValueError, TypeError):
                raise ValueError(f"{value!r} is not an HH:MM time")
        if not isinstance(self.share_people, bool):
            raise ValueError("share_people must be true or false")
        if not isinstance(self.sensors, list) or any(
            not isinstance(s, str) for s in self.sensors
        ):
        ...
        def validate_model(value, fallback=False):
            ...
            if any(
                not isinstance(v, str) or len(v) > 500 or any(ord(ch) < 32 for ch in v)
            ...
            if value.get("base_url"):
                ...
                if (
                    ...
                    raise ValueError(
                        "Use an HTTP(S) model tier URL without credentials"
                    )
            for field in ("api_key_env", "key_env"):
                if value.get(field) and not re.fullmatch(
                    r"[A-Za-z_][A-Za-z0-9_]*", value[field]
                ):
                    raise ValueError(
                        "Model credentials must name an environment variable"
                    )
        ...
        for key, value in self.models.items():
            if key == "fallbacks":
                if not isinstance(value, list) or len(value) > 8:
                    raise ValueError(
                        "models.fallbacks must be a list of at most eight providers"
                    )
            ...
            if key not in JOB_TIERS:
                raise ValueError(
                    f"unknown model tier {key!r}; expected {sorted(JOB_TIERS)}"
                )
        ...
        if self.pronoun_set not in PRONOUNS or self.human_pronoun_set not in PRONOUNS:
            raise ValueError("Invalid pronouns")
        if not isinstance(self.image_timeline, bool):
            raise ValueError("image_timeline must be true or false")
        if self.context_mode not in ("auto", "fixed"):
            raise ValueError("Invalid context mode")
        if (
            not isinstance(self.context_tokens, int)
            or not 2048 <= self.context_tokens <= 10_000_000
        ):
            raise ValueError("Invalid context window")
        if not isinstance(self.cron_active, bool):
            raise ValueError("cron_active must be true or false")
        if (
            not isinstance(self.outreach_per_day, int)
            or not 0 <= self.outreach_per_day <= 100
        ):
            raise ValueError(
                "outreach_per_day must be a whole number of messages, 0 for no limit"
            )
        if not isinstance(self.soul_in_vault, bool):
            raise ValueError("soul_in_vault must be true or false")
        if (
            isinstance(self.vault_index_tokens, bool)
            or not isinstance(self.vault_index_tokens, int)
            or self.vault_index_tokens < -1
        ):
            raise ValueError(
                "vault_index_tokens must be -1 (automatic), 0 (off) or a number of tokens"
            )
        if not isinstance(self.vault_index_exclude, list) or not all(
            isinstance(x, str) for x in self.vault_index_exclude
        ):
    ...
    def birthday_in(self, today=None):
        ...
        for year in (today.year, today.year + 1):
            try:
                when = _dt.date(year, born.month, born.day)
            except ValueError:
                when = _dt.date(year, 3, 1) if born.month == 2 else None
    ...
    def home(self) -> pathlib.Path:
        ...
        return (
            self.hermes_root
            if not self.profile
            else self.hermes_root / "profiles" / self.profile
        )
    ...
    def soul_linked(self) -> bool:
        return (
            self.soul.is_symlink()
            and self.soul.resolve() == self.canonical_soul.resolve()
        )
    ...
    @property
    def life(self) -> pathlib.Path:
        return self.data / "companion-life"
    ...
    @property
    def human_dir(self) -> pathlib.Path:
        return self.people / _slug(self.human)
    ...
    @property
    def soul_dir(self) -> pathlib.Path:
    ...
    def refl(self):
        return self.pronouns[4]

    @property
    def human_pronouns(self):
        return PRONOUNS.get(self.human_pronoun_set, PRONOUNS["he"])

    def h_subj(self):
        return self.human_pronouns[0]

    def h_obj(self):
        return self.human_pronouns[1]

    def h_poss(self):
        return self.human_pronouns[2]

    # ---- budgets ---------------------------------------------------------
    @property
    def tier(self) -> str:
        return tier(self.context_tokens)

    @property
    def soul_cap(self) -> int:
        return soul_cap(self.context_tokens)

    @property
    def soul_warn(self) -> int:
        return int(self.soul_cap * 0.9)
    ...
    def budgets(self) -> dict:
        ...
        for k in keep:
            out[k] = max(_MIN_USEFUL, int(free * _WEIGHTS[k] / wsum))
        ...
        while over > 0 and keep:
            ...
            if slack:
                k = slack[0]
                take = min(over, out[k] - _MIN_USEFUL)
                out[k] -= take
                over -= take
            else:
                dropped = keep.pop()
    ...
    def to_dict(self) -> dict:
        ...
        d["hermes_root"] = str(self.hermes_root)
    ...
    def save(self, path: Optional[pathlib.Path] = None) -> pathlib.Path:
        ...
        atomic_write(
            path, json.dumps(self.to_dict(), indent=2, ensure_ascii=False) + "\n"
        )
        try:
...
def detect_context_tokens(
    home: pathlib.Path,
    hermes_root: Optional[pathlib.Path] = None,
    ...
    hermes_root = pathlib.Path(hermes_root or home)
    try:
        import yaml
    except ImportError:
    ...
    if name:
        ...
        if not base:
            ...
            for key, value in cache.items():
                ...
                if model_part != name or not sane(value):
                    continue
    ...
    for root in dict.fromkeys((home, hermes_root)):
        try:
            ...
            if base and (not native or urlparse(base).hostname != native):
                continue
            value = (
                entry.get("models", {}).get(name, {}).get("limit", {}).get("context")
            )
            if (
                isinstance(value, int)
                and not isinstance(value, bool)
...
def migrate(kwargs: dict) -> dict:
    ...
    for key, fallback in (("pronoun_set", "she"), ("human_pronoun_set", "he")):
        ...
        if isinstance(value, str) and value not in PRONOUNS:
            out[key] = fallback
    return out
...
def load(home: Optional[pathlib.Path] = None) -> Companion:
    ...
    home = (
        pathlib.Path(
            home
            or os.environ.get("TAMANITOMO_HOME")
    ...
    if path.exists():
        try:
            ...
            raise ValueError(f"Invalid companion configuration: {path}") from exc
    ...
    for k in ("hermes_root", "vault"):
        if k in kwargs:
            kwargs[k] = pathlib.Path(kwargs[k])
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {
                "agent": c.agent,
                "profile": c.profile or "(root)",
                "home": str(c.home),
                "data": str(c.data),
                "tier": c.tier,
                "context_tokens": c.context_tokens,
                "soul_cap": c.soul_cap,
                "soul_warn": c.soul_warn,
                "budgets": c.budgets(),
            },
            indent=2,
        )
...
def access_pin(root, fallback=""):
    ...
    if path.exists():
        ...
        for legacy in [
            root / CONFIG_NAME,
            *sorted((root / "profiles").glob("*/" + CONFIG_NAME)),
        ]:
            if legacy.exists():
                pin = json.loads(legacy.read_text(encoding="utf-8")).get(
                    "remote_pin", ""
                )
                if pin:
                    pins.add(pin)
    ...
    if not isinstance(value, str) or (
        value and (len(value) != 4 or not value.isascii() or not value.isdigit())
    ):
...
def save_access_pin(root, pin):
    if not isinstance(pin, str) or (
        pin and (len(pin) != 4 or not pin.isascii() or not pin.isdigit())
    ):
