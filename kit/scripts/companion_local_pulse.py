...
def schema(wardrobe, care_enabled=False, themes=None):
    ...
    fields["outfit"] = {
        ...
        "items": {
            "type": "string",
            "enum": [item["id"] for item in wardrobe]
            + ["nude", "undressed", "bathing", "towel"],
        },
    }
    ...
    if themes:
        ...
        item = {
            ...
            "properties": {
                ...
                "kind": {
                    "type": "string",
                    "enum": ["commitment", "idea", "person", "anchor", "other"],
                },
                "ref": {"type": "string", "maxLength": 80},
            },
            "required": ["start", "end", "what", "where", "kind", "ref"],
            ...
            "anyOf": [
                {
                    ...
                    "required": ["intent", "theme", "items", "outfit"],
                    "additionalProperties": False,
                },
                {"type": "null"},
            ]
...
def lay_out(chosen, closet):
    ...
    nightwear = {
        item["id"]
        for item in (closet or [])
        if isinstance(item, dict) and item.get("category") == "sleep" and item.get("id")
    }
    out = []
    for item in chosen or []:
        ident = item.get("id") if isinstance(item, dict) else str(item)
        if ident and ident not in nightwear and ident not in out:
            out.append(ident)
    return out
...
def declare_night(c, now, tomorrow=None):
    ...
    if plan and now < plan["until"]:
        return plan
    wake = companion_sleep._minutes(c.quiet_end, 8 * 60)
    starts = [
        companion_sleep._minutes(str(r.get("start", "")), None)
        for r in ((tomorrow or {}).get("items") or [])
        if isinstance(r, dict)
    ]
    starts = [m for m in starts if m is not None and 4 * 60 <= m < wake]
    if starts:
        wake = min(starts)
    return companion_sleep.declare(
        c, f"{wake//60:02d}:{wake%60:02d}", "Declared at wind-down.", now=now
    )
...
def pulse(
    ...
    if phase not in ("pulse", "morning", "winddown"):
        raise ValueError("Unknown presence phase")
    ...
    if not previous:
        raise ValueError(
            "Initialize companion presence before enabling the local pulse"
        )
    closet = presence.wardrobe(c)["items"]
    if not closet:
        raise ValueError("Initialize the companion wardrobe first")
    utc = now.astimezone(dt.timezone.utc)
    prior_time = dt.datetime.fromisoformat(previous["recorded_at"]).astimezone(
        dt.timezone.utc
    )
    ...
    if phase_id and any(
        row.get("id") == phase_id
        for row in presence.read_events(c.life, now.date().isoformat(), limit=0)
    ):
        return {"status": "skipped", "reason": "This daily phase is already recorded"}
    if (
        apply
        and phase == "pulse"
        and previous["state"].get("confirmed", True)
        and prior_time >= interval
    ):
        return {
            "status": "skipped",
            "reason": "This interval already has a model-confirmed state",
        }
    system = (
    ...
    if lifestyle.enabled(c):
        ...
        user = (
            companion_active.build(c, now)
            + "\nRoutine anchors:\n"
            + json.dumps(
                companion_life.routine(c.life, now, c.agent), ensure_ascii=False
            )
            + "\n"
    ...
    if phase == "winddown":
        ...
        offered = companion_life.suggest(
            c.life, (now.date() + dt.timedelta(days=1)), 6, now
        )
            ...
            user += (
                ...
                + "\n".join(
                    ...
                    + (
                        ...
                        + (
                            f". You tend to: {', '.join(p['together'][:3])}"
                            if p.get("together")
                            else ""
                        )
                    )
    ...
    started = day.timestamp(state.get("started_at", previous["recorded_at"]))
    elapsed = (now - started).total_seconds() / 60
    if (
        lifestyle.enabled(c)
        and state.get("duration_minutes")
        and elapsed >= state["duration_minutes"]
    ):
        user += (
            '\nPRIORITY: the old activity "'
            + state["activity"]
            + f'" has lasted {elapsed:.0f} minutes against an estimate of {state["duration_minutes"]}. '
    ...
    for attempt in range(2):
        ...
        if reply.get("reasoned") is False:
            print(
                "warning: model returned no reasoning; routine and wardrobe rules are easy to miss without it",
                file=sys.stderr,
            )
        if reply.get("finish_reason") not in ("stop", None):
            raise ValueError("Local pulse response was incomplete")
        data = json.loads(reply["content"])
        candidate = {
            **previous["state"],
            **{key: data[key] for key in ("activity", "location") if key in data},
        }
        import companion_sleep

        candidate.update(
            day.evolve(
                data, candidate, previous, now, asleep=companion_sleep.asleep(c, now)
            )
        )
        try:
            ...
            payload["messages"] += [
                ...
                {
                    "role": "user",
                    "content": "The proposed record was NOT saved. "
                    + str(exc)
                    + " Return the full corrected JSON record for the same checkpoint.",
                },
            ]
            continue
        break
    ...
    if not apply:
        return {
            ...
            "model": route["model"],
            "provider": route["provider"] or route["base_url"],
        }
    ...
    if data.get("commitments"):
        try:
            ...
            companion_plan.sync_commitments(
                c, presence.current(c)["state"].get("commitments"), now=now
            )
        except Exception as exc:
            print(
                f"note: commitments not folded into the plan ({exc})", file=sys.stderr
            )
    if phase == "winddown":
        try:
            declare_night(c, now, data.get("tomorrow"))
    ...
    if phase == "winddown" and isinstance(data.get("tomorrow"), dict):
        ...
        seeing = [
            str(r.get("ref"))
            for r in rows
            if r.get("kind") == "person" and r.get("ref")
        ]
        ...
        if theme not in companion_life.themes(c.life):
            print(
                f"note: {theme!r} is not one of her day-shapes; recording the plan as custom",
                file=sys.stderr,
            )
        ...
        for row in rows:
            try:
                candidate = companion_plan.clean_item(row)
            except ValueError as exc:
                refused.append(str(exc))
                continue
            if companion_plan.conflicts(items, candidate):
                refused.append(
                    f"{candidate['what'][:60]} overlaps something already in the day"
                )
                continue
            items.append(candidate)
        try:
            companion_plan.settle(
                c,
                target,
                plan.get("intent") or "an ordinary day",
                theme=theme,
                items=items,
                now=now,
            )
        ...
        companion_life.save_laid_out(
            c.life, target, lay_out(plan.get("outfit"), closet)
        )
        # Recorded by id, so what she has done lately is a fact rather than
        # something read back out of her own descriptions of it.
        try:
            companion_life.record_choice(c.life, taken, target.isoformat())
        except ValueError:
            pass  # An id she invented is not a reason to lose the plan.
        for who in seeing:
            try:
                companion_life.saw_person(c.life, who, target.isoformat())
            except ValueError:
                pass
    return {
        "status": "recorded",
...
def main():
    ...
    parser.add_argument(
        "--base-url",
        default="",
        help="Override the configured model with a specific endpoint",
    )
    parser.add_argument("--model", default="")
    parser.add_argument(
        "--tier",
        default="loops",
        choices=worker.TIERS,
        help="Which of the companion's configured models to use",
    )
    parser.add_argument("--slot", type=int, default=1)
    parser.add_argument("--preview", action="store_true")
    parser.add_argument(
        "--phase", choices=["pulse", "morning", "winddown"], default="pulse"
    )
