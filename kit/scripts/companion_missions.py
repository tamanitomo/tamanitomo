...
def add(c, entry, now):
    ...
    row = {
        "id": entry.get("id")
        or "mis-" + hashlib.sha256(title.lower().encode()).hexdigest()[:16],
        "kind": "mission",
        "title": title,
...
def update(c, ident, status, detail="", now=None):
    ...
    return slf._append(
        path_for(c),
        {
            ...
            "kind": "mission_update",
            "status": status,
            "detail": _text(detail, 1500, "detail", required=False),
            ...
            try:
                since = (
                    ...
                    - dt.datetime.fromisoformat(
                        item.get("updated_at") or item["recorded_at"]
                    )
                ).total_seconds() / 3600
            except ValueError:
                since = 0
...
def render(c, now=None):
    ...
    for row in rows:
        out.append(
            f"- {row['title']}"
            + (f" (wanted by {row['wanted_by']})" if row.get("wanted_by") else "")
        )
        if row.get("detail"):
            out.append(f"  {row['detail'][:300]}")
    return "\n".join(out)
...
def batch(c, path, now):
    ...
    if not isinstance(data, list) or not data:
        raise ValueError("file must hold a list of missions")
    if len(data) > 20:
        raise ValueError("at most 20 missions at once")
    results = []
...
def main():
    ...
    if a.cmd == "add":
        ...
        out = {"missions": missions(c, None if a.status == "all" else a.status, now)}
    elif a.cmd == "claim":
        got = claim(c, now)
        out = got or {
            "claimed": None,
            "reason": "nothing open; this window is yours to spend on your own life",
        }
    elif a.cmd == "done":
        out = update(c, a.id, "done", a.detail, now)
    else:
        out = update(c, a.id, "open", a.detail or "released without finishing", now)
