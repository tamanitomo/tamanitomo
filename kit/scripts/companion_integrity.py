...
def compute_checksum(locked_settings: Dict[str, Any]) -> str:
    canonical = json.dumps(
        {k: locked_settings.get(k) for k in LOCKED_FIELDS},
        sort_keys=True,
        ensure_ascii=False,
    )
...
def verify_integrity(c) -> Dict[str, Any]:
    ...
    target = (
        path
        if path.exists()
        else (data_path if (data_path and data_path.exists()) else None)
    )
    ...
    if record.get("checksum") != expected_checksum:
        ...
        try:
            ...
            atomic_write(
                target, json.dumps(record, indent=2, ensure_ascii=False) + "\n"
            )
        except Exception:
            pass
...
def revoke_nsfw(c) -> Dict[str, Any]:
    ...
    revoked_data = {
        "revoked_at": now_iso,
        "reason": "Turned off by user; permanently locked at friendship",
    }
