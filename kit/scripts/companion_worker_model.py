...
def resolve(c, tier="loops"):
    ...
    if not route["model"]:
        raise ValueError(
            f"No model configured for the {tier} tier, and Hermes has no default"
        )
    return route
...
def _bridge(c, payload):
    ...
    run = subprocess.run(
        ...
        capture_output=True,
        text=True,
        timeout=payload.get("timeout", 300) + 60,
    )
    try:
        reply = json.loads(
            next(
        ...
        raise ProviderFailure(
            "Hermes text provider is unavailable; inspect its setup"
            + (f" ({run.stderr.strip()[:200]})" if run.stderr.strip() else "")
        )
    if isinstance(reply, dict) and reply.get("error"):
        raise ProviderFailure(
            reply["error"], reply.get("status_code"), reply.get("transient")
        )
    if run.returncode:
        raise ValueError("Hermes text provider failed")
    return reply
...
def complete(
    ...
    *,
    ...
    routes = (
        inference.configured_routes(c, primary=route)
        if allow_fallback
        else [inference.normalize(route)]
    )
    primary = routes[0]
    ...
    def attempt(selected):
        ...
        key = inference.credential(
            c,
            selected,
            api_key_env if selected is primary else "",
            primary=selected is primary,
        )
        mode = selected.get("api_mode")
        native_messages = selected["provider"].lower() in (
            "anthropic",
            "anthropic-oauth",
        )
        if (
            selected["direct"]
            and mode in (None, "", "chat_completions")
            and (not native_messages or mode == "chat_completions")
        ):
            ...
            request = urllib.request.Request(
                ...
                data=json.dumps(
                    companion_endpoint.shape(
                        body, selected["base_url"], require_thinking=require_thinking
                    )
                ).encode(),
                headers=headers,
            )
            ...
            if (
                not reasoned
                and not details
                and not companion_endpoint.is_loopback(selected["base_url"])
            ):
                reasoned = None
            return {
        ...
        return _bridge(
            ...
            {
                "provider": selected["provider"]
                or ("custom" if selected["base_url"] else ""),
                "model": selected["model"],
                "base_url": selected["base_url"],
                "api_key": key
                or (
                    "no-key-required"
                    if selected["base_url"] or selected.get("api_key_env")
                    else ""
                ),
                "api_mode": selected.get("api_mode") or "",
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {key: value for key, value in route.items() if key != "api_key"}, indent=2
        )
    )
