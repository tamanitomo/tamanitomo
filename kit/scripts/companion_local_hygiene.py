...
def run(c, now=None, apply=False):
    ...
    targets = [
        (c.soul_dir / "continuity/Autonomy.md", 7),
        (c.soul_dir / "Lifelog.md", 180),
    ]
    for path, days in targets:
        if not path.exists():
            continue
        if path.is_symlink():
            raise ValueError("Linked journal is not a housekeeping target")
        entries = rotate.split_entries(path.read_text(encoding="utf-8"))[1]
        if any(rotate.entry_date(e) is None for e in entries):
            raise ValueError("Undated journal heading: " + str(path))
        report = rotate.rotate(path, today=now.date(), keep_days=days)
        if report["entries"] != report["keep"] + report["archive"]:
            raise ValueError("Rotation count mismatch")
        reports.append((path, days, report))
    result = {
        "mode": "apply" if apply else "preview",
        "rotation": [],
        "memory": memory.status(c),
        "prune": prune.prune(c, now=now),
        "soul": slf.soul_show(c),
        "open_loops": loops.loops(c, None),
    }
    if apply:
        result["memory_maintenance"] = memory.maintain(c)
        result["rotation"] = [
            rotate.rotate(p, today=now.date(), keep_days=d, apply=True)
        ...
        result["rotation"] = [r for _, _, r in reports]
    built = context.build(c, now=now)
    if not isinstance(built, str) or not built.strip():
        raise ValueError("Continuity builder returned no context")
