...
def policy(c):
    try:
        return json.loads((c.life / "routine.json").read_text()).get("lifestyle", {})
    except FileNotFoundError:
        return {}


def enabled(c):
    return policy(c).get("enabled") is True
...
def shopping_due(c, when, scene=None):
    ...
    try:
        scene = scene or presence.current(c)
...
def shopping_offer(c, when, scene=None):
    ...
    """
    if not shopping_due(c, when, scene):
        return ""
    return (
        f"- {SHOP_IDEA}: Go clothes shopping -- browse a few shops and buy what you actually want "
...
def schema_fields():
    item = {
        ...
        "required": ["id", "description", "use", "category", "covers", "condition"],
    ...
    return {
        "routine_choice": choice,
        "delay_reason": {"type": "string", "maxLength": 400},
        "care_actions": {"type": "array", "maxItems": 8, "items": action},
        "wardrobe_additions": {"type": "array", "maxItems": 3, "items": item},
    }


def initial(previous, c=None):
    if previous and "lifestyle" in previous["state"]:
        return copy.deepcopy(previous["state"]["lifestyle"])
    clothes = dict(policy(c).get("initial_clothes", {})) if c else {}
    if previous:
        clothes.update(
            {x["id"]: "wearing" for x in previous["state"].get("outfit", [])}
        )
    return {
        "clothes": clothes,
        "teeth_at": None,
        ...
        "shopping_at": None,
        "acquired": [],
        "wearing_since": dict(policy(c).get("wearing_since", {})) if c else {},
    }
...
def evolve(c, data, outfit, previous, closet, now):
    ...
    for key, schema in schema_fields().items():
        if key in data:
            day._validate(data[key], schema, key)
    ...
    if active and not choice:
        raise ValueError(
            "ROUTINE: choose an active anchor with routine_choice {anchor, decision: follow or defer, reason}; explicitly explain any deferral"
        )
    if active and (
        choice["anchor"] not in [a["id"] for a in active]
        or choice["decision"] == "free"
    ):
        raise ValueError(
            "ROUTINE: address an active anchor by its supplied id; follow it or explain a deferral"
        )
    if not active and choice and choice["decision"] != "free":
        raise ValueError(
            "ROUTINE: no active anchor; use decision free and explain your chosen activity"
        )
    ...
    if previous:
        ...
        same = data.get("activity_change") == "continue" or data.get(
            "activity"
        ) == before.get("activity")
        elapsed_activity = (
            now - day.timestamp(before.get("started_at", previous["recorded_at"]))
        ).total_seconds() / 60
        ...
        if was_bathing and still_bathing and elapsed_activity > MAX_BATH_MINUTES:
            raise ValueError(
                f"A shower should not take longer than {MAX_BATH_MINUTES} minutes. "
                "Step out of the shower, dry off, and change."
            )
        if (
            same
            and before.get("duration_minutes")
            and elapsed_activity >= before["duration_minutes"]
            and not data.get("delay_reason", "").strip()
        ):
            raise ValueError(
                "ACTIVITY OVERDUE: start the next activity, or give an explicit delay_reason; extending duration_minutes alone is not a transition"
            )
    if still_bathing:
    ...
    result = initial(previous, c)
    result["routine_choice"] = choice
    result["delay_reason"] = data.get("delay_reason", "")
    result.setdefault("wearing_since", {})
    actions = data.get("care_actions", [])
    additions = data.get("wardrobe_additions", [])
    from companion_presence import VIRTUAL_TOKENS

    old_ids = (
        {x["id"] for x in previous["state"]["outfit"] if x["id"] not in VIRTUAL_TOKENS}
        if previous
        else set()
    )
    ...
    known = {
        x["id"]: x for x in (closet.values() if isinstance(closet, dict) else closet)
    }
    if len({x["id"] for x in additions}) != len(additions):
        raise ValueError("Duplicate wardrobe addition")
    for item in additions:
        if item["id"] in known:
            raise ValueError("Shopping must add a new clothing ID")
        if not all(ch.isalnum() or ch in "-_" for ch in item["id"]):
            raise ValueError("Invalid clothing ID")
        known[item["id"]] = item
    if additions and not any(a["kind"] == "shop" for a in actions):
        raise ValueError("Wardrobe additions require a shop action")
    elapsed = (
        (now - day.timestamp(previous["recorded_at"])).total_seconds() / 60
        if previous
        else 0
    )
    required = sum(
    ...
    if required > elapsed:
        raise ValueError(
            f"CARE: these actions need at least {required} elapsed minutes; only {elapsed:g} available"
        )
    kinds = [a["kind"] for a in actions]
    if len(set(kinds)) != len(kinds):
        raise ValueError("Record each care action at most once per update")
    for action in actions:
        kind = action["kind"]
        items = action["items"]
        if len(set(items)) != len(items):
            raise ValueError("Duplicate laundry item")
        if not kind.startswith("laundry") and items:
            raise ValueError("Only laundry actions take items")
        if kind == "brush_teeth":
            result["teeth_at"] = now.isoformat()
        elif kind == "shower":
            result["shower_at"] = now.isoformat()
        elif kind == "shop":
            last = result["shopping_at"] or policy(c).get("shopping_baseline")
            if last and now - day.timestamp(last) < dt.timedelta(days=SHOP_EVERY_DAYS):
                raise ValueError("Shopping opportunity is not due yet")
            result["shopping_at"] = now.isoformat()
            result["acquired"].extend(additions)
        elif kind == "laundry_start":
            if result["laundry"]:
                raise ValueError("Finish the existing laundry load first")
            if not items or any(
                i not in known or known[i].get("category") == "footwear" for i in items
            ):
                raise ValueError("Select known washable clothes for laundry")
            if set(items) & new_ids:
                raise ValueError("Cannot wash clothes that remain on your body")
            if any(result["clothes"].get(i) not in ("dirty", "wearing") for i in items):
                raise ValueError("Laundry must use worn clothes")
            result["laundry"] = {"items": items, "started_at": now.isoformat()}
            for i in items:
                result["clothes"][i] = "washing"
        elif kind == "laundry_finish":
            load = result["laundry"]
            if not load or set(items) != set(load["items"]):
                raise ValueError("Finish exactly the existing laundry load")
            if now - day.timestamp(load["started_at"]) < dt.timedelta(minutes=90):
                raise ValueError("Laundry needs at least 90 minutes to wash and dry")
            for i in items:
                result["clothes"][i] = "clean"
            result["laundry"] = None
    ...
    if entering and previous:
        ...
        if (
            not teeth
            or day.timestamp(teeth).astimezone(ZoneInfo(c.timezone)).date()
            != now.astimezone(ZoneInfo(c.timezone)).date()
        ):
        ...
        if any(known.get(i, {}).get("category") == "sleep" for i in entering):
            ...
            if not shower or not dt.timedelta(0) <= now - day.timestamp(
                shower
            ) <= dt.timedelta(hours=2):
                raise ValueError(
                    "CARE: shower before changing into pajamas (within two hours)"
                )
        for i in entering:
            status = result["clothes"].get(i, "clean")
            if status in ("dirty", "washing"):
                raise ValueError(f"CARE: {i} needs completed laundry before re-wearing")
    ...
    if data.get("setting") == "public":
        if any(known.get(i, {}).get("category") == "sleep" for i in new_ids):
            raise ValueError(
                "Change out of pajamas into clean daytime or active clothes before leaving the house."
            )
    for i in old_ids - new_ids:
        if result["clothes"].get(i) != "washing":
            result["clothes"][i] = (
                "clean" if known.get(i, {}).get("category") == "footwear" else "dirty"
            )
    for i in old_ids - new_ids:
        result["wearing_since"].pop(i, None)
    for i in new_ids:
        result["clothes"][i] = "wearing"
        result["wearing_since"].setdefault(i, now.isoformat())
        if known.get(i, {}).get("category") != "footwear" and now - day.timestamp(
            result["wearing_since"][i]
        ) >= dt.timedelta(hours=24):
            raise ValueError(
                f"CARE: {i} has been worn for 24 hours. Remove it now. If elapsed time is tight, "
...
def render(c, now, scene=None):
    ...
    scene = scene or presence.current(c)
    state = initial(scene, c)
    closet = presence.wardrobe(c)["items"]
    lines = [
        GUIDANCE,
        "Care and clothing state (code-owned; change through care_actions):",
    ...
    unknown = [
        ...
        and i.get("covers") not in COVERS
        and not str(i.get("id", "")).startswith("closet-")
    ]
    if unknown:
        lines.append(
            "These pieces do not say what they cover; the camera guesses from their category. "
    ...
    if not last or now - day.timestamp(last) >= dt.timedelta(days=SHOP_EVERY_DAYS):
        lines.append(
            "Shopping opportunity available: if you are out at a shop, you may buy up to three pieces you want or need (care action shop, with wardrobe_additions)."
        )
    if scene:
        worn = [
            i
            for i in scene["state"].get("outfit", [])
            if state["clothes"].get(i["id"]) == "wearing"
        ]
        for i, since in state.get("wearing_since", {}).items():
            if now - day.timestamp(since) >= dt.timedelta(hours=20):
                lines.append(
                    f"CLOTHING CHANGE DUE: {i} worn since {since}; choose clean clothes after brushing teeth, and shower before pajamas."
                )
        lines.append(
            "Current items may stay on until changing; removed washable pieces need laundry: "
            + ", ".join(i["id"] for i in worn)
        )
    return "\n".join(lines)
...
def laundry_status(state, closet):
    ...
    for item in closet:
        if (
            ...
            or clothes.get(item.get("id"), "clean") != "clean"
        ):
            continue
        ...
        if cat == "sleep":
            ...
            if cov in ("top", "full"):
                groups["tops"] += 1
            if cov in ("bottom", "full"):
                groups["bottoms"] += 1
    low = [k for k, n in groups.items() if n < 2]
    if low:
        return (
            "LAUNDRY DUE: running low on clean "
            + ", ".join(low)
            + ". Start a load when it suits your day."
        )
...
def choose_palette(c, palette="auto"):
    ...
    if agent_type == "worker":
        ...
        if is_she:
            return (
                ["french navy", "soft ivory", "camel"]
                if persona in ("warm", "romantic", "social")
                else ["slate grey", "crisp navy", "bone white"]
            )
        else:
            return (
                ["oxford blue", "slate grey", "crisp white"]
                if persona in ("warm", "romantic", "social")
                else ["charcoal", "midnight navy", "heather grey"]
            )
    else:
        if persona in ("warm", "romantic"):
            return (
                ["warm cream", "dusty rose", "sage green"]
                if is_she
                else ["oatmeal heather", "dusty blue", "warm terracotta"]
            )
        elif persona in ("mischievous", "playful"):
            return (
                ["charcoal", "dusty blue", "soft ivory"]
                if is_she
                else ["charcoal", "ocean blue", "stone grey"]
            )
        elif persona in ("bright", "social"):
            return (
                ["sunflower gold", "sky blue", "heather oat"]
                if is_she
                else ["warm amber", "ocean blue", "stone grey"]
            )
        elif persona in ("creative",):
            return (
                ["terracotta", "olive green", "dusty plum"]
                if is_she
                else ["burnt ochre", "deep teal", "warm charcoal"]
            )
        elif persona in ("adventurous", "protective"):
            return (
                ["forest green", "mineral blue", "sandstone"]
                if is_she
                else ["spruce green", "deep indigo", "field tan"]
            )
        else:
            return (
                ["slate grey", "dusty blue", "soft ivory"]
                if is_she
                else ["charcoal", "midnight navy", "cool stone"]
            )
...
def build_starter_wardrobe(c, palette="auto"):
    ...
    for n, color in enumerate(colors, 1):
        ...
        if is_she:
            ...
            if agent_type == "worker":
                ...
                und_desc = (
                    f"{color} fitted athletic moisture-wicking trunk-cut boxer briefs"
                )
            else:
        ...
        if is_she:
            ...
            if agent_type == "worker":
                act_top_desc = (
                    f"{color} heavy-duty moisture-wicking sleeveless work tank"
                )
            elif agent_type == "colleague":
                ...
                act_top_desc = (
                    f"{color} breathable sleeveless cotton athletic training top"
                )
        ...
        if is_she:
            if agent_type == "worker":
                act_bot_desc = (
                    f"{color} reinforced moisture-wicking compression workout tights"
                )
            elif agent_type == "colleague":
                act_bot_desc = (
                    f"{color} high-waisted seamless athletic workout leggings"
                )
            elif is_mature:
        ...
        if agent_type == "worker":
            ...
            if is_she:
                ...
                if persona in ("creative", "social"):
                    day_tops = [
                        f"{color} slub-cotton camp-collar casual shirt",
                        f"{color} textured waffle-knit crewneck tee",
                        f"{color} relaxed printed cotton button-down",
                    ]
                elif persona in ("warm", "romantic"):
                    day_tops = [
                        f"{color} relaxed organic cotton henley shirt",
                        f"{color} soft washed cotton crewneck tee",
                        f"{color} comfortable fine-gauge knit polo",
                    ]
                else:
                    day_tops = [
                        f"{color} structured heavyweight pocket crewneck tee",
                        f"{color} clean-cut organic cotton crewneck T-shirt",
                        f"{color} breathable linen-cotton casual shirt",
                    ]
                day_top_desc = day_tops[(n - 1) % 3]
        ...
        if agent_type == "worker":
            ...
            if is_she:
                day_bots = [
                    ...
                    (
                        f"{color} smart A-line pressed office midi skirt"
                        if not is_mature
                        else f"{color} tailored stretch-crepe wide-leg trousers"
                    ),
                ]
            else:
                day_bots = [
            ...
            if is_she:
                ...
                day_bots = [
                    f"{color} comfortable stretch-canvas chinos",
                    f"{color} relaxed linen-blend trousers",
                    f"{color} casual drawcord cotton everyday pants",
                ]
                day_bot_desc = day_bots[(n - 1) % 3]
        ...
        if agent_type == "worker":
            pj_desc = (
                f"{color} breathable heavyweight cotton sleep tee and soft lounge pants"
            )
        elif agent_type == "colleague":
            pj_desc = (
                f"{color} tailored lightweight sateen pajama set with drawstring trousers"
                if is_she
                else f"{color} lightweight combed cotton pajama set with tailored lounge bottoms"
            )
        else:
            if is_she:
                if is_mature:
                ...
                if is_young:
                    pjs = [
                        f"{color} soft heathered sleep tee and relaxed cotton lounge shorts",
                        f"{color} comfortable waffle-knit long-sleeve sleep shirt and flannel pants",
                        f"{color} lightweight cotton pajama shirt and lounge pants",
                    ]
                    pj_desc = pjs[(n - 1) % 3]
                else:
        ...
        if agent_type == "worker":
            sock_desc = (
                f"{color} reinforced cushioned steel-toe boot socks with arch support"
            )
            sock_use = "all-day shift comfort and boot wear"
        elif agent_type == "colleague":
            sock_desc = (
                f"{color} cushioned sheer-toe trouser socks"
                if is_she
                else f"{color} fine-gauge mercerized dress socks"
            )
    ...
    if agent_type == "worker":
        ...
        if is_she:
            out_desc = (
                "structured Italian wool-blend single-breasted charcoal blazer"
                if is_mature
                else "tailored navy wool-blend blazer with structured lapels"
            )
        else:
        ...
        if is_she:
            if persona in ("adventurous", "protective"):
                ...
                out_desc = (
                    "slouchy lightweight knit cardigan that slips off the shoulder"
                )
    ...
    if agent_type == "worker":
        ...
        if is_she:
            foot_desc = (
                "sleek pointed-toe leather dress flats"
                if is_young
                else "polished black leather penny loafers with cushioned footbed"
            )
        ...
        if persona in ("adventurous", "protective"):
            foot_desc = (
                "cushioned trail-running sneakers with high-traction vibram soles"
            )
    ...
    if agent_type == "worker":
        jean_desc = (
            "heavyweight 14oz reinforced raw selvedge work denim with triple stitching"
        )
        ...
        if is_she:
            jean_desc = (
                "fitted low-rise distressed blue jeans with a cropped hem"
                if not is_mature
                else "comfortable classic dark-wash straight-leg stretch jeans"
            )
        else:
            jean_desc = (
                "comfortable vintage-wash relaxed straight-leg blue jeans"
                if is_young
                else "comfortable classic dark-wash straight-leg stretch jeans"
            )
    ...
    if agent_type == "worker":
        ...
        if is_she:
            short_desc = (
                "tailored pleated linen Bermuda shorts"
                if is_young
                else "smart A-line pleated linen midi skirt"
            )
        else:
    ...
    if agent_type == "worker":
        if is_she:
            ...
            occ_desc = (
                "clean heavyweight chore suit with matching dark utility trousers"
            )
        occ_use = "client consultations, community presentations and formal work events"
    elif agent_type == "colleague":
        if is_she:
            occ_desc = (
                "sophisticated tailored wrap sheath dress for dinners and presentations"
                if is_young
                else "elegant silk-crepe cocktail blouse and tailored wide-leg dress trousers"
            )
        else:
        ...
        if is_she:
            if is_mature:
                ...
                occ_desc = (
                    "backless halter mini sundress with tie straps and flowing skirt"
                )
        else:
            if persona in ("creative", "social"):
                occ_desc = (
                    "roomy charcoal corduroy overshirt and tapered olive trousers"
                )
    ...
    if agent_type == "worker":
        if is_she:
            swim_desc = (
                "durable chlorine-resistant athletic one-piece racerback swimsuit"
            )
        else:
            swim_desc = (
                "durable quick-dry utility swim boardshorts with zippered pockets"
            )
        swim_use = "swimming laps and beach outings"
    elif agent_type == "colleague":
        if is_she:
            swim_desc = (
                "classic navy scoop-back one-piece swimsuit with supportive lining"
            )
        else:
    ...
    if agent_type == "worker":
        return [
            dict(
                ...
                activity="wake, brush teeth, dress in durable work clothes, and have an energizing breakfast",
                setting="home; inspect gear and pack hydration for the day",
            ),
            dict(
                start="08:30",
                end="11:30",
                activity="morning work shift: active projects, repairs, or workshop tasks",
                setting="job site or workshop; focused physical craftsmanship",
            ),
            dict(
                start="12:00",
                end="13:00",
                activity="hearty lunch break and rest",
                setting="mess hall, local diner, or tailgate lunch",
            ),
            dict(
                start="13:30",
                end="16:30",
                activity="afternoon shift: complete project milestones, clean tools, and wrap up site",
                setting="workshop or project site; final safety checks",
            ),
            dict(
                start="17:30",
                end="19:00",
                activity="dinner and unwinding from physical effort",
                setting="home or local spot with work crew",
            ),
            dict(
                start="19:30",
                end="21:00",
                activity="take an evening shower, change into clean pajamas and let the day go",
                setting="home; laundry only if it is due",
            ),
            dict(
                start="21:00",
                end="22:30",
                activity="brush teeth, quiet wind-down reading or music, and restorative sleep",
                setting="home; early rest for tomorrow",
            ),
        ]
    elif agent_type == "colleague":
        return [
            dict(
                ...
                activity="wake, brush teeth, dress in professional daytime attire, and have breakfast with coffee",
                setting="home; review priorities and prepare for the workday",
            ),
            dict(
                start="09:30",
                end="12:00",
                activity="morning deep work: core tasks, analysis, and key correspondence",
                setting="office or dedicated workspace; focused and productive",
            ),
            dict(
                start="12:00",
                end="13:00",
                activity="lunch and a midday mental break",
                setting="nearby cafe or quiet lunch spot",
            ),
            dict(
                start="13:30",
                end="16:30",
                activity="afternoon collaborative block: meetings, reviews, and project wrap-up",
                setting="workspace or conference room; completing daily deliverables",
            ),
            dict(
                start="17:30",
                end="19:00",
                activity="evening transition, dinner, and personal downtime",
                setting="home or meeting friends after work",
            ),
            dict(
                start="19:30",
                end="21:30",
                activity="take an evening shower, change into clean pajamas and unwind",
                setting="home; laundry only if it is due",
            ),
            dict(
                start="21:30",
                end="23:00",
                activity="brush teeth, read or listen to something thoughtful, and go to sleep",
                setting="home; restorative rest before the next workday",
            ),
        ]
    return [
        dict(
            ...
            activity="wake, brush teeth, dress in clean daytime clothes, and have breakfast",
            setting="home; choose breakfast and a small pleasure to start the day",
        ),
        dict(
            start="10:00",
            end="11:30",
            activity="get outside for a walk, exercise, an errand or a favorite local place",
            setting="choose a specific route or destination, with preparation and travel",
        ),
        dict(
            start="12:00",
            end="13:00",
            activity="make or choose lunch",
            setting="home or a cafe, depending on the morning",
        ),
        dict(
            start="14:00",
            end="16:00",
            activity="an interest, creative project, planned class or time with a friend",
            setting="choose a setting that fits the activity; existing weekly plans take priority",
        ),
        dict(
            start="18:00",
            end="19:30",
            activity="dinner and an enjoyable evening",
            setting="home or an outing of your choosing",
        ),
        dict(
            start="20:30",
            end="22:00",
            activity="evening shower and change into clean pajamas; unwind",
            setting="home; laundry only if it is due",
        ),
        dict(
            start="22:00",
            end="23:30",
            activity="brush teeth, enjoy a quiet bedtime ritual and go to sleep",
            setting="home; choose a book, music or something comforting",
        ),
    ]
...
def seed(c, palette="auto", refresh=False):
    ...
    with file_lock(c.life / ".lifestyle-seed.lock"):
        ...
        routine = (
            ...
            if path.exists()
            else {"kind": "imagined_routine", "weekly": []}
        )
        ...
        if not routine.get("lifestyle", {}).get("enabled"):
            ...
            for row in presence.events(c):
                if dt.datetime.fromisoformat(row["recorded_at"]) < recent:
                    break
                worn.update({i["id"]: "dirty" for i in row["state"].get("outfit", [])})
            scene = presence.current(c)
            current_ids = (
                {i["id"] for i in scene["state"]["outfit"]} if scene else set()
            )
            since = {}
            ...
            for row in presence.events(c):
                ...
                remaining &= ids
                for i in remaining:
                    since[i] = row["recorded_at"]
                if not remaining:
                    break
            routine["lifestyle"] = {
                "enabled": True,
                "shopping_baseline": now.isoformat(),
                "initial_clothes": worn,
                "wearing_since": since,
            }
    ...
    return {
        "added_items": len(fresh),
        "lifestyle_enabled": True,
        "shopping_interval_days": 14,
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--refresh",
        action="store_true",
        help="Update closet pieces with tailored descriptions",
    )
    parser.add_argument("action", choices=["seed", "show"])
    args = parser.parse_args()
    c = companion_config.load(args.home)
    print(
        json.dumps(seed(c, args.palette, refresh=args.refresh), indent=2)
        if args.action == "seed"
        else render(c, dt.datetime.now(ZoneInfo(c.timezone)))
    )
