...
def standing_path(c):
    return c.human_dir / "standing.jsonl"


def moments_path(c):
    return c.life / "relationship.jsonl"
...
def add_standing(c, entry, now):
    ...
    row = {
        "id": entry.get("id")
        or "rule-" + hashlib.sha256(instruction.lower().encode()).hexdigest()[:16],
        "kind": "standing",
...
def retire_standing(c, ident, reason, now):
    if not any(
        r.get("id") == ident for r in slf._read(standing_path(c), kind="standing")
    ):
    ...
    return slf._append(
        standing_path(c),
        {
            ...
            "status": "retired",
            "reason": _text(reason, 300, "reason", required=False),
...
def add_moment(c, entry, now):
    ...
    if kind not in MOMENT_KINDS:
        raise ValueError(f"moment must be one of {MOMENT_KINDS}")
    text = _text(entry.get("text"), 500, "text")
    row = {
        "id": entry.get("id")
        or "rel-" + hashlib.sha256((kind + text.lower()).encode()).hexdigest()[:16],
        "kind": "moment",
        "moment": kind,
        "text": text,
        "happened_on": _text(
            entry.get("happened_on"), 20, "happened_on", required=False
        ),
        "status": "active",
        "recorded_at": now.isoformat(),
    }
...
def moments(c, status="active", moment=None):
    ...
    if moment:
        rows = [r for r in rows if r["moment"] == moment]
    return rows
...
LABELS = {
    ...
    "nickname": "What you call each other",
    "joke": "Running jokes",
    "ritual": "Rituals",
    "first": "Firsts",
    "note": "Other",
...
def render_standing(c, limit=2000):
    ...
    if not rows:
        return ""
    lines = [
        f'{r["instruction"]}' + (f" ({r['scope']})" if r.get("scope") else "")
        for r in rows
    ]
...
def render_relationship(c, limit=2000):
    ...
    grouped = {}
    for row in rows:
        grouped.setdefault(row["moment"], []).append(row)
    out = []
    for kind in MOMENT_KINDS:
        ...
        for row in grouped[kind]:
            out.append(
                "- "
                + row["text"]
                + (f" ({row['happened_on']})" if row.get("happened_on") else "")
            )
    text = "\n".join(out)
    return (
        text
...
def batch(c, path, now):
    ...
    if not isinstance(data, list) or not data:
        raise ValueError("file must hold a list of entries")
    if len(data) > 50:
        raise ValueError("at most 50 entries at once")
    results = []
    for i, entry in enumerate(data):
        if not isinstance(entry, dict):
            raise ValueError("each entry must be a JSON object")
        try:
            if entry.get("kind") == "standing":
                out = add_standing(c, entry, now)
            elif entry.get("kind") == "moment":
                out = add_moment(c, entry, now)
            elif entry.get("kind") == "retire_standing":
                out = retire_standing(
                    c, entry.get("id", ""), entry.get("reason", ""), now
                )
            elif entry.get("kind") == "retire_moment":
                out = retire_moment(
                    c, entry.get("id", ""), entry.get("reason", ""), now
                )
            else:
                raise ValueError(
                    "kind must be standing, moment, retire_standing or retire_moment"
                )
...
def main():
    ...
    st = s.add_parser("standing")
    st.add_argument("--status", choices=list(STATUSES) + ["all"], default="active")
    rl = s.add_parser("relationship")
    rl.add_argument("--status", choices=list(STATUSES) + ["all"], default="active")
    ...
    if a.cmd == "add":
        ...
        out = {"standing": standing(c, None if a.status == "all" else a.status)}
    elif a.cmd == "relationship":
        out = {
            "relationship": moments(
                c, None if a.status == "all" else a.status, a.moment
            )
        }
    else:
        out = {"standing": render_standing(c), "relationship": render_relationship(c)}
