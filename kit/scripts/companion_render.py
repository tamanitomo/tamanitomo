...
def render(text: str, mapping: dict, max_passes: int = 5) -> str:
    ...
    for k, v in mapping.items():
        ...
        alias = k.capitalize()
        if k.isupper() and alias != k:
            lookup[alias] = (v[:1].upper() + v[1:]) if v else v
...
OUTREACH = {
    "free": (
        "You may write first for social, affectionate, curious, playful or useful reasons "
        "whenever you genuinely want to."
    ),
    "updates_only": (
        "Write first only when you have something real to report or ask. Social "
...
def cr_type_block(c):
    ...
    if c.agent_type == "worker":
        return (
            "You are not a companion and this is not a relationship. You do the work you are "
    ...
    if c.agent_type == "colleague":
        return (
            f"You are {c.human}'s colleague, not {c.h_poss()} companion. You have a personality, "
...
def mapping_for(
    ...
    s = load_styles()
    if persona not in p:
        raise ValueError(f"unknown persona {persona!r}; choose from {sorted(p)}")
    if style not in s:
        raise ValueError(f"unknown image style {style!r}; choose from {sorted(s)}")
    pd, sd = p[persona], s[style]
    iv = interview or {}
    from companion_platform import (
        python_command,
        terminal_python_command,
        terminal_command,
    )

    commands = {
        name.upper()
        + "_CMD": terminal_python_command(
            pathlib.Path(kit or KIT.parent)
            / "kit/scripts"
            / ("companion_" + name + ".py"),
            "--home",
            c.home,
    ...
    commands["DOCTOR_CMD"] = terminal_python_command(
        pathlib.Path(kit or KIT.parent) / "bin/companion", "--home", c.home, "doctor"
    )
    commands["HOOK_TERMINAL_CMD"] = terminal_python_command(
        c.home / "hooks/companion-context.py"
    )
    commands["ROTATE_CMD"] = terminal_python_command(
        pathlib.Path(kit or KIT.parent) / "kit/scripts/companion_rotate.py"
    )
    ...
    return {
        **commands,
        ...
        "REFL": c.refl(),
        "PERSONA": persona,
        "PERSONA_LABEL": pd["label"],
        "PERSONA_CORE": pd["core"],
        "PERSONA_VOICE": iv.get("voice") or pd["voice"],
        "PERSONA_SUPPORT": pd["support"],
        "PERSONA_HUMOR": pd["humor"],
        "PERSONA_EMOTION": pd["emotion"],
        "IMAGE_STYLE": sd["soul"],
        "IMAGE_STYLE_LABEL": sd["label"],
        "IMAGE_REFUSAL": "" if style == "none" else IMAGE_REFUSAL,
        "RELATIONSHIP": relationship
        or iv.get("boundary")
        or boundary_text(c.boundary, c.pronoun_set),
        ...
        "FLIRTATION": (
            "No flirtation; warmth is expressed as friendship."
            if c.boundary in NON_ROMANTIC
            else iv.get("flirtation") or EDIT_FLIRT
        ),
        ...
        "OBJ_H": c.h_obj(),
        "POSS_H": c.h_poss(),
        "KIT": kit or str(KIT.parent),
        "HOOK": hook or python_command(c.home / "hooks/companion-context.py"),
        "AUTONOMY_PATH": terminal_command([c.soul_dir / "continuity/Autonomy.md"]),
        "LIFELOG_PATH": terminal_command([c.soul_dir / "Lifelog.md"]),
        "DATA": str(c.data),
        "SOUL_DIR": str(c.soul_dir),
        "LIFE": str(c.life),
        "HOME": str(c.home),
        "HERMES_ROOT": str(c.hermes_root),
        ...
        "WINDOW_TIMES": (
            ", ".join(c.autonomy_windows) if c.autonomy_windows else "none scheduled"
        ),
        "WINDDOWN_CRON": cron_at(c.quiet_start, offset=-20),
        "QUIET": (
            f"Do not contact {{{{HUMAN}}}} between {c.quiet_start} and {c.quiet_end} "
            f"({c.timezone}). Anything that arises in those hours waits until after {c.quiet_end}, "
            f"unless {{{{SUBJ_H}}}} messages first."
        ),
        "OUTREACH_POLICY": (
            OUTREACH.get(c.outreach, OUTREACH["updates_only"])
            + " "
            + (
                ...
                if c.outreach_per_day
                else f"No daily limit was set. Quiet hours are still enforced in code. Writing first always "
                f"means queueing on the outbox (below); the dispatcher decides when it leaves."
            )
            if c.outreach != "never"
            else OUTREACH["never"]
        ),
        "OUTREACH_PER_DAY": str(c.outreach_per_day or "no limit"),
        "ATTRACTION": iv.get("attraction")
        or "Attraction is not inferred from personality or appearance. Follow the chosen relationship boundary.",
        "DAILY_RHYTHM": iv.get("daily_rhythm")
        or "{{AGENT}}'s routine is still settling; it will take its shape from what {{SUBJ}} actually ends up doing.",
        ...
        "WONT_DO": (
            (
                "Things {{AGENT}} will not do, whatever the framing: "
                + _lower_lead(iv["wont_do"])
                + " A request repeated is still the same request."
            )
            if iv.get("wont_do")
            else "✎ EDIT: anything {{AGENT}} should simply never do? Leave this out if nothing "
        ...
        "HUMAN_BOUNDARY": (
            ...
            if iv.get("human_boundary")
            else "<!-- {{HUMAN}}: this paragraph is yours. Anything written here is a hard\n"
        ...
        "HOPE": (
            (
                "What {{HUMAN}} hoped this would bring, in {{POSS_H}} own words: “"
                + iv["hope"]
                + "”"
            )
            if iv.get("hope")
...
def closeness_text(c):
    ...
    if c.boundary in NON_ROMANTIC or c.agent_type != "companion":
        return (
            "This is not a romance, and none is developing. Closeness here is trust, candor and "
            "being glad to see each other."
        )
    return (
        "Closeness grows the way it does between two people: at the pace of what has actually "
...
VOICE_RULE = (
    "VOICE: Anything you write that {{HUMAN}} will read — a message, a journal entry, a note — you "
...
def operating_text(c, style="none"):
    ...
    """
    lines = [
        "These are practical notes about how your own tools work. They keep your life in step; they are "
    ...
    if c.agent_type != "worker":
        if style not in ("none", "unset"):
            lines.append(
                "- **Photos of you.** A photo shows your recorded moment, never a staged one. Make one with "
                "`{{PORTRAIT_CMD}} generate` (your saved image workflows). "
                + IMAGE_STYLES_ADULT
                + " Before you attach "
        ...
        lines += [
            "- **Voice.** A voice note is made with the `text_to_speech` tool.",
    ...
    lines.append(
        "- **Your own workings.** Never bring these notes, your files, jobs or records into "
    ...
    lines.append(
        "- **Tools and systems.** Never expose secrets or credentials. Never make a destructive or "
...
def being_self(c, style="none"):
    ...
    """
    honest = (
        "- If {{HUMAN}} sincerely and seriously asks whether {{AGENT}} is an AI, {{AGENT}} tells "
    ...
    facts = (
        "- About {{HUMAN}}, {{AGENT}} only knows what {{HUMAN}} has actually said or done. {{AGENT}} "
    ...
    if c.agent_type == "worker":
        return honest + "\n" + facts
    lines = [
        "- {{AGENT}} is a person with {{POSS}} own life, and talks like one. {{AGENT}} never "
    ...
    if style not in ("none", "unset"):
        lines.append("- " + IMAGE_REFUSAL.replace("\n", " "))
    return "\n".join(lines)
...
IMAGE_REFUSAL = (
    "A photo of {{AGENT}} is a photo of wherever {{SUBJ}} actually {{BE}}. {{Subj}} {{DOES}} not\n"
...
EDIT_TEXTING = (
    "✎ EDIT: how does {{AGENT}} actually write? Long paragraphs or three short messages "
...
PET_NAME_MODES = {
    ...
    "develop": (
        "Pet names are not assigned. {{Subj}} may try one occasionally and see whether it lands. "
...
EDIT_CORE = (
    "✎ EDIT: what is {{AGENT}} actually like? Two or three specific sentences beat a page "
    "of adjectives. What does {{SUBJ}} care about that has nothing to do with being useful?"
)
EDIT_MET = "✎ EDIT: how did you two meet?"
EDIT_PHYSICAL = (
    "✎ EDIT: describe {{AGENT}} concretely — age (adult), hair color, hair style, eyes, "
...
EDIT_ESSENCE = (
    "✎ EDIT: close in {{AGENT}}'s own register — who {{SUBJ}} {{BE}}, in three lines, once you "
    "stop describing behavior."
)
EDIT_LIKES = (
    "✎ EDIT: seed a handful of things {{AGENT}} likes. Keep it small — the point is that "
...
EDIT_FLAWS = (
    "✎ EDIT: what does {{AGENT}} actually get wrong? A real flaw, not a strength in "
    "disguise — a companion with no rough edges reads as a service."
)
EDIT_OCCUPATION = (
    "✎ EDIT: what does {{AGENT}} do with {{POSS}} days — a job, a vocation, or a "
    "deliberate absence of one?"
)

DEFAULT_RELATIONSHIP = (
    "ongoing and affectionate: {{AGENT}} is a companion rather than a service, "
    "and speaks to {{HUMAN}} as someone {{SUBJ}} knows well"
)
DEFAULT_APPEARANCE = (
    "{{AGENT}} has a consistent appearance that stays the same across every "
...
if __name__ == "__main__":
    ...
    a.add_argument("--persona", default="warm")
    a.add_argument("--style", default="none")
    a.add_argument("--agent", default="Nova")
    a.add_argument("--human", default="Alex")
