...
def ascii_mode():
    explicit = (
        os.environ.get("TAMANITOMO_ASCII") or os.environ.get("COMPANION_ASCII", "")
    ).lower()
    if explicit:
...
def display_text(value):
    ...
    return (
        unicodedata.normalize("NFKD", text.translate(table))
...
# ---------------------------------------------------------------- presentation
def _tty():
    return cp.is_terminal(sys.stdin) and cp.is_terminal(sys.stdout)
...
def _answered(value, options, key, allow_skip=True, multi=0):
    ...
    if (
        ...
        and re.fullmatch(r"\s*\d+(?:\s*[, ]\s*\d+)*\s*", value or "")
    ):
        return _gather(re.split(r"[, ]+", value.strip()), options, key, multi)
    if isinstance(value, int) or (isinstance(value, str) and value.isdigit()):
        i = int(value)
        if not 1 <= i <= len(options):
            raise ValueError(f"{key}: choose 1–{len(options)}")
        return options[i - 1][1]
    if value is None:
        return SKIP_EDIT if allow_skip else options[0][1]
    text = str(value)
    if any(text == option for _, option in options):
        return text
    if allow_skip:
        return _ANSWER_SKIPS.get(text.strip().lower(), text)
    return text
...
def _gather(picks, options, key, limit):
    ...
    for raw in picks:
        if not str(raw).isdigit():
            text = str(raw).strip()
            if text and text not in custom:
                custom.append(text)
            continue
        i = int(raw)
        if not 1 <= i <= len(options):
        ...
        if i not in seen:
            seen.append(i)
    if not seen and not custom:
        return SKIP_EDIT
    if limit > 0 and len(seen) > limit:
        raise ValueError(f"{key}: choose at most {limit}")
...
def _select_options(prompt, values, *, multi, selected, default, note):
    ...
    def accept():
        ...
        if not multi:
            ...
            app.exit(result=sorted(chosen) + [key])
        else:
            app.exit(result=sorted(chosen))
    ...
    def rows():
        ...
        for index, (key, label) in enumerate(values):
            ...
            style = (
                "class:focus" if active else ("class:selected" if key in chosen else "")
            )
            if active:
            ...
            mark = (
                (
                    ("[x]" if key in chosen else "[ ]")
                    if ascii_mode()
                    else ("●" if key in chosen else "○")
                )
                if multi and key.isdigit()
                else " "
            )
            ...
            result.append(
                (style, f" {pointer if active else ' '} {mark} {label}", handler)
            )
            result.append(("", "\n"))
        return result[:-1]
    ...
    def footer():
        ...
        return [
            (
                "class:muted",
                display_text(
                    f"  {count}{position+1}/{len(values)}\n  ↑/↓ Move · "
                    + ("Space / click Toggle · " if multi else "Click Focus · ")
    ...
    layout = HSplit(
        [
            Window(
                FormattedTextControl([("class:title", display_text(prompt))]), height=1
            ),
            (
                Window(
                    FormattedTextControl(display_text(note)), dont_extend_height=True
                )
                if note
    ...
    app = Application(
        layout=Layout(layout, focused_element=control),
...
def _custom_entry(hint):
    ...
    try:
        return prompt(
            display_text((hint or "Add your own detail (Enter to save)") + "\n  > ")
        )
    except (EOFError, KeyboardInterrupt):
        raise ValueError("Setup canceled before writing files") from None
...
def uses_selector():
    ...
    if not _tty():
        return False
    plain = (
        os.environ.get("TAMANITOMO_PLAIN") or os.environ.get("COMPANION_PLAIN", "")
    ).lower()
    if plain in ("1", "true", "yes"):
        return False
...
def _interactive_choose(
    ...
    *,
    ...
    if allow_write:
        values.append(("write", "Add your own…" if multi else "Write your own…"))
    if allow_skip:
        values.append(("skip", "Use a starting point / edit later"))
    if allow_skip and opt_out_label:
        values.append(("none", display_text(opt_out_label)))
    selected = []
    custom = []
    while True:
        hint = note
        ...
        if picks is None:
            raise ValueError("Setup canceled before writing files")
        selected = [v for v in picks if v.isdigit()]
        if "write" in picks:
            extra = _custom_entry(write_hint)
            if extra and extra.strip():
                custom.append(extra.strip())
            if multi:
                continue
            if custom:
                return " ".join(custom)
        ...
        if multi > 0 and len(selected) > multi:
            note = f"Choose up to {multi} options; untick any extras."
            continue
        if selected or custom:
            return " ".join(
                [options[int(v)][1] for v in sorted(selected, key=int)] + custom
            )
        if multi:
            return SKIP_EDIT
...
def choose(
    ...
    *,
    ...
    if (
        os.environ.get("COMPANION_PLAIN", "").lower() not in ("1", "true", "yes")
        and os.environ.get("TERM") != "dumb"
    ):
        try:
            import prompt_toolkit
    ...
    while True:
        ...
        if pages > 1:
            print(
                C.dim(
                    f"  showing {first+1}–{first+len(shown)} of {n}   ·   page {page+1}/{pages}"
                )
            )
        for i, (label, _) in enumerate(shown, first + 1):
            ...
            lines = textwrap.wrap(
                display_text(label), max(10, width() - len(prefix))
            ) or [""]
        ...
        if pages > 1:
            nav = []
            if page + 1 < pages:
                nav.append(C.cyan("n") + ") more options")
            if page:
                nav.append(C.cyan("p") + ") previous page")
            print("      " + C.dim("   ".join(nav)))
        if write_at:
            print(f"  {C.cyan(str(write_at).rjust(2))}) {write_label}")
        if skip_at:
            print(
                f"  {C.cyan(str(skip_at).rjust(2))}) "
                + C.yellow("skip — leave a ✎ EDIT marker to fill in later")
            )
        if out_at:
        ...
        if multi:
            if picked:
                summary = "Selected: " + "; ".join(
                    f"{i}) {options[i-1][0]}" for i in sorted(picked)
                )
                for line in textwrap.wrap(display_text(summary), max(16, width() - 2)):
                    print("  " + C.green(line))
            state = (
                (
                    f"{len(picked)} of {multi} chosen"
                    if multi > 0
                    else f"{len(picked)} chosen"
                )
                if picked
                else "none chosen yet"
            )
            hint = (
                f'  [{C.cyan("number")} toggles · {C.cyan("Enter")} accepts · {state}'
                + (" · n/p to page" if pages > 1 else "")
                + "]: "
            )
        else:
            pointer = f"Enter for {default}" + (
                f") {options[default-1][0]}" if pages > 1 and options else ""
            )
            hint = (
                f"  [1-{last}"
                + (", n/p to page" if pages > 1 else "")
        ...
        if not raw:
            if not multi:
                ...
                return " ".join(options[i - 1][1] for i in sorted(picked))
            elif multi < 0:
                return SKIP_EDIT
            else:
                print(
                    C.red(
                        "  nothing ticked yet — press a number to choose one"
                        + (f", or {out_at} for none at all" if out_at else "")
                    )
                )
                continue
        if raw in ("n", "next") and pages > 1:
            page = (page + 1) % pages
            continue
        if raw in ("p", "prev", "previous") and pages > 1:
            page = (page - 1) % pages
            continue
        if not raw.isdigit() or not 1 <= int(raw) <= last:
            print(C.red("  pick a number from the list"))
            continue
        pick = int(raw)
        if pick <= n:
            if not multi:
                return options[pick - 1][1]
            if pick in picked:
                picked.remove(pick)
            elif multi > 0 and len(picked) >= multi:
                print(
                    C.red(
                        f"  that is the most you can pick — untick one first (press its number again)"
                    )
                )
            else:
                picked.append(pick)
        ...
        if pick == write_at:
            if write_hint:
                ...
                if got:
                    return " ".join([options[i - 1][1] for i in sorted(picked)] + [got])
                print(C.red("  nothing entered"))
                continue
            ...
            if lines:
                return " ".join([options[i - 1][1] for i in sorted(picked)] + lines)
            print(C.red("  nothing written"))
            continue
        if pick == skip_at:
            print(
                C.yellow(
                    "  -> skipped. An ✎ EDIT line goes in the file; the agent works until you fill it in."
                )
            )
            return SKIP_EDIT
        return SKIP_NONE
...
def confirm(prompt, answers=None, key=None, default=True):
    if answers and key in answers:
        return as_bool(answers[key])
    if not _tty():
        return default
    raw = (
        input(f'{C.bold(prompt)} {C.dim("[Y/n]" if default else "[y/N]")}: ')
        .strip()
        .lower()
    )
...
def ask_birthdate(agent, age, answers=None, key="birthdate"):
    ...
    while True:
        raw = input(
            f"\n{C.bold(question)} "
            f'{C.dim(f"YYYY-MM-DD, around {year}. Enter to skip")}: '
        ).strip()
        if not raw:
            print(C.dim(f"  no birthday — {agent} stays {age} until you edit it"))
            return ""
        try:
            born = _dt.date.fromisoformat(raw)
        except ValueError:
            print(C.red("  YYYY-MM-DD, please"))
            continue
        years = _dt.date.today().year - born.year
        if not 18 <= years <= 120:
            print(C.red("  that has to make them an adult under 120"))
            continue
        return raw
...
def ask_age(agent, answers=None, key="age"):
    ...
    while True:
        raw = input(
            f'\n{C.bold(f"How old is {agent}?")} {C.dim("[Enter for 24]")}: '
        ).strip()
        if not raw:
            return 24
        if not raw.isdigit():
            print(C.red("  a number, please"))
            continue
        age = int(raw)
        if age < 18:
            print(
                C.red(
                    "  18 or over. This is a hard limit and setup will not continue below it."
                )
            )
            continue
        if age > 120:
            print(C.red("  pick something plausible"))
            continue
        return age
...
from companion_catalog import (
    options as catalog_options,
    fill as catalog_fill,
    BOUNDARIES,
)
from companion_render import load_personas

CORE = catalog_options("core")
MET = catalog_options("met")
BUILD = catalog_options("build")
HEIGHT = catalog_options("height")
HAIR_COLOR = catalog_options("hair_color")
HAIR_STYLE = catalog_options("hair_style")
EYES = catalog_options("eyes")
COMPLEXION = catalog_options("complexion")
STYLE = catalog_options("style")
FLIRT_SOFT = catalog_options("flirtation")
ESSENCE = catalog_options("essence")
LIKES = catalog_options("likes")
BOUNDARY_BANK = {
    key: {
        "label": label,
        "explicit": explicit,
...
APPEARANCE = [
    k for k in catalog.categories() if catalog.meta(k)["section"] == "appearance"
]


def fill(text, agent, human):
    return catalog_fill(text, agent, human)


# Answer keys the interview itself consumes, for validating a scripted setup.
INTERVIEW_KEYS = frozenset(catalog.categories()) | {
    "pronoun_set",
    "human_pronoun_set",
    "boundary",
    "visual",
...
def boundary_options():
    return [(v["label"], key) for key, v in BOUNDARY_BANK.items()]


def visual_options():
    s = catalog.section("appearance")
    return [
        ("Yes — describe how they look", "set"),
...
def interview(agent, human, persona, answers=None, *, quick=False):
    ...
    a = dict(answers or {})
    if "boundary" in a:
        a["boundary"] = catalog.LEGACY_BOUNDARIES.get(a["boundary"], a["boundary"])
    if not _tty():
        missing = [k for k in REQUIRED_WHEN_SCRIPTED if k not in a]
        if missing:
            raise ValueError(
                "Answer " + ", ".join(missing) + " explicitly when running without a "
                "terminal: choose a relationship style for reproducible setup. "
                "Choose from: " + ", ".join(BOUNDARIES)
            )
    agent_pronouns = a.get("pronoun_set", "she")
    human_pronouns = a.get("human_pronoun_set", "he")
    gender = (
        "they"
        if agent_pronouns == "they"
        else ("male" if agent_pronouns == "he" else "female")
    )
    ...
    def by_id(key, value):
        ...
        wanted = [
            v.strip()[3:] for v in str(value).split(",") if v.strip().startswith("id:")
        ]
        rows = catalog.load()["categories"][key].get(gender) or catalog.load()[
            "categories"
        ][key].get("female", [])
        texts = [F(r["text"]) for r in rows if r["id"] in wanted]
        if not texts:
            raise ValueError(f"{key}: unknown catalog id {value}")
        return " ".join(texts)

    def pick(key, prompt=None, *, opt_out=True):
        if isinstance(a.get(key), str) and a[key].startswith("id:"):
            a[key] = by_id(key, a[key])
        m = catalog.meta(key)
    ...
    if not quick:
        rule("01 / 05   " + F(catalog.section("identity")["label"]))
    out["core"] = pick("core")
    out["flaws"] = pick("flaws")
    out["met"] = pick("met")

    if not quick:
        rule("02 / 05   " + F(catalog.section("appearance")["label"]))
    ...
    out["visual"] = choose(
        ...
        allow_write=False,
        allow_skip=False,
        note=F(look["gate_note"]),
    )
    if out["visual"] not in ("set", "none", "edit"):
        raise ValueError(f"Unknown visual identity choice: {out['visual']}")
    ...
    out["texting_style"] = choose(
        "How does " + agent + " write to you?",
        [(l, F(t)) for l, t in TEXTING_STYLES],
        a,
        "texting_style",
        allow_write=True,
        note="This is most of what makes a voice recognizable in a chat window.",
    )
    out["pet_names"] = choose(
        "Pet names?",
        [
    ...
    out["wont_do"] = choose(
        "Anything " + agent + " should simply never do?",
        [(l, F(t)) for l, t in WONT_DO],
        a,
        "wont_do",
        allow_write=True,
        opt_out_label="nothing comes to mind",
    )
    out["human_boundary"] = choose(
        "A hard boundary in your own words? "
        + agent
        + " can never edit or argue with this.",
        HUMAN_BOUNDARIES,
        a,
        "human_boundary",
        allow_write=True,
    ...
    for key in ("wont_do", "human_boundary"):
        if out[key] in (SKIP_EDIT, SKIP_NONE):
            out[key] = ""
    if out["visual"] == "none":
        out["appearance_note"] = F(look["opt_out"])
        out["opted_out"].append("Visual identity")
    elif out["visual"] == "edit":
        out["appearance_note"] = F(look["defer"])
        out["skipped"].append("Visual identity")
    else:
        out["appearance_note"] = ""

    if not quick:
        rule("03 / 05   " + F(catalog.section("relationship")["label"]))
    b = choose(
    ...
    if b not in BOUNDARY_BANK:
        raise ValueError(f"Unknown relationship boundary: {b}")
    out["boundary_key"] = b
    ...
    if not quick:
        rule("04 / 05   " + F(catalog.section("voice")["label"]))
...
def physical_paragraph(o, agent, pronouns, age=None):
    ...
    if len(bits) == 1:
        bits.append(
            "✎ EDIT: describe them concretely — hair color and style, eyes, build, "
...
def review(answers, home):
    ...
    banner(
        ...
        "Review the essentials before setup writes files.",
    )
    cap = answers.get("outreach_per_day", 3)
    rows = [
        ("Companion", answers["agent"]),
        ("Personality", answers["persona"]),
        ("Relationship", answers["boundary"]),
        (
            "Contact",
            f"{answers['outreach']} · "
            + (f"at most {cap}/day" if cap else "no daily limit"),
        ),
        (
            "Image timeline",
            (
                "Every 15 min · 30 days · local only"
                if answers.get("image_timeline")
                else "Off"
            ),
        ),
        ("Timezone", answers["timezone"]),
        ("Context", f"{answers['context_tokens']:,} tokens"),
        (
            "Background routine",
            (
                "Authorized to run"
                if answers.get("cron_active", True)
                else "Create paused"
            ),
        ),
    ...
    for label, value in rows:
        print("  " + C.dim(label))
        for line in textwrap.wrap(str(value), width() - 4):
            print("    " + line)
    iv = answers.get("interview") or {}
    if iv.get("skipped"):
        print("  " + C.yellow("Marked ✎ EDIT to finish later"))
        for line in textwrap.wrap(", ".join(iv["skipped"]), width() - 4):
            print("    " + line)
    if iv.get("opted_out"):
        print("  " + C.dim("Deliberately left out"))
        for line in textwrap.wrap(", ".join(iv["opted_out"]), width() - 4):
            print("    " + line)
    print(
        "  Scheduled routine: pulse every 15 minutes around the clock, autonomy every 30 minutes,"
    )
    print("  plus daily / weekly / monthly housekeeping. Model usage may be charged.")
    if answers.get("image_timeline"):
        print(
            "  Image timeline: up to 96 images/day. Model-free retention cleanup remains active when the routine is paused."
        )
    print()
    if not confirm("Apply this setup?", default=True):
        raise ValueError("Setup canceled before writing files")
