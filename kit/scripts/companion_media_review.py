...
def preferences(c):
    return {**DEFAULTS, **read_json(c.home / "companion-media-preferences.json")}


def save_preferences(c, data):
    if (c.home / "companion-media-preferences.json").is_symlink():
        raise ValueError("Linked media preferences are not writable")
    if set(data) - set(DEFAULTS):
        raise ValueError("Unknown media preference")
    for key, value in data.items():
        if key in (
            "blur_nsfw_initially",
            "blur_unknown_initially",
            "review_before_delivery",
        ):
            if not isinstance(value, bool):
                raise ValueError("Use a boolean media preference")
        elif (
            not isinstance(value, str)
            or len(value) > 150
            or any(ord(x) < 32 for x in value)
        ):
            raise ValueError("Invalid reviewer provider/model")
    value = {**preferences(c), **data}
    atomic_write(
        c.home / "companion-media-preferences.json", json.dumps(value, indent=2)
    )
    return value


def sidecar(path):
    return path.with_name("." + path.name + ".companion.json")


def metadata(path):
    old = read_json(path.with_suffix(".json"))
    label = old.get("generation") or (
        "Workflow: " + str(old["preset"])
        if old.get("preset")
        else "Generation source not recorded"
    )
    title = {}
...
def write_metadata(path, data):
    ...
    if target.is_symlink():
        raise ValueError("Linked media metadata is not writable")
    atomic_write(target, json.dumps({**metadata(path), **data}, indent=2))


def should_blur(prefs, meta):
    rating = meta.get("rating", "unknown")
    return (rating == "nsfw" and prefs["blur_nsfw_initially"]) or (
        rating not in ("safe", "nsfw") and prefs.get("blur_unknown_initially", True)
    )
...
def inspect(c, path, prompt, allow_nsfw=False):
    ...
    if prefs["review_provider"] == "local-nsfw":
        ...
        try:
            ...
            raise ValueError(
                "Local scan unavailable; image stays blurred. No remote reviewer was contacted."
            )
        ...
        result.update(
            ...
            allow_nsfw=explicit_mode,
            sha256=digest,
        )
        write_metadata(path, {"rating": rating, "review": result})
        return result
    result = hermes_bridge(
        c,
        "review",
        {
            "path": str(path),
            "prompt": prompt,
            "provider": prefs["review_provider"],
            "model": prefs["review_model"],
        },
    )
    if not isinstance(result, dict) or not all(
        isinstance(result.get(k), bool) for k in ("matches_request", "nsfw")
    ):
        raise ValueError("Image reviewer returned no valid decision")
    if hashlib.sha256(path.read_bytes()).hexdigest() != digest:
        raise ValueError("Image changed during review")
    explicit_mode = _adult_ready(c)
    ...
    result = {
        ...
        if k in ("matches_request", "nsfw", "reason", "provider", "model")
    }
    result.update(
        status="passed" if passed else "held", allow_nsfw=explicit_mode, sha256=digest
    )
    write_metadata(
        path, {"rating": "nsfw" if result["nsfw"] else "safe", "review": result}
    )
    return result
...
def ensure_delivery(c, path, prompt):
    ...
    if (
        rated in ("nsfw", "unknown")
        and decision.get("status") != "unavailable"
        and not _adult_ready(c)
    ):
        raise ValueError(
            "Image held: it is rated adult and adult images are not available for this companion right now"
        )
    if not preferences(c)["review_before_delivery"]:
        return
    local = preferences(c)["review_provider"] == "local-nsfw"
    if (
        decision.get("status") == "passed"
        and (not local or decision.get("provider") == "local-nsfw")
        and decision.get("sha256") == hashlib.sha256(path.read_bytes()).hexdigest()
    ):
        return
    # A previously held image never becomes approved merely by entering the queue.
    if decision.get("status") == "held":
        raise ValueError(
            "Image review held this image; regenerate or inspect it before sending"
        )
    decision = inspect(c, path, prompt)
    if decision["status"] != "passed":
        raise ValueError(
            "Image review held this image: "
            + str(decision.get("reason", "request mismatch"))[:300]
        )
