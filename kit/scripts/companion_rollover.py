...
import argparse, datetime as dt, json, pathlib, sqlite3, subprocess, sys
sys.path.insert(0,str(pathlib.Path(__file__).resolve().parent))
import companion_config as cc

QUIET_MINUTES=120
CHAT_PLATFORMS=('telegram','discord','slack','signal','whatsapp','matrix','weixin','feishu','dingtalk','sms','email')
...
def routes(c):
    ...
    try:data=json.loads((c.home/'sessions/sessions.json').read_text(encoding='utf-8'))
    ...
    return {k:v['session_id'] for k,v in entries.items()
            if isinstance(v,dict) and v.get('session_id') and ':dm:' in k
            and str(v.get('platform','')).lower() in CHAT_PLATFORMS}

def _db(c):
    return sqlite3.connect(pathlib.Path(c.home/'state.db').resolve().as_uri()+'?mode=ro',uri=True,timeout=2)

def chain_tip(con,session_id):
    """The newest session a compression chain has moved to, and every open link."""
    open_links=[];current=session_id;seen=set()
    while current and current not in seen:
        seen.add(current)
        row=con.execute('select end_reason from sessions where id=?',(current,)).fetchone()
        if row and row[0] is None:open_links.append(current)
        child=con.execute('select id from sessions where parent_session_id=? order by started_at desc limit 1',(current,)).fetchone()
        current=child[0] if child else None
    return open_links

def last_human(con,platforms=CHAT_PLATFORMS):
    marks=','.join('?'*len(platforms))
    row=con.execute(f"""select max(m.timestamp) from messages m join sessions s on s.id=m.session_id
        where m.role='user' and lower(coalesce(s.source,'')) in ({marks})""",platforms).fetchone()
...
def hermes_python():
    ...
    try:
        text=exe.read_text(encoding='utf-8',errors='replace') if exe.stat().st_size<20000 else ''
    except OSError:text=''
    import re
    m=re.search(r'exec\s+"?([^"\s]+/bin/)hermes"?',text)
    for candidate in ([pathlib.Path(m.group(1))/'python'] if m else [])+[exe.resolve().parent/'python']:
        if candidate.is_file():return str(candidate)
    return None

def end_sessions(c,ids,python=None):
    python=python or hermes_python()
    if not python:raise ValueError('cannot find the Python that Hermes runs')
    script=('import sys,pathlib\nfrom hermes_state import SessionDB\n'
            'db=SessionDB(pathlib.Path(sys.argv[1]))\n'
            'for sid in sys.argv[3:]:db.end_session(sid,sys.argv[2])\n')
    r=subprocess.run([python,'-c',script,str(pathlib.Path(c.home/'state.db').resolve()),END_REASON,*ids],
                     capture_output=True,text=True,timeout=60,env={'HERMES_HOME':str(c.home),'PATH':'/usr/bin:/bin'})
    if r.returncode:raise ValueError('Hermes could not end the session: '+(r.stderr or '').strip()[-200:])

def run(c,now=None,apply=True,ender=None):
    now=(now or dt.datetime.now(_tz(c))).astimezone(_tz(c))
    if c.agent_type=='worker':return {'rolled':False,'reason':'workers keep their sessions'}
    mapped=routes(c)
    if not mapped:return {'rolled':False,'reason':'no chat session to roll over'}
    try:
        con=_db(c)
        try:
            last=last_human(con)
    ...
    except sqlite3.Error as exc:return {'rolled':False,'reason':f'state.db unreadable: {exc}'}
    if last and now.timestamp()-last<QUIET_MINUTES*60:
        return {'rolled':False,'reason':f'{c.human} wrote {int((now.timestamp()-last)//60)} minutes ago'}
    ids=[sid for links in targets.values() for sid in links]
    if not ids:return {'rolled':False,'reason':'the chat is already fresh'}
    ...
    log=c.life/'rollover.jsonl';log.parent.mkdir(parents=True,exist_ok=True)
...
def main():
    ...
    if a.json or a.dry_run:print(json.dumps(out,indent=2))
    return 0

if __name__=='__main__':
    try:sys.exit(main() or 0)
    except (ValueError,OSError) as e:
        print(f'rollover: {e}',file=sys.stderr);sys.exit(1)
