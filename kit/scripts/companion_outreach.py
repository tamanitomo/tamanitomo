#!/usr/bin/env python3
"""Deterministic quiet hours and daily slots for unprompted messages.

Prompts express preferences; the clock, configured policy and durable counter
enforce them. Otherwise a changed model, competing workers or an enthusiastic
retry could turn a quiet night into repeated outreach. ``claim`` checks and
reserves a daily slot under one file lock before transport runs. Failed or
unconfirmed sends keep their slot, and an unreadable counter refuses delivery.
An explicit urgent decision may bypass quiet hours, never the configured daily
cap. A configured cap of zero means unlimited; it is not a zero-message cap.

These checks govern callers of this helper, not arbitrary external tools.
The CLI ``send`` command queues a message for the outbox dispatcher; it does
not deliver immediately. The Python ``send`` helper also exists for callers
that reserve through this same gate. Delivery decisions must remain in code
so provider fallback and prompt changes cannot weaken the human's boundaries.
"""
...
def in_quiet_hours(c, now):
    ...
    start = _minutes(c.quiet_start, 23 * 60)
    end = _minutes(c.quiet_end, 8 * 60)
...
def path(c):
    return pathlib.Path(c.data) / LEDGER
        ...
        with path(c).open(encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    raise ValueError(
                        "Corrupt outreach ledger; reconcile before sending"
                    ) from exc
                if not isinstance(row, dict) or row.get("kind") != "outreach":
                    raise ValueError("Invalid outreach ledger entry")
...
def decide(c, now=None, urgent=False):
    ...
    try:
        used = sent_today(c, now)
    except (OSError, ValueError) as exc:
        return {
            "allowed": False,
            "reason": "Outreach ledger is unreadable or corrupt; repair it before sending",
            "storage_error": True,
        }
    out = {
    ...
    if c.outreach == "never":
        return {
            **out,
            "allowed": False,
            "reason": "This companion does not message first. Reply only.",
        }
    ...
    return {
        **out,
        "allowed": True,
        "reason": (
            "Outside quiet hours and within the daily limit."
            if limit
...
def _append(c, reason, now, attempt=None):
    ...
    # The dispatcher's attempt identity (recorded before delivery): a crashed run's
    # charge is then found by lookup, never inferred from the outbox marker alone.
    if attempt:
        row["attempt"] = str(attempt)[:80]
...
def charges_for(c, attempt):
    ...
    an unreadable ledger is never evidence that no charge happened (delivery recovery).
    ...
    with file_lock(path(c).with_suffix(".jsonl.lock")):
        try:
            with path(c).open(encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Corrupt outreach ledger; the charge cannot be established"
                        ) from exc
                    if not isinstance(row, dict) or row.get("kind") != "outreach":
                        raise ValueError(
                            "Invalid outreach ledger entry; the charge cannot be established"
                        )
            ...
            raise ValueError("Unreadable outreach ledger") from exc
    return found
...
def send(c, message, reason="", target="telegram", now=None):
    ...
    try:
        ...
        return {
            **result,
            ...
            "delivery_status": (
                "sent"
                if delivered
                else "failed_or_unconfirmed; slot retained; do not retry"
            ),
        }
        ...
        return {
            **result,
            "delivered": False,
            "delivery_status": "failed_or_unconfirmed; slot retained; do not retry",
        }
...
def main():
    ...
    message_input.add_argument(
        "--message", help="Message text for send; otherwise read stdin"
    )
    message_input.add_argument(
        "--message-file",
        type=pathlib.Path,
        help="Read literal UTF-8 message text from a file",
    )
    p.add_argument(
        "--to",
        default="telegram",
        help="Hermes delivery target (default: telegram home channel)",
    )
    p.add_argument(
        "--ttl-hours",
        type=float,
        default=4,
        help="how long the message is still worth sending",
    )
    p.add_argument(
        "--urgent",
        action="store_true",
        ...
        try:
            message = (
                ...
                if a.message is not None
                    ...
                    a.message_file.read_text(encoding="utf-8")
                    if a.message_file
                    else (sys.stdin.read() if not is_terminal(sys.stdin) else "")
                )
            )
                ...
                json.dumps(
                    {
                        "allowed": False,
                        "delivered": False,
                        "reason": "Message input file could not be read",
                    }
                )
            )
            return 1
        ...
        try:
            result = outbox.queue(
                c,
                {
                    "kind": "text",
                    "body": message,
                    "reason": a.reason or "outreach",
                    "priority": "high" if a.urgent else "normal",
                    "ttl_hours": a.ttl_hours,
                    "target": a.to,
                },
            )
        except ValueError as exc:
            print(json.dumps({"queued": False, "reason": str(exc)}, ensure_ascii=False))
            return 1
        print(
            json.dumps(
                {
                    "queued": True,
                    "id": result["entry"]["id"],
                    "already_waiting": not result.get("written", True),
                    "note": "Queued, not sent. The dispatcher decides when it goes, within quiet hours "
                    "and the daily limit. Do not queue it again.",
                },
                ensure_ascii=False,
            )
        )
    ...
    if a.action == "record":
        print(
            json.dumps(
                {"recorded": True, "sent_today": record(c, a.reason)},
                ensure_ascii=False,
            )
        )
        return 0
