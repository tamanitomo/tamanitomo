...
def is_secret(path: pathlib.Path) -> bool:
    ...
    for part in parts:
        if part in SECRET_NAMES:
            return True
        if part.startswith(".env") or any(part.endswith(s) for s in SECRET_SUFFIXES):
            return True
        ...
        if any(a + b in SECRET_WORDS for a, b in zip(words, words[1:])):
            return True
        if "id" in words and "rsa" in words:
            return True
    return False
...
def find_peer(c, name):
    if not c.peer_interaction:
        raise ValueError("Peer interaction is switched off for this companion")
    name = (name or "").strip().lower()
    for p in peers(c):
        if (p.profile or "root").lower() == name or p.agent.lower() == name:
            if not p.peer_interaction:
                raise ValueError("That companion has peer interaction switched off")
            return p
...
def resolve_in(peer, rel):
    ...
    if (
        ...
        raise ValueError(
            "Named agents must be accessed as their own peer, not through the root vault"
        )
...
def cmd_list(c, a):
    return {
        "peers": [
            {
                "name": p.profile or "root",
                "agent": p.agent,
                "vault": str(p.data),
                "skills": str(p.home / "skills"),
                "readable": p.data.exists(),
            }
            for p in peers(c)
        ]
    }
...
def cmd_ls(c, a):
    p = find_peer(c, a.agent)
    target = resolve_in(p, a.path or "vault")
    ...
    if target.is_file():
        ...
        entries = [
            {
                "name": x.name + ("/" if x.is_dir() else ""),
                "size": (x.stat().st_size if x.is_file() else None),
                "kind": "dir" if x.is_dir() else "file",
            }
            for x in sorted(target.iterdir())
            if not x.is_symlink()
            and not is_secret(x.name)
            and not (p.is_root and target == p.data and x.name == "agents")
        ][:400]
    log(c, "ls", str(target))
...
def cmd_read(c, a):
    p = find_peer(c, a.agent)
    target = resolve_in(p, a.path)
    if not target.is_file():
        raise ValueError("not a file")
    if target.stat().st_size > MAX_READ:
        raise ValueError(
            f"file is {target.stat().st_size} bytes; over the {MAX_READ} read cap"
        )
    log(c, "read", str(target))
    return {
        "peer": p.profile or "root",
        "path": str(target),
...
def cmd_copy(c, a):
    p = find_peer(c, a.agent)
    src = resolve_in(p, a.path)
    if not src.is_file():
        raise ValueError("only files can be copied")
    if src.stat().st_size > MAX_READ:
        raise ValueError("File exceeds copy size cap")
    area, _, rest = (a.dest or "").strip().lstrip("/").partition("/")
    mine = {"vault": c.data, "skills": c.home / "skills"}
    if area not in mine:
        raise ValueError(f"--as must start with one of {sorted(mine)}")
    dest = ((mine[area] / rest) if rest else (mine[area] / src.name)).resolve()
    base = mine[area].resolve()
    if dest != base and base not in dest.parents:
        raise ValueError("destination escapes your own tree")
    relative = dest.relative_to(base)
    if is_secret(relative):
        raise ValueError("Refusing credential destination")
    if (
        area == "vault"
        and c.is_root
        and relative.parts
    ...
    if dest == c.soul.resolve() or dest == c.canonical_soul.resolve():
        raise ValueError("Cannot overwrite identity through peer copy")
...
def main():
    ...
    cp.add_argument(
        "--as", dest="dest", required=True, help="vault/<path> or skills/<path>"
    )
