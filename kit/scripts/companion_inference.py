...
def redact(message):
    ...
    text = re.sub(r"(?i)((?:api[_ -]?key|token|secret|password|authorization)[\"']?\s*[=:]\s*[\"']?)[^\s,;\"'}]+",
                  r'\1[redacted]', text)
...
class ProviderFailure(ValueError):
    ...
    def __init__(self, message, status_code=None, transient=None):
        ...
        self.transient = self.status_code in TRANSIENT_STATUSES if transient is None else bool(transient)
...
def is_transient(error):
    ...
    if isinstance(error, (TimeoutError, ConnectionError, subprocess.TimeoutExpired, URLError)):
        return True
    # Hermes may use an optional provider SDK that is absent in the app's Python.
    return type(error).__name__ in ('APITimeoutError', 'APIConnectionError', 'RateLimitError',
...
def is_local(route):
    return urlsplit(route.get('base_url') or '').hostname in ('127.0.0.1', 'localhost', '::1')
...
def configured_routes(c, primary=None, tier='chat'):
    ...
    if primary is None:
        ...
        same_endpoint = not chosen.get('base_url') or chosen['base_url'].rstrip('/') == str(default.get('base_url') or '').rstrip('/')
        native = {key: default[key] for key in ('api_key', 'api_mode', 'api_key_env', 'key_env')
                  if key in default} if same_provider and same_endpoint else {}
        primary = {**native, **chosen, 'provider': chosen.get('provider') or provider,
                   'model': chosen.get('model') or default.get('default') or default.get('model') or '',
                   'base_url': chosen.get('base_url') or (default.get('base_url') if same_provider else '') or ''}
    first = normalize(primary, provider if not primary.get('base_url') else '')
    chain = (config.get('fallback_providers') if 'fallback_providers' in config
             else (getattr(c, 'models', None) or {}).get('fallbacks')) or []
    ...
    for row in chain[:8]:
        ...
        candidates.append(normalize(row, first['provider']))
...
def credential(c, route, primary_env=''):
    """Resolve only this route's credential, from this profile or its root."""
    if route.get('api_key'):
        return str(route['api_key']).strip()
    name = route.get('api_key_env') or primary_env or PROVIDER_KEYS.get(route.get('provider', '').lower(), '')
    if not name:
        return ''
    if os.environ.get(name):
        return os.environ[name].strip()
    from companion_gateway import _env_values
    return str(_env_values(c.home).get(name) or _env_values(c.hermes_root).get(name) or '').strip()
