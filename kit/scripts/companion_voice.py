...
def read_config(c):
    import yaml
    try:return yaml.safe_load((c.home/'config.yaml').read_text(encoding='utf-8')) or {}
...
def settings(c):
    ...
    return {'configured':bool(provider),'provider':provider,
            ...
            'note':('' if provider else
                    'No TTS provider is configured in Hermes, so this companion has no voice yet. '
...
def queue_note(c,text,audio_path,reason='',ttl_hours=4):
    ...
    text=(text or '').strip()
    if not text:raise ValueError('nothing to say')
    if len(text)>1200:raise ValueError('a voice note over 1200 characters is a monologue; shorten it')
    state=settings(c)
    if not state['configured']:return {'ok':False,'reason':state['note']}
    audio=pathlib.Path(audio_path)
    if not audio.is_absolute():raise ValueError('the audio path must be absolute')
    if not audio.is_file():
        return {'ok':False,'reason':f'no audio at {audio}. Synthesize it with the text_to_speech '
                                    f'tool first and pass the file path it returns.'}
    queued=outbox.queue(c,{'kind':'voice','body':text,'media_path':str(audio),
                           'reason':reason,'ttl_hours':ttl_hours})
    return {'ok':True,'queued':queued['entry']['id'],'file':str(audio),
...
def synthesize(c,text,output_path=None):
    ...
    import companion_tts_adapter as adapter, hashlib
    text=(text or '').strip()
    if not text:raise ValueError('nothing to say')
    cfg=read_config(c)
    provider=(cfg.get('tts') or {}).get('provider')
    if not provider:raise ValueError('No TTS provider configured in Hermes (config.yaml tts.provider).')
    if not output_path:
...
def set_clone(c,ref_audio,ref_text,confirmed=False):
    ...
    """
    if not confirmed:
        raise ValueError('Refused: confirm that this recording is your own voice, or one you have '
                         'permission to use, before it becomes your companion\'s voice.')
    import yaml
    source=pathlib.Path(ref_audio)
    if not source.is_file():raise ValueError('that audio file does not exist')
    if not (ref_text or '').strip():
        raise ValueError('ref_text is required: it must be a transcript of exactly what is said in '
    ...
    block['ref_audio']=str(stored);block['ref_text']=ref_text.strip()
    tts[CLONE_PROVIDER]=block;tts['provider']=CLONE_PROVIDER
    config['tts']=tts
    atomic_write(c.home/'config.yaml',yaml.safe_dump(config,sort_keys=False,allow_unicode=True))
    return {'provider':CLONE_PROVIDER,'ref_audio':str(stored),
...
def main():
    ...
    sp.add_argument('--reason',default='');sp.add_argument('--ttl-hours',type=float,default=4)
    sn=s.add_parser('send',help='synthesize and queue a voice note')
    sn.add_argument('--text')
    sn.add_argument('--file',type=pathlib.Path,help='path to UTF-8 file containing text')
    sn.add_argument('--reason',default='');sn.add_argument('--ttl-hours',type=float,default=4)
    cl=s.add_parser('clone',help="write Hermes's voice-cloning keys")
    cl.add_argument('--audio',required=True);cl.add_argument('--transcript',required=True)
    cl.add_argument('--i-have-permission',action='store_true',dest='confirmed')
    a=p.parse_args();c=cc.load(a.home)
    if a.cmd=='status':out=settings(c)
    elif a.cmd=='say':
        out=queue_note(c,a.text_file.read_text(encoding='utf-8'),a.audio,a.reason,a.ttl_hours)
    elif a.cmd=='send':
        text=(a.file.read_text(encoding='utf-8') if a.file else a.text or '').strip()
        if not text:raise ValueError('either --file or --text with non-empty content required')
        out=send_note(c,text,a.reason,ttl_hours=a.ttl_hours)
    else:out=set_clone(c,a.audio,a.transcript,a.confirmed)
