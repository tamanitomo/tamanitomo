...
RECALL_TRIGGER = re.compile(
    r"\b(remember|recall|last time|ever|when did|when was|have i|did i|where did|what did i)\b",
    re.I,
)
...
def written_at(path, tz):
    ...
    try:
        return dt.datetime.fromtimestamp(pathlib.Path(path).stat().st_mtime, tz)
    except (OSError, ValueError, OverflowError):
        return None


def elapsed_seconds(then, now):
    if then is None or then.tzinfo is None or now.tzinfo is None:
        return None
    # Same-ZoneInfo subtraction counts wall-clock hours across DST, not elapsed time.
    return (
        now.astimezone(dt.timezone.utc) - then.astimezone(dt.timezone.utc)
    ).total_seconds()
...
def age_phrase(then, now):
    ...
    if seconds is None:
        return "age unknown"
    if seconds < -60:
        return "future timestamp (check the clock)"
    if seconds < 60:
        return "just now"
    if seconds < 90 * 60:
        n = int(seconds // 60)
...
def _latest(events, tz):
    ...
    for e in events:
        try:
            ...
            stamps.append(
                (when if when.tzinfo else when.replace(tzinfo=tz)).astimezone(
                    dt.timezone.utc
                )
            )
        except ValueError:
...
def cross_channel_handoff(c, payload, now, cap):
    ...
    unknown = (
        f"[Recent messages from {who} could not be checked this turn ({{}}). Do not assume nothing "
        "happened elsewhere; ask if it matters.]"
    )
    session = payload.get("session_id") if isinstance(payload, dict) else None
    if not isinstance(session, str) or not session:
        return unknown.format("this session is not identified")
    try:
        ...
        def read(view):
            ...
            if view.session(session) is None:
                return "unknown-session", None
            kind = view.owner_session(session)
            if kind is None:
                return "not-owner", None
            since = (now - HANDOFF_WINDOW).timestamp()
            records, complete = view.recent(
                since, view.lineage(session), HANDOFF_ITEMS + 1
            )
        ...
        return unknown.format(
            "the conversation store could not be read: " + type(exc).__name__
        )
    if status == "unknown-session":
        return unknown.format("this session is not in the conversation store yet")
    if status != "ok":
        return None
    ...
    if not lines:
        return None
    head = (
        f"[Recent messages from {who}, on other channels than this one ({CHANNEL_LABELS.get(here,here)}) — "
    ...
    if omitted or not complete:
        tail.append(
            f"[Older messages from other channels in the last {int(HANDOFF_WINDOW.total_seconds()//3600)} "
...
def build(c, payload=None, now=None, maintenance_notice=""):
    ...
    parts = [
        (0, f'[Current time: {now.astimezone(tz).isoformat(timespec="minutes")}]'),
        (0, rules),
    ]
    try:
        ...
        if recipes.get("presets"):
            ...
            command = terminal_python_command(
                pathlib.Path(__file__).with_name("companion_portrait.py"),
                "--home",
                c.home,
                "generate",
            )
            available = ", ".join(
                p["id"] + " (" + p["category"] + ")" for p in recipes["presets"]
            )
            parts.append(
                (
                    3,
                    "[Image Studio: user-selected image providers]\n"
                    + available
                    + "\nGenerate using "
                    + command
        ...
        pass
    if c.agent_type == "companion" and (
        c.relationship_pace != "natural" or not c.peer_interaction or c.explicit
    ):
        pace = {
            "slow": "Let familiarity build slowly; do not rush intimacy.",
            "natural": "Let familiarity follow actual shared experiences.",
            "quick": "Be open to growing familiar sooner when reciprocated; never presume intimacy.",
        }[c.relationship_pace]
        parts.append(
            (
                0,
                ...
                + pace
                + " Respect the chosen relationship frame and boundaries. "
                "Never make time away a debt or treat milestones as proof of consent. "
                + (
                    "Do not use peer interaction tools. "
                    if not c.peer_interaction
                    else ""
                )
                + (
                    "The adult user opted into adult themes within the agreed frame; consent and their boundaries still apply."
                    if c.explicit
                    else ""
                ),
            )
    ...
    if maintenance_notice:
        parts.append((0, maintenance_notice))
    ...
    if born:
        ...
        if days == 0:
            line = (
                f"[It is your birthday. You are {c.current_age(today)} today. "
    ...
    if platform or first_turn:
        ...
        if first_turn:
            note.append(
                "This is the first turn of a new session. Pick the thread back up from the "
                "state and loops above rather than greeting as if you had just met; if real "
                "time has passed, notice it once and move on."
            )
        else:
            note.append(
                "This session is already under way. Do not re-greet or restate context."
            )
        parts.append((0, "[This conversation]\n" + " ".join(note)))

    if handoff_enabled():
        cap = min(HANDOFF_MAX_CHARS, b["total"] // 6)
        if cap < HANDOFF_MIN_CHARS:
            suppressed.add("recent messages from other channels")
        else:
            handed = cross_channel_handoff(c, payload or {}, now, cap)
            if handed:
                parts.append((6, handed))
    ...
    try:
        ...
        if scene:
            ...
            label = "[Current lived state — " + age_phrase(
                dt.datetime.fromisoformat(scene["recorded_at"]), now
            )
            if not state.get("confirmed", True):
                ...
                label += (
                    ". UNCONFIRMED"
                    + since
                    + ": carried forward by a script, not looked at. "
            ...
            if companion_lifestyle.enabled(c):
                parts.append(
                    (
                        10,
                        "[Clothing care rules: brush teeth before fresh clothes; shower before pajamas; removed clothes need laundry. "
            ...
            if state.get("wants"):
                body += "; wants: " + "; ".join(state["wants"])
            if state.get("private_stance"):
                body += (
                    "\nWhere you actually stand (yours, not a line to say): "
                    + state["private_stance"]
                )
            parts.append((10, label + "]\n" + clip(body, max(350, b["handoff"]))))
            parts.append(
                (
                    10,
                    "[Day continuity — plans are not completed events; full state: companion_presence.py show]\n"
                    + clip(companion_day.render(state, now), max(700, b["handoff"]))
                    + "\nPersist agreed timed plans through presence update commitments; read "
                    + str(c.life / "PRESENCE.md")
                    + " for the update format.",
                )
            )
            ...
            try:
                ...
                moved = companion_plan.render_displacements(
                    companion_plan.displacements(c, now.date())
                )
                if moved:
                    parts.append((7, clip(moved, max(300, b["handoff"] // 2))))
            ...
            parts.append(
                (
                    1,
                    "[No current outfit/location recorded. Read companion_presence.py show and establish state before describing a current scene or making a photo.]",
                )
            )
        ...
        parts.append(
            (
                10,
                "[Current lived state unavailable; do not invent a current outfit or location.]",
            )
        )
    ...
    if c.bars:
        try:
            ...
            parts.append(
                (8, companion_feelings.render(c, companion_feelings.compute(c, now)))
            )
        except (OSError, ValueError, KeyError, TypeError):
            parts.append(
                (
                    8,
                    "[Contextual feelings unavailable; do not invent relationship experiences or infer intent from silence.]",
                )
            )
        try:
            import companion_intimacy

            rendered_intimacy = companion_intimacy.render(
                c, companion_intimacy.compute(c, now)
            )
    ...
    try:
        ...
        if confirmed:
            ...
            handoff_at = (
                stamp.astimezone(tz) if stamp.tzinfo else stamp.replace(tzinfo=tz)
            )
    except Exception:
        pass
    ...
    if b["episodes"]:
        try:
            ...
            allep = read_events(c.life, today, limit=0, tz=tz)
            shown = allep[-5:]
            newest_episode = _latest(allep, tz)
            text = "\n".join(
                f"{e['recorded_at']} | {e['status']} | {e['activity']}: {clip(e['text'],240)}"
                for e in shown
            )
            ...
            fresher = bool(
                newest_episode and handoff_at and newest_episode > handoff_at
            )
            parts.append(
                (
                    10 if fresher else 5,
                    ...
                    + "]\n"
                    + (
                        clip(text, b["episodes"])
                        if text
                        else "No episodes recorded today; do not invent remembered activities from the schedule."
                    ),
                )
            )
            ...
            if sug.get("active_suggestions") or sug.get("later_today"):
                parts.append(
                    (
                        4,
                        "[Routine suggestions only — invitations, not attendance]\n"
                        + clip(
                            json.dumps(
                                {
                                    k: sug[k]
                                    for k in ("active_suggestions", "later_today")
                                    if k in sug
                                },
                                ensure_ascii=False,
                            ),
                            max(120, b["episodes"] // 3),
                        ),
                    )
                )
            ...
            parts.append(
                (
                    5,
                    "[Episode data unavailable; do not assume planned activities happened.]",
                )
            )

    if not b["handoff"] and section(active, "Right now", 20000):
        suppressed.add("the handoff")
    if not b["loops"] and section(active, "Active open loops", 20000):
        suppressed.add("open loops")
    if b["handoff"]:
        ...
        if cur:
            ...
            if handoff_stale:
                label += (
                    ". STALE: the file is old or its timestamp is unreliable, so treat it as a "
    ...
    if b["loops"]:
        ...
        if loops:
            parts.append(
                (
                    8,
                    "[Active open loops; derived — verify facts from original sources]\n"
                    + loops,
                )
            )
    ...
    try:
        ...
        for name, text in (
            ...
            if text and not b[key]:
                suppressed.add(name)
        if me["human_profile"] and b["facts"]:
            parts.append(
                (
                    6,
                    f"[What {c.agent} knows about {c.human} — {n['facts']} recorded facts, all kept; "
        ...
        if me["preferences"] and b["preferences"]:
            parts.append(
                (
                    3,
                    f"[{c.agent}'s own tastes and feelings — {n['preferences']} recorded, all kept; "
        ...
        if me["open_questions"] and b["questions"]:
            parts.append(
                (
                    2,
                    f"[{c.agent}'s open curiosity — {n['open_questions']} open]\n"
                    + me["open_questions"],
                )
            )
    except (OSError, ValueError, KeyError):
        ...
        parts.append(
            (
                6,
                "[Reflection ledgers unavailable this turn — they are not empty, they could not be read. Do not conclude you know nothing.]",
            )
        )
    ...
    try:
        ...
        if not b.get("standing") and companion_notes.standing(c):
            suppressed.add("standing instructions")
        if not b.get("relationship") and companion_notes.moments(c):
            suppressed.add("relationship history")
        if b.get("standing"):
            ...
            if rules:
                parts.append(
                    (
                        7,
                        f"[How {c.human} has asked to be worked with — {c.poss()} rules, not "
        ...
        if b.get("relationship"):
            ...
            if shared:
                parts.append(
                    (
                        5,
                        "[What has actually happened between you — use it the way a person "
        ...
        parts.append(
            (
                5,
                "[Standing instructions and relationship history unavailable this turn — "
    ...
    if RECALL_TRIGGER.search(msg):
        try:
            ev = search(c, msg, limit=3, seconds=0.75)
            parts.append(
                (
                    7,
                    "[Personal-history retrieval; quoted candidates require interpretation]\n"
            ...
            parts.append(
                (
                    7,
                    "[History retrieval unavailable. Acknowledge uncertainty rather than inventing.]",
                )
            )
    ...
    if b["ambient"]:
        amb = []
        for p in (
            sorted((c.soul_dir / "ambient").glob("*.md"))
            if (c.soul_dir / "ambient").is_dir()
            else []
        ):
            t = read(p, 1000).strip()
            if t:
                amb.append(f"{p.stem}: {t}")
        if amb:
            parts.append(
                (1, "[Ambient context]\n" + clip("\n".join(amb), b["ambient"]))
            )
    elif (c.soul_dir / "ambient").is_dir() and any(
        (c.soul_dir / "ambient").glob("*.md")
    ):
        suppressed.add("ambient senses")

    tail = (
        f"[Read more only as needed]\nLife history: companion_life.py history --day YYYY-MM-DD. "
    ...
    if c.compact:
        tail = (
            f"[Lookups] companion_life.py history | companion_self.py profile|pref-history|wonder|soul "
    ...
    if dropped:
        notice = (
            "\n\n[Omitted this turn for context space, nothing lost: "
            + "; ".join(dropped)
...
def main():
    ...
    if c.context_mode == "auto" and isinstance(model, str) and model:
        tokens, source = cc.detect_context_tokens(
            c.home, c.hermes_root, model_name=model
        )
    ...
    try:
        ...
        # conversation.
        if os.environ.get("COMPANION_MEMORY_READ_ONLY") != "1":
            maintain(c, lock_timeout=HOOK_LOCK_SECONDS)
        ...
        notice = (
            "[Memory maintenance skipped this turn: another process holds the lock. "
        ...
        notice = (
            ...
            + str(exc)
            + ". Originals are preserved; check companion_memory.py status before adding memory.]\n"
        )
