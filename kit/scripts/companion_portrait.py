...
def identity_block(c, use_override=True):
    ...
    if not section or not section["body"].strip():
        ...
        try:
            match = re.search(
                r"^#{1,3}\s+(?:Physical Description|Appearance|Visual Identity)\s*\n(.*?)(?=^#{1,3}\s|\Z)",
                text,
                re.M | re.S | re.I,
            )
    ...
    for block in re.split(r"\n\s*\n", section["body"]):
        if EXCLUDE in block or "`" in block:
            break
        kept.append(block)
    lines = [
        l.strip()
        for l in "\n".join(kept).splitlines()
        if l.strip() and "✎ EDIT" not in l
    ]
...
def scene_block(c, record=None):
    ...
    if place and not re.match(
        r"(in|on|at|by|near|inside|outside|under|beside)\b", place.strip(), re.I
    ):
        place = "in " + place
    ...
    parts.extend(
        ...
        if value and key not in OWN_BOX
    )
...
DAYLIGHT = (
    (5, "soft dawn light"),
    (8, "bright morning light"),
    (11, "midday daylight"),
    (16, "warm late afternoon light"),
    (19, "golden hour, low sun"),
    (21, "blue hour dusk"),
)
INDOOR_WORDS = (
    "indoors",
...
def lighting_guess(c, location="", now=None):
    ...
    try:
        zone = (
            dt.timezone.utc
            if not c.timezone
            else __import__("zoneinfo").ZoneInfo(c.timezone)
        )
    except Exception:
    ...
    for start, label in DAYLIGHT:
        if hour >= start:
            daylight = label
    if indoor:
        return daylight + " through a window"
    return daylight
...
def feeling_text(state):
    ...
    if isinstance(wants, list):
        values.extend(str(value).strip() for value in wants if str(value).strip())
    elif str(wants).strip():
        values.append(str(wants).strip())
    return ", ".join(values)
...
def undressed_render_allowed(c):
    ...
    return allowed, (
        ""
        if allowed
        else (" ".join(blockers)[:200] or "closeness has not reached that point")
    )
...
def compile_prompt(c, extra="", use_reference=False):
    ...
    pieces = [identity]
    if parts.get("wardrobe"):
        pieces.append("She is " + wardrobe_clause(parts["wardrobe"]))
    pieces.append(scene)
    if parts.get("feeling"):
        pieces.append(parts["feeling"])
    if parts.get("lighting"):
        pieces.append(parts["lighting"])
    if parts.get("camera"):
        pieces.append(parts["camera"])
    if extra.strip():
    ...
    return {
        "ready": True,
        "prompt": prompt,
        "image_preset": selected or None,
        "generation_command": (
            ("companion_portrait.py --home " + str(c.home) + " generate")
            if selected
            else None
        ),
        "identity": identity,
        ...
        "style": style,
        "reference_image": (
            str(reference) if use_reference and reference.is_file() else None
        ),
        "recorded_at": record["recorded_at"] if record else None,
        "note": (
            "Generate a fresh image from the appearance description and scene. "
...
def main():
    ...
    pr.add_argument(
        "--use-reference",
        action="store_true",
        help="Explicitly reuse the saved portrait as an input image",
    )
    pr.add_argument("--extra", default="", help="anything specific to this shot")
    gp = s.add_parser(
        "generate", help="generate using the saved Image Studio category/workflow"
    )
    ...
    gp.add_argument(
        "--allow-nsfw",
        action="store_true",
        help="The intended image explicitly includes adult content",
    )
    gp.add_argument(
        "--recorded",
        action="store_true",
        help="Use the current recorded scene and wardrobe exactly",
    )
    rv = s.add_parser(
        "review", help="Review an externally generated image before delivery"
    )
    ...
    c = cc.load(a.home)
    if a.cmd == "prompt":
        out = compile_prompt(c, a.extra, a.use_reference)
    elif a.cmd == "generate":
        import companion_media as media
        ...
        if a.recorded:
            ...
            if reason == "declared":
                raise ValueError(
                    "She marked this moment private; it cannot be photographed."
                )
            if reason == "undressed":
                allowed, why = undressed_render_allowed(c)
                if not allowed:
                    raise ValueError(
                        "This moment is a private one and cannot be photographed: "
                        + why
                    )
                intimate = undress(state, closet)[0] in (
                    "undressed",
                    "bathing",
                    "underwear",
                )
