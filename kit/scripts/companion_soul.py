...
LOCKABLE = tuple(
    k for k, v in SECTIONS.items() if v["keeper"] == "shared" or v.get("unlockable")
)
...
def policy_path(c):
    return c.soul_dir / "soul-policy.json"


def changes_path(c):
    return c.soul_dir / "soul-changes.jsonl"


# Hidden: the vault browser, the vault map and the app's document list all skip dotfiles.
def private_path(c):
    return c.soul_dir / ".private.md"


def policy(c):
    try:
        data = json.loads(policy_path(c).read_text(encoding="utf-8"))
...
def locked(c, section, pol=None):
    ...
    if spec["keeper"] in ("anchor", "fixed", "system"):
        if spec.get("unlockable"):
            return (pol or policy(c))["locks"].get(section, True)
        return True
    if spec["keeper"] == "hers":
        return False
...
def set_lock(c, section, value):
    ...
    if section not in LOCKABLE:
        raise ValueError(f"{section} cannot be locked or unlocked")
    data = policy(c)
    data["locks"][section] = bool(value)
...
def stage_reached(c, state=None):
    ...
    try:
        state = state or intimacy.compute(c)
    except Exception:
        state = state or {"stage": 0}
...
def days_together(c, now=None):
    ...
    try:
        start = dt.date.fromisoformat(c.relationship_started)
...
def _stage_name(c, stage):
    try:
        ...
        return STAGE_NAMES_ROMANTIC[stage]
...
def access(c, section, now=None, stage=None, pol=None):
    ...
    if not spec:
        return {"can": False, "why": f"no section {section!r}"}
    if section in SECTIONS and locked(c, section, pol):
        keeper = SECTIONS[section]["keeper"]
        return {
            "can": False,
            "why": (
                "the app keeps this one"
                if keeper in ("fixed", "system")
                else (
                    f"{c.human} keeps this one"
                    if keeper == "anchor"
                    else f"{c.human} has locked it"
                )
            ),
        }
    need = int(spec.get("stage", 0))
    reached = stage_reached(c) if stage is None else stage
    if reached < need:
        return {
            "can": False,
            "why": f"opens at {_stage_name(c,need)}",
            "opens_stage": need,
        }
    ...
    if days < int(spec.get("days", 0)):
        return {
            "can": False,
            "why": f'opens after {spec["days"]} days together ({days} so far)',
    ...
    if last:
        ...
        try:
            when = dt.datetime.fromisoformat(last["at"])
        except (KeyError, ValueError):
            when = None
        if when and now - when < wait:
            return {
                "can": False,
                "why": f"rewritten on {when.date().isoformat()}; next change after "
                f"{(when+wait).date().isoformat()}",
                "cooldown": True,
            }
...
def status(c, now=None):
    ...
    for key, spec in SECTIONS.items():
        rows.append(
            {
                "id": key,
                "title": _fill(spec["title"], c),
                "keeper": spec["keeper"],
                "lockable": key in LOCKABLE,
                "locked": locked(c, key, pol),
                "guide": _fill(spec["guide"], c),
                "her": access(c, key, now, stage, pol),
    ...
    return {
        "stage_reached": stage,
        "stage_name": _stage_name(c, stage),
        "days_together": days_together(c, now),
        "sections": rows,
    }
...
def private_notes(c):
    ...
    try:
        text = private_path(c).read_text(encoding="utf-8")
    except OSError:
        return {}
    out = {}
    for key in PRIVATE:
        m = re.search(
            re.escape(PRIVATE_BEGIN.format(key))
            + r"\n?(.*?)"
            + re.escape(PRIVATE_END.format(key)),
            text,
            re.S,
        )
...
def _write_private(c, key, body):
    ...
    for k, spec in PRIVATE.items():
        if k in notes:
            lines += [
                "## " + _fill(spec["title"], c),
                PRIVATE_BEGIN.format(k),
                notes[k],
                PRIVATE_END.format(k),
                "",
            ]
...
def render_private(c, budget=1600):
    ...
    return (
        "[Your private notes, written by you and seen by nobody else. They inform how you feel; "
...
def write(c, section, text, mode="set", why="", now=None):
    ...
    if mode not in ("set", "append"):
        raise ValueError("mode must be set or append")
    if not text:
        raise ValueError("nothing to write")
    why = (why or "").strip()
    if not why:
        raise ValueError(
            "say why: a change to who you are needs a reason, in a sentence"
        )
    gate = access(c, section, now)
    if not gate["can"]:
        raise ValueError(f'you cannot write "{section}" yet: {gate["why"]}')
    import companion_identity as ident

    guarded = ident.guard(c, text)
    if guarded and section != "appearance":
        raise ValueError(
            "Refused: this restates something that is not yours to change ("
            + ", ".join(guarded)
            + ")."
        )
    if section == "own-words":
        ...
        if len(body) > gate["cap"]:
            raise ValueError(
                f'keep it under {gate["cap"]} characters; this is {len(body)}'
            )
        _write_private(c, section, body)
        chars = len(body)
    else:
        _, soul = ident.read(c)
        found = ident.sections(soul)
        if section not in found:
            raise ValueError(f'this SOUL has no "{section}" section yet')
        ...
        ident.replace(c, section, body, now)
        chars = len(body)
    row = {
    ...
    with file_lock(changes_path(c).with_suffix(".lock")):
        with changes_path(c).open("a", encoding="utf-8") as f:
...
def refresh_operating(c, report=None, now=None):
    ...
    if fresh.strip() == body.strip():
        return False
    ident.replace(c, "operating", fresh, now)
    if report is not None:
        report.append('  soul: "How things work" brought up to date (local notes kept)')
    return True
...
def for_prompt(c, now=None):
    ...
    for row in st["sections"]:
        if row["keeper"] in ("anchor", "fixed", "system"):
            continue
        state = "you can write this now" if row["her"]["can"] else row["her"]["why"]
        where = (
            " (private — only you ever read it)" if row["keeper"] == "private" else ""
        )
...
def main():
    ...
    w = s.add_parser(
        "write", help="rewrite or add to one section you are allowed to write"
    )
    w.add_argument("--section", required=True)
    w.add_argument(
        "--file", type=pathlib.Path, required=True, help="UTF-8 file with the new text"
    )
    ...
    c = cc.load(a.home)
    if a.cmd == "status":
        out = {**status(c), "summary": for_prompt(c)}
    elif a.cmd == "history":
        out = {"changes": changes(c)}
    else:
        out = write(c, a.section, a.file.read_text(encoding="utf-8"), a.mode, a.why)
