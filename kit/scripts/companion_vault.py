...
def available():
    try:return subprocess.run(['git','--version'],capture_output=True,timeout=30).returncode==0
...
def init(c):
    ...
    if not available():return {'ready':False,'reason':'git is not installed'}
    root.mkdir(parents=True,exist_ok=True)
    if is_repo(root):return {'ready':True,'created':False,'root':str(root)}
    r=_git(root,'init','-q')
...
def commit(c,now=None,message=None):
    ...
    root=pathlib.Path(c.vault)
    state=init(c)
    if not state.get('ready'):return {'committed':False,**state}
    status=_git(root,'status','--porcelain')
    if status.returncode:return {'committed':False,'reason':(status.stderr or '').strip()[:200]}
    if not status.stdout.strip():return {'committed':False,'reason':'nothing changed'}
    changed=len(status.stdout.strip().splitlines())
    add=_git(root,'add','-A')
    if add.returncode:return {'committed':False,'reason':(add.stderr or '').strip()[:200]}
    text=message or f'{changed} file(s) at {now.isoformat(timespec="minutes")}'
    r=_git(root,'commit','-q','-m',text)
    if r.returncode:return {'committed':False,'reason':(r.stderr or r.stdout or '').strip()[:200]}
...
def history(c,relative,limit=20):
    ...
    if _git(root,'rev-parse','--verify','HEAD').returncode:return []
    r=_git(root,'log',f'-{max(1,min(limit,200))}','--format=%H\t%cI\t%s','--',relative)
    if r.returncode:raise ValueError((r.stderr or 'no history for that path').strip()[:200])
    rows=[]
    for line in r.stdout.splitlines():
        parts=line.split('\t',2)
        if len(parts)==3:rows.append({'commit':parts[0],'at':parts[1],'message':parts[2]})
    return rows

def show(c,relative,commit_id):
    r=_git(pathlib.Path(c.vault),'show',f'{commit_id}:{relative}')
    if r.returncode:raise ValueError((r.stderr or 'that version does not exist').strip()[:200])
    return r.stdout
...
def restore(c,relative,commit_id,now=None):
    ...
    return {'restored':str(out),'from':commit_id,'original':str(target),
...
def main():
    ...
    s.add_parser('init');s.add_parser('commit')
    h=s.add_parser('history');h.add_argument('path');h.add_argument('--limit',type=int,default=20)
    r=s.add_parser('restore');r.add_argument('path');r.add_argument('--commit',required=True)
    a=p.parse_args();c=cc.load(a.home)
    if a.cmd=='init':out=init(c)
    elif a.cmd=='commit':
        out=commit(c)
        # No-agent job: say nothing on an ordinary quiet commit.
        if out.get('committed') or out.get('reason')=='nothing changed':return 0
    elif a.cmd=='history':out={'versions':history(c,a.path,a.limit)}
    else:out=restore(c,a.path,a.commit)
