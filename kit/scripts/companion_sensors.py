...
import argparse, datetime as dt, json, math, pathlib, sys, urllib.error, urllib.parse, urllib.request
...
def weather(c,now):
    ...
    if not c.location:return None,'no location is set for this companion'
    url='https://wttr.in/'+urllib.parse.quote(c.location)+'?format=j1'
    try:data=json.loads(_get(url))
    ...
    try:
        ...
        text=(f"{c.location}: {desc.lower()}, {current['temp_C']}°C "
        ...
        extra={'sunrise':today['astronomy'][0]['sunrise'],'sunset':today['astronomy'][0]['sunset'],
               'temp_c':current['temp_C'],'description':desc}
        return text,extra
    except (KeyError,IndexError,TypeError) as exc:
        return None,f'weather service returned a shape we do not understand: {exc}'

# ---- daylight, season, moon ---------------------------------------------
SEASONS_NORTH=('winter','winter','spring','spring','spring','summer',
...
def daylight(c,now):
    ...
    lines=[f"It is {now.strftime('%A %-d %B')}, in {season}.",
    ...
    try:
        ...
        if data.get('sunrise') and data.get('sunset'):
            lines.append(f"Sunrise was {data['sunrise'].lower()}, sunset is {data['sunset'].lower()}.")
...
def dates(c,now):
    ...
    if not path.exists():
        return None,(f'no dates file yet — write one at {path}, one per line as "MM-DD | what it is"')
    upcoming=[]
    for raw in path.read_text(encoding='utf-8').splitlines():
        ...
        try:month,day=(int(x) for x in parts[0].split('-')[-2:])
        except ValueError:continue
        for year in (now.year,now.year+1):
            try:when=dt.date(year,month,day)
            except ValueError:break
            delta=(when-now.date()).days
            if 0<=delta<=7:
                when_text='today' if not delta else 'tomorrow' if delta==1 else f'in {delta} days'
                upcoming.append((delta,f'{parts[1]} — {when_text} ({when.isoformat()})'))
                break
    if not upcoming:return None,'nothing in the next seven days'
    upcoming.sort()
    return 'Coming up:\n'+'\n'.join(f'- {text}' for _,text in upcoming),{'count':len(upcoming)}
...
def thread(c,now):
    ...
    if not data.get('available'):return None,data.get('reason','no conversation history yet')
...
def durations(c,now):
    ...
    """
    path=c.data/'dates.md'
    if not path.exists():return None,'no dates file'
    out=[]
    for raw in path.read_text(encoding='utf-8').splitlines():
        line=raw.strip()
        if not line or line.startswith('#'):continue
        ...
        if len(parts)!=2:continue
        try:when=dt.date.fromisoformat(parts[0])
        except ValueError:continue     # MM-DD lines belong to the lookahead only
        years=now.year-when.year-((now.month,now.day)<(when.month,when.day))
        if years<1:continue
        out.append(f'{parts[1]}: {years} year{"s" if years!=1 else ""} as of today ({parts[0]})')
    if not out:return None,'no dated anniversaries to count'
    return ('Counted from the dates themselves, so these are right today and will be right next '
            'year:\n'+'\n'.join(f'- {line}' for line in out),{'count':len(out)})
            ...
            try:
                ...
                if cur and cur.get('item'):
                    ...
                    try:
                        ...
                        pass
                    return text, {'source': 'spotify', 'state': 'playing' if playing else 'paused',
                                  'track': it.get('name'), 'artists': artists}
                ...
                if items:
                    ...
                    try:
                        ...
                        pass
                    return text, {'source': 'spotify', 'state': 'recent',
                                  'track': it.get('name'), 'artists': artists}
            except Exception:
                pass
...
REGISTRY={
 'weather':{'fn':'weather','needs':'a location','blurb':'Temperature and conditions where you live, hourly. '
            'No account needed. Grounds "it is grim out" in something true.'},
 'daylight':{'fn':'daylight','needs':None,'blurb':'The season, the moon, and the length of the day. '
             'Computed, no network, always available.'},
 'dates':{'fn':'dates','needs':'a dates.md file','blurb':'Birthdays and anniversaries you write down, '
          'surfaced a week ahead so they can be remembered rather than announced.'},
 'care':{'fn':'care','needs':'a care.md file','blurb':'Things worth asking about again — an interview, '
         'a hospital appointment, a hard week. This is how a companion circles back.'},
 'durations':{'fn':'durations','needs':'dated lines in dates.md','blurb':
   'How long things have been true — "married 13 years" — counted from the date rather than '
   'remembered as a number that goes quietly wrong.'},
 'thread':{'fn':'thread','needs':None,'blurb':'How long since either of you wrote, and what that stretch '
           'of quiet tends to mean. The most useful thing on this list.'},
 'music':{'fn':'music','needs':'Spotify auth or ambient mood','blurb':
          'What the human is playing on Spotify, or the ambient listening mood for the room.'},
}

def run_one(c,name,now):
    spec=REGISTRY.get(name)
    if not spec:raise ValueError(f'unknown sensor {name!r}; known: {sorted(REGISTRY)}')
    folder=c.soul_dir/'ambient';folder.mkdir(parents=True,exist_ok=True)
    md=folder/f'{name}.md';data_path=folder/f'{name}.json'
    try:text,extra=globals()[spec['fn']](c,now)
    except Exception as exc:text,extra=None,f'sensor failed: {exc}'
    if not text:
        ...
        return {'sensor':name,'wrote':False,'reason':extra if isinstance(extra,str) else 'nothing to report'}
    atomic_write(md,text.rstrip('\n')+'\n')
    if isinstance(extra,dict):
        atomic_write(data_path,json.dumps({**extra,'checked_at':now.isoformat()},ensure_ascii=False,indent=2))
...
def main():
    ...
    p.add_argument('--list',action='store_true',help='what is available and what each needs')
    a=p.parse_args()
    if a.list:
        print(json.dumps({k:{'needs':v['needs'],'blurb':v['blurb']} for k,v in REGISTRY.items()},indent=2))
        return 0
    c=cc.load(a.home)
    ...
    if problems and a.only:print(json.dumps(results,indent=2))
    return 0
