...
def _next_at(now, hhmm):
    ...
    if minute is None:
        raise ValueError("Give the time as HH:MM, e.g. 07:00")
...
def read(c, now=None):
    ...
    try:
        ...
        return None
    if start.tzinfo is None or until.tzinfo is None:
        return None
    if (
        not dt.timedelta(hours=MIN_HOURS)
        <= until - start
        <= dt.timedelta(hours=MAX_HOURS)
    ):
        return None
    ...
    if plan:
        ...
        if now < plan["until"] + dt.timedelta(hours=MAX_HOURS):
            return {
                "asleep": False,
                "source": "declared",
                "starts": plan["from"].isoformat(timespec="minutes"),
                "until": plan["until"].isoformat(timespec="minutes"),
...
def declare(c, until, note="", now=None):
    ...
    if span < dt.timedelta(hours=MIN_HOURS):
        raise ValueError(
            f"A declared night runs at least {MIN_HOURS} hour; for a nap, just stay awake on the record"
        )
    if span > dt.timedelta(hours=MAX_HOURS):
        raise ValueError(
            f"A declared night runs at most {MAX_HOURS} hours; {until} is too far off"
        )
    plan = {
        "declared_at": now.isoformat(timespec="minutes"),
        "from": now.isoformat(timespec="minutes"),
        "until": end.isoformat(timespec="minutes"),
        "hours": round(span.total_seconds() / 3600, 2),
...
def wake(c, now=None):
    ...
    if not plan:
        return {
            "woke": True,
            "was": None,
            "note": "No night was declared; nothing to end.",
        }
    was = plan["until"]
    ...
    if now - plan["from"] < dt.timedelta(hours=MIN_HOURS):
        ...
        ended = {
            **{k: v for k, v in plan.items() if k not in ("from", "until")},
    ...
    return {
        "woke": True,
        "was": was.isoformat(timespec="minutes"),
...
def wake_moment(c, day):
    ...
    plan = read(c)
    if plan and plan["until"].astimezone(tz).date() == day:
        moment = plan["until"].astimezone(tz)
    lo = end - dt.timedelta(hours=WAKE_BEFORE_HOURS)
    hi = end + dt.timedelta(hours=WAKE_AFTER_HOURS)
...
def main():
    ...
    d = s.add_parser(
        "declare", help="Declare how long tonight is meant to be, before sleeping"
    )
    d.add_argument(
        "--until", required=True, help="Intended wake time as HH:MM, e.g. 07:00"
    )
    ...
    if a.cmd == "fingerprint":
        sys.stdout.write(wake_fingerprint(c))
        return 0
    if a.cmd == "declare":
        out = declare(c, a.until, a.note)
    elif a.cmd == "wake":
        out = wake(c)
    else:
        out = status(c)
