...
def open_url(url, key="", timeout=60):
    ...
    if key and parsed.hostname in ("civitai.com", "www.civitai.com"):
...
def download(payload, report=lambda _: None):
    ...
    if not (root / "main.py").is_file():
        raise ValueError("Choose the ComfyUI root directory containing main.py")
    ...
    if (
        ...
        or name.startswith(".")
        or Path(name).suffix.lower() not in (".safetensors", ".gguf")
    ):
    ...
    if not re.fullmatch("[a-f0-9]{64}", digest):
        raise ValueError("A publisher SHA-256 is required")
    ...
    if (
        ...
        raise ValueError("Invalid or oversized model file")
    url = payload["url"]
    host = urllib.parse.urlsplit(url).hostname
    if host not in ("civitai.com", "www.civitai.com"):
        raise ValueError("Downloads must originate at Civitai")
    ...
    for p in [root / "models", folder]:
        if p.is_symlink():
            raise ValueError(
                "Use a real model folder beneath the configured Comfy root"
            )
    ...
    if dest.exists():
        with dest.open("rb") as f:
            existing = hashlib.file_digest(f, "sha256").hexdigest()
        if existing != digest:
            raise ValueError(
                "A different file already has this name; it was not replaced"
            )
        return {**record_metadata(root, payload), "reused": True}
    if shutil.disk_usage(folder).free < size + 512 * 1024**2:
        raise ValueError("Not enough free disk space for these weights")
    temporary = folder / ("." + name + "." + uuid.uuid4().hex + ".partial")
    count = 0
    sha = hashlib.sha256()
    last = 0
    try:
        with (
            open_url(url, payload.get("api_key", "")) as response,
            temporary.open("xb") as output,
        ):
            while chunk := response.read(1024**2):
                count += len(chunk)
                if count > min(MAX_BYTES, max(size * 1.02, size + 1024**2)):
                    raise ValueError("Download is larger than the published file size")
                sha.update(chunk)
                output.write(chunk)
                if count - last >= 16 * 1024**2:
                    report(f"Downloaded {count/1024**2:.0f} / {size/1024**2:.0f} MB")
                    last = count
            output.flush()
            os.fsync(output.fileno())
        if sha.hexdigest() != digest:
            raise ValueError(
                "SHA-256 mismatch; incomplete or changed weights were discarded"
            )
...
def record_metadata(root, payload):
    ...
    if manifest.is_symlink():
        raise ValueError("Library metadata must stay beneath the Comfy root")
    manifest.mkdir(exist_ok=True)
    record = manifest / (payload["sha256"].lower() + ".json")
    if record.is_symlink():
        raise ValueError("Library metadata cannot be a symbolic link")
...
def library(payload):
    root = Path(payload["root"]).expanduser().resolve()
    if not (root / "main.py").is_file():
        raise ValueError("Choose the ComfyUI root containing main.py")
    result = []
    for path in (root / "models/.companion-library").glob("*.json"):
        ...
        try:
            ...
            if (
                ...
                and (root / "models" / SLOTS[row["slot"]] / row["filename"]).is_file()
            ):
                result.append(row)
...
if __name__ == "__main__":
    try:
        ...
        result = (
            library(payload)
            if payload.get("action") == "library"
            else download(payload, lambda value: print("PROGRESS=" + value, flush=True))
        )
        ...
        message = (
            ...
            else type(exc).__name__ + ": model download failed"
        )
