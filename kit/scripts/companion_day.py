    ...
    commitment = obj(
        {
            ...
            "starts_at": string(80),
            "ends_at": string(80),
            "buffer_minutes": {"type": "integer", "minimum": 0, "maximum": 1440},
            "status": {"type": "string", "enum": ["planned", "completed", "cancelled"]},
            "reason": string(300),
        }
    )
    return {
        "activity_change": {"type": "string", "enum": ["continue", "transition"]},
        "duration_minutes": {
            "type": ["integer", "null"],
            "minimum": 1,
            "maximum": 1440,
...
def _leaf_reason(exc):
    ...
    if (
        "expected" in inner
        and "null" in inner
        and "string" not in inner
        and "integer" not in inner
    ):
        return "", True
...
def _validate(value, schema, name):
    ...
    if "anyOf" in schema:
        ...
        for option in schema["anyOf"]:
            try:
                _validate(value, option, name)
                return
            except ValueError as exc:
                failures.append(_leaf_reason(exc))
        ...
        if not reasons:
            ...
            reasons = [
                f"expected an object such as {{activity, duration_minutes, reason}} or null, got {_short(value)}"
            ]
    ...
    if kind not in kinds:
        raise ValueError(f"Invalid {name}: expected {kinds}, got {_short(value)}")
    if kind == "object":
        ...
        for key, item in value.items():
            _validate(item, schema["properties"][key], f"{name}.{key}")
    elif kind == "array":
        ...
        if (
            "maxLength" in schema
            and not schema.get("minLength", 0)
            <= len(value.strip())
            <= schema["maxLength"]
        ):
            ...
            raise ValueError(
                f'Invalid length for {name}: expected {low}-{schema["maxLength"]} characters, got {len(value.strip())}'
            )
        if "enum" in schema and value not in schema["enum"]:
            raise ValueError(f"Invalid {name}")
    elif kind == "integer" and not schema["minimum"] <= value <= schema["maximum"]:
        raise ValueError(
            f'Invalid {name}: expected {schema["minimum"]}-{schema["maximum"]}, got {value}'
        )
...
def timestamp(value):
    try:
        ...
        raise ValueError("Commitment timestamps must be ISO dates with timezone")
    if parsed.tzinfo is None:
        raise ValueError("Commitment timestamps require timezone")
    return parsed
...
def evolve(data, state, previous, now, asleep=False):
    ...
    for key in fields:
        if key in data:
            _validate(data[key], fields[key], key)
    changed = any(state.get(k) != before.get(k) for k in ("activity", "location"))
    if data.get("activity_change") == "continue" and state.get(
        "location"
    ) == before.get("location"):
        changed = False
    elif data.get("activity_change") == "transition":
        changed = True
    ...
    if (
        asleep
        and data.get("activity_change") != "transition"
        and state.get("location") == before.get("location")
    ):
        changed = False
    result = {
        "started_at": (
            now.isoformat()
            if changed
            else before.get("started_at", previous["recorded_at"])
        ),
    ...
    if previous and changed:
        ...
        result["previous"].update(
            started_at=before.get("started_at", previous["recorded_at"]),
            last_recorded_at=previous["recorded_at"],
            confirmed=before.get("confirmed", True),
        )
    result["duration_minutes"] = data.get(
        "duration_minutes", None if changed else before.get("duration_minutes")
    )
    ...
    for item in data.get("commitments", []):
        ...
        seen.add(ident)
        if timestamp(item["ends_at"]) <= timestamp(item["starts_at"]):
            raise ValueError("Commitment ends_at must follow starts_at")
        old = commitments.get(ident)
        if (old and old != item or item["status"] != "planned") and not item[
            "reason"
        ].strip():
    ...
    if len(active) > 64:
        raise ValueError("Resolve outstanding commitments before adding more")
    ...
    result["visual"] = data.get(
        "visual", {} if visible_changed else before.get("visual", {})
    )
    return result
...
def conflicts(state, now):
    ...
    if state.get("duration_minutes") and state.get("started_at"):
        remaining = max(
            ...
            - (now - timestamp(state["started_at"])).total_seconds() / 60,
        )
    result = []
    for item in state.get("commitments", []):
        if item["status"] != "planned":
            continue
        deadline = timestamp(item["starts_at"]) - dt.timedelta(
            minutes=item["buffer_minutes"]
        )
        available = (deadline - now).total_seconds() / 60
        if 0 < available < remaining + intent["duration_minutes"]:
            result.append(
                f"{item['title']}: {remaining:g} minutes remaining now + {intent['duration_minutes']} minutes next exceeds {available:g} minutes before preparation at {deadline.isoformat()}"
            )
    return result
...
def validate_plan(data, state, now):
    ...
    if problems:
        raise ValueError(
            "PLAN CONFLICT: "
            + "; ".join(problems)
            + ". Shorten, defer or replace next; reconsider current duration. Set next to null if undecided."
        )
...
def render(state, now):
    ...
    if previous:
        lines.append(
            f"[1. What did I just do?] Previously: {previous['activity']} at {previous['location']} (last recorded {previous['last_recorded_at']}"
            + ("; unconfirmed" if not previous.get("confirmed", True) else "")
            + ")."
        )
    if state.get("started_at"):
        lines.append(
            f"[2. What am I doing right now?] Now: {state.get('activity')} since {state['started_at']}; estimated total duration {state.get('duration_minutes') or 'unknown'} minutes."
        )
        elapsed = (now - timestamp(state["started_at"])).total_seconds() / 60
        if state.get("duration_minutes") and elapsed >= state["duration_minutes"]:
            lines.append(
                f"ACTIVITY DUE FOR RECONSIDERATION: {elapsed:g} minutes elapsed. Choose the next activity or explain a concrete delay; do not restart this timer by paraphrasing."
            )
    intent = state.get("next")
    lines.append(
        "[3. What will I do next?] Next (tentative, not completed): "
        + (
            f"{intent['activity']} (~{intent['duration_minutes']} min). {intent['reason']}"
            if intent
            else "undecided."
        )
    )
    lines.append(
        "[4. What should I do later?] Later today: Check upcoming daily anchors, meals, hobbies, and evening wind-down."
    )
    ...
    for item in state.get("commitments", []):
        ...
        lines.append(
            f"Commitment {item['id']}: {item['title']}, {item['starts_at']} to {item['ends_at']}; preparation/travel begins {deadline.isoformat()} ({minutes} min from now)."
        )
        if now >= timestamp(item["ends_at"]):
            lines.append(
                "OVERDUE: establish what happened; completion is not recorded."
            )
        elif minutes <= 0:
            lines.append(
                "DUE: preparation or commitment time; reconsider the current activity and next intention."
            )
        elif intent:
            remaining = 0
            if state.get("duration_minutes") and state.get("started_at"):
            ...
            if remaining + intent["duration_minutes"] > minutes:
                lines.append(
                    "PLAN CONFLICT: current activity plus next intention will not fit before preparation. Shorten, defer or replace next; explain why."
                )
    return "\n".join(lines)
...
def visual_key(state):
    return hashlib.sha256(
        json.dumps(
            {k: state.get(k) for k in ("activity", "location", "outfit", "visual")},
            sort_keys=True,
            ensure_ascii=False,
        ).encode()
    ).hexdigest()
