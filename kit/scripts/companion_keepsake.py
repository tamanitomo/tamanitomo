...
import argparse, datetime as dt, hashlib, json, pathlib, shutil, sys
...
def save(c, image_path, title, note, share=False, now=None) -> dict:
    ...
    if src.suffix.lower() not in ALLOWED_EXTENSIONS:
        raise ValueError(f"Image extension {src.suffix} not supported; must be one of {ALLOWED_EXTENSIONS}")
...
def share(c, ident, note=None, now=None) -> dict:
    ...
    caption = f"{title}\n\n{effective_note}" if title and effective_note else (title or effective_note)
...
def main():
    ...
    save_p.add_argument('--share', action='store_true', help='Queue to outbox for delivery')
