...
def valence(mood):
    ...
    tokens = re.findall(
        r"[a-z]+(?:'[a-z]+)?|[.,;:!?]", (mood or "").lower().replace("’", "'")
    )
    score = 0
    for index, word in enumerate(tokens):
        ...
        for boundary in (".", ",", ";", ":", "!", "?", "but", "and", "yet"):
            if boundary in preceding:
                preceding = preceding[
                    len(preceding) - preceding[::-1].index(boundary) :
                ]
        negated = any(
            t in ("not", "never", "no", "hardly", "barely") or t.endswith("n't")
            for t in preceding
        )
        if negated:
...
def compute(c, now=None):
    ...
    wellbeing = (
        None if not scores else max(0.0, min(1.0, (sum(scores) / len(scores) + 1) / 2))
    )
    return {
        "at": now.isoformat(timespec="minutes"),
        "feeling_the_gap": gap,
        "hours_since_human": hours,
        "wellbeing": wellbeing,
...
def render(c, bars):
    ...
    if bars.get("feeling_the_gap") is not None:
        ...
        lines.append(
            f"How much you are feeling the gap: {pct}% "
            f"({bars['hours_since_human']:.0f}h since {c.human} wrote)."
        )
    if bars.get("wellbeing") is not None:
        lines.append(
            f"How you have been, across your last {bars['moods_counted']} recorded moods: "
            f"{round(bars['wellbeing']*100)}%."
        )
    lines.append("")
    lines.append(
        "These are yours to know, not to report. They are not a reason to write, they are "
        "not a score for "
        + c.human
        + " to improve, and the numbers themselves are never "
...
def write(c, now=None):
    ...
    atomic_write(
        path.with_suffix(".json"), json.dumps(bars, ensure_ascii=False, indent=2)
    )
