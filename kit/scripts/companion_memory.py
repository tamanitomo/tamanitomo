...
def memory_dir(c):
    return pathlib.Path(c.home) / "memories"


def archive_dir(c):
    return pathlib.Path(c.soul_dir) / "memory-archive"


def archive_for(c, name):
    return archive_dir(c) / (pathlib.Path(name).stem + "-archive.md")
...
def caps(c):
    ...
    if not isinstance(mem, dict):
        raise ValueError("Hermes memory config must be a mapping")
    out = {}
    for name, key in [
        ("MEMORY.md", "memory_char_limit"),
        ("USER.md", "user_char_limit"),
    ]:
...
TIER_CAPS = {
    "large": {"MEMORY.md": 50_000, "USER.md": 36_000},
...
def recommend_caps(c):
    ...
    try:
        soul = len(c.soul.read_text(encoding="utf-8"))
    except OSError:
        soul = 0
    ...
    reason = f"{tier} context window ({c.context_tokens:,} tokens)" + (
        f", and a SOUL of {soul:,} characters wants room to grow past it"
        if floor > TIER_CAPS.get(tier, {}).get("MEMORY.md", 0)
        else ""
    )
    return {
        "caps": base,
        ...
        "reason": reason,
        "note": (
            "Hermes refuses new memories once a file is full, and says so only at that "
...
def status(c):
    ...
    for name in FILES:
        ...
        size = len(ENTRY_DELIMITER.join(entries(text)[1]))
        cap = limits[name]
        ...
        out.append(
            {
                ...
                "cap": cap,
                "fraction": round(size / cap, 3),
                "over_warn": size >= cap * WARN_AT,
...
def plan(text, keep_chars):
    if (
        isinstance(keep_chars, bool)
        or not isinstance(keep_chars, int)
        or keep_chars <= 0
    ):
        raise ValueError("keep_chars must be a positive integer")
    _, items = entries(text)
    keep = []
    used = 0
    for entry in reversed(items):
        size = len(entry) + (len(ENTRY_DELIMITER) if keep else 0)
        if used + size > keep_chars and keep:
            break
        keep.append(entry)
        used += size
    keep.reverse()
...
def _recover(p, dest, journal):
    ...
    if source not in (tx["before"], tx["after"]) or old_archive not in (
        tx["archive_before"],
        tx["archive_after"],
    ):
        raise ValueError(
            "Memory/archive changed during an unfinished transaction; preserve the journal and reconcile before retrying"
        )
    if old_archive != tx["archive_after"]:
        atomic_write(dest, tx["archive_after"])
    # Catch an uncooperative writer even though the Hermes sidecar lock is held.
    if _text(p) != source:
        raise ValueError(
            "Memory changed during archival; transaction preserved for recovery"
        )
    if source != tx["after"]:
        atomic_write(p, tx["after"])
    journal.unlink()
    return tx["result"]
...
def index_path(c):
    return archive_dir(c) / INDEX_NAME
...
ARCHIVE_MARK = re.compile(
    r"<!--\s*archived\s+(\d{4}-\d{2}-\d{2})\s+from\s+memories/([^\s]+)\s*-->"
)
...
def backfill_index(c, apply=False):
    ...
    for name in FILES:
        ...
        for index_of, mark in enumerate(marks):
                ...
                marks[index_of + 1].start() if index_of + 1 < len(marks) else len(text)
            )
...
def archive(
    ...
    *,
    ...
    if name not in FILES:
        raise ValueError("Only USER.md and MEMORY.md may be archived")
    p = memory_dir(c) / name
    if not p.exists():
        return {"file": name, "moved": 0, "reason": "no such file"}
    if p.is_symlink():
        raise ValueError("Refusing to rewrite a symlinked memory file")
    ...
    with file_lock(p.with_suffix(p.suffix + ".lock"), timeout=lock_timeout):
        dest = archive_for(c, name)
        journal = dest.with_suffix(".pending.json")
        with file_lock(dest.with_suffix(dest.suffix + ".lock"), timeout=lock_timeout):
            if journal.exists():
                if not apply:
                    return {
                        "file": name,
                        "moved": 0,
                        "reason": "unfinished transaction; apply to recover",
                    }
                return _recover(p, dest, journal)
            text = _text(p)
            _, keep, move = plan(text, target)
            if not move:
                return {
                    "file": name,
                    "moved": 0,
                    "reason": "single entry or already under target; entries are never split",
                }
            ...
            if apply:
                ...
                indexed = index_entries(
                    c, name, move, dt.datetime.now(dt.timezone.utc).isoformat()
                )
                ...
                backup = (
                    dest.parent / "originals" / (name + "." + uuid.uuid4().hex + ".bak")
                )
                atomic_write(backup, text)
                if _text(p) != text:
                    raise ValueError(
                        "Memory changed before archive commit; original saved"
                    )
                tx = {
                    "before": text,
                    "after": after,
                    "archive_before": old_archive,
                    "archive_after": old_archive + moved,
                    "result": result,
                }
...
def maintain(c, lock_timeout=30):
    ...
    for row in status(c):
        if (
            row["over_warn"]
            or archive_for(c, row["file"]).with_suffix(".pending.json").exists()
        ):
            results.append(
                archive(c, row["file"], apply=True, lock_timeout=lock_timeout)
            )
    return results
...
def main():
    ...
    p.add_argument(
        "action", choices=("status", "archive", "caps", "index", "backfill-index")
    )
    ...
    p.add_argument(
        ...
        dest="keep_chars",
        type=int,
        help="character target (--keep-bytes is a legacy alias)",
    )
    ...
    if a.action == "backfill-index":
        print(
            json.dumps(backfill_index(c, apply=a.apply), ensure_ascii=False, indent=2)
        )
        return
    if a.action == "index":
        rows = index(c)
    ...
    if a.action == "caps":
        ...
        if a.apply:
            plan_["written"] = write_caps(c, plan_["caps"])
        print(json.dumps(plan_, indent=2))
        return 0
    rows = status(c)
    if a.action == "status":
        for r in rows:
            print(
                f"{'!' if r['over_warn'] else ' '} {r['file']}: {r['chars']:,} / {r['cap']:,} characters ({r['fraction']:.0%})"
            )
        return 0
    targets = (
        [a.file]
        if a.file
            ...
            for r in rows
            if r["over_warn"]
            or archive_for(c, r["file"]).with_suffix(".pending.json").exists()
        ]
    )
    for name in targets:
        print(archive(c, name, keep_chars=a.keep_chars, apply=a.apply))
    if not targets:
        print("All memory files are below the warning threshold; nothing to archive.")
    if not a.apply:
        print("(dry run — pass --apply to move entries)")
    return 0
