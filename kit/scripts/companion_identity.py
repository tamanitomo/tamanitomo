...
def sections(text):
    ...
    for m in OPEN.finditer(text):
        name=m.group(1);locked=bool(m.group(2))
        end=text.find(CLOSE.format(name),m.end())
        if end<0:continue
        found[name]={'name':name,'locked':locked,'start':m.start(),
...
def retrofit(text):
    ...
    for section in sorted(found.values(),key=lambda s:s['start'],reverse=True):
        if not section['body'].strip():
            text=text[:section['start']]+text[section['body_start']:section['body_end']]+text[section['end']:]
    found=sections(text)
    aliases={'core identity':'core','physical description':'appearance','appearance':'appearance',
             'visual identity':'appearance','voice and tone':'voice',
             'romantic and affectionate boundaries':'boundary',
             'visual presence':'visual','photorealistic mode (when requested)':'photorealistic',
             'emotional framing':'emotion','conversational behavior':'conversation',
             'continuity and memory style':'memory','daily life realism':'daily-life',
             'environment and presence':'environment','support style':'support',
             'conflict and repair':'conflict','humor and playfulness':'humor',
             'flirtation style':'flirtation','personal details and preferences':'likes',
             'accuracy and honesty':'honesty','operational priorities':'priorities',
             'essence':'essence','flaws and friction':'flaws'}
    edits=[]
    ...
    for index,heading in enumerate(headings):
        ...
        if title.startswith('relationship with '):name='relationship'
        if not name or name in claimed:continue
        end=headings[index+1].start() if index+1<len(headings) else len(text)
        if any(s['start']<end and s['end']>heading.start() for s in found.values()):continue
        ...
        if self_marker>=0:end=self_marker
        locked=' LOCKED' if name in ('core','appearance','relationship','boundary','boundaries') else ''
        edits.extend([(heading.start(),'<!-- COMPANION-SECTION:'+name+locked+' -->\n'),
                      (end,'<!-- /COMPANION-SECTION:'+name+' -->\n')])
        claimed.add(name)
    for offset,addition in sorted(edits,reverse=True):text=text[:offset]+addition+text[offset:]
    return text
...
def listing(c):
    _,text=read(c)
    return [{'name':s['name'],'locked':s['locked'],'chars':len(s['body'])}
...
def replace(c,name,body,now=None,backups=None):
    ...
    after=path.read_text(encoding='utf-8');produced=sections(after)
...
def render_section(c,name,persona=None,style=None,interview=None):
    ...
    m=cr.mapping_for(c,persona or c.persona,style or c.image_style,interview=interview or {})
    rendered=cr.render_template('SOUL.md.tmpl',m)
    found=sections(rendered)
    if name not in found:raise ValueError(f'no section {name!r} in the template')
...
def guard(c,text):
    ...
    tripwires=[]
    if c.birthdate and c.birthdate in lowered:tripwires.append('birthdate')
    for phrase in ('my boundaries are','my boundary is','i am willing to','i am now willing'):
        if phrase in lowered:tripwires.append('boundaries')
    for phrase in ('my build is','my body is','i look like','my appearance is'):
        if phrase in lowered:tripwires.append('appearance')
    if f'my name is' in lowered and c.agent.lower() not in lowered:tripwires.append('name')
...
def main():
    ...
    s.add_parser('list')
    sh=s.add_parser('show');sh.add_argument('name')
    rp=s.add_parser('replace');rp.add_argument('name')
    rp.add_argument('--file',type=pathlib.Path,required=True,help='new text for that section')
    rr=s.add_parser('rerender');rr.add_argument('name')
    rr.add_argument('--apply',action='store_true')
    a=p.parse_args();c=cc.load(a.home)
    if a.cmd=='list':out={'sections':listing(c),'locked':locked_names(c)}
    elif a.cmd=='show':
        _,text=read(c);found=sections(text)
        if a.name not in found:raise ValueError(f'no section {a.name!r}; known: {sorted(found)}')
        out={'section':a.name,'locked':found[a.name]['locked'],'body':found[a.name]['body']}
    elif a.cmd=='replace':
        out=replace(c,a.name,a.file.read_text(encoding='utf-8'))
    else:
        body=render_section(c,a.name)
        out={'section':a.name,'body':body}
        if a.apply:out.update(replace(c,a.name,body))
