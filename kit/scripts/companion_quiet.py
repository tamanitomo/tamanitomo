...
def evidence(c, now, messages=None):
    ...
    for when in messages:
        ...
        if 0 < after_start <= EDGE_MINUTES:
            late.add(night)
        if 0 < before_end <= EDGE_MINUTES:
            early.add(night)
    return {
        "late_nights": len(late),
        "early_mornings": len(early),
        "start": start,
        "end": end,
...
def propose(c, now=None, messages=None):
    ...
    if not c.adaptive_quiet:
        return {
            "change": False,
            "reason": "adaptive quiet hours are switched off for this companion",
        }
    ...
    if facts["late_nights"] >= MIN_NIGHTS:
        ...
        notes.append(
            f"{c.human} wrote after {_hhmm(start)} on {facts['late_nights']} nights"
        )
    if facts["early_mornings"] >= MIN_NIGHTS:
        new_end = (end - STEP_MINUTES) % (24 * 60)
        notes.append(
            f"{c.human} wrote before {_hhmm(end)} on {facts['early_mornings']} mornings"
        )
    if (new_start, new_end) == (start, end):
        return {
            "change": False,
            "reason": "no consistent pattern in the last fortnight",
            "evidence": facts,
        }
    ...
    return {
        "change": True,
        "quiet_start": _hhmm(new_start),
        "quiet_end": _hhmm(new_end),
...
def apply(c, now=None, messages=None):
    ...
    c.quiet_start = plan["quiet_start"]
    c.quiet_end = plan["quiet_end"]
    c.save()
...
def main():
    ...
    p.add_argument(
        "--apply",
        action="store_true",
        help="without this, only reports what it would do",
    )
    a = p.parse_args()
    c = cc.load(a.home)
    result = apply(c) if a.apply else propose(c)
    if result.get("change") or result.get("applied"):
