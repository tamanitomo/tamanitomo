...
def state(c):
    ...
    return {
        ...
        'stale': bool(isinstance(stored, str) and stored.strip()
...
def tidy(text):
    ...
    for term in media.split_terms(' '.join(str(text or '').split())):
        ...
        if key and key not in known:
            known.add(key); seen.append(term)
    out = ', '.join(seen)
    if len(out) > MAX_CHARS:
        kept = []
        for term in seen:
            if len(', '.join(kept + [term])) > MAX_CHARS: break
            kept.append(term)
        out = ', '.join(kept)
    return out
...
def propose(c, model='', provider='', timeout=TIMEOUT):
    ...
    argv = list(hermes_command('chat', '--oneshot', '-Q', '--ignore-rules', '--max-turns', '1'))
    if model: argv += ['--model', model]
    if provider: argv += ['--provider', provider]
    handle = tempfile.NamedTemporaryFile('w', suffix='.txt', encoding='utf-8', delete=False)
    try:
        handle.write(PROMPT.format(appearance=appearance, limit=MAX_CHARS)); handle.close()
    ...
    if result.returncode:
        ...
        raise ValueError('Hermes refused the call: ' + (' '.join(lines[-3:])[:300] if lines else 'no output'))
    proposed = tidy(_extract(result.stdout).get('identity', ''))
    if not proposed:
        raise ValueError('the model returned nothing usable; nothing was written')
    return {'identity': proposed, 'appearance': appearance,
            'source_fingerprint': fingerprint(appearance), 'written': False,
...
def save(c, text, source=None):
    ...
    text = tidy(text)
    if not text:
        raise ValueError('An empty block would send nothing; clear it with `follow` instead')
    data = media.load(c)
    data['identity_override'] = text
    data['identity_source'] = fingerprint(appearance_text(c) if source is None else source)
...
def main():
    ...
    d = s.add_parser('propose', help='ask a model to derive it from the appearance section')
    d.add_argument('--model', default=''); d.add_argument('--provider', default='')
    d.add_argument('--apply', action='store_true', help='keep it without reading it first')
    ...
    if a.cmd == 'show': out = state(c)
    elif a.cmd == 'set': out = save(c, a.text)
    elif a.cmd == 'follow': out = follow(c)
    else:
        out = propose(c, model=a.model, provider=a.provider)
        if a.apply: out = {**out, **save(c, out['identity']), 'written': True}
