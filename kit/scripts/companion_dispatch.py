...
Phase 1B C2 (PHASE1B_DESIGN.md section 7). One dispatcher run per life directory
holds `<life>/.dispatch.run.lock` (non-blocking; busy means another run is going,
and this one does nothing). Each send is an ATTEMPT (`<run_id>:<n>`) that moves the
entry through guarded phases, every one a durable outbox row written only if the
entry is still exactly where this attempt left it:

  claim `dispatching` -> reservation intent `reserving_slot` -> charge the daily slot
  (the charge row carries the attempt) -> `slot_reserved` -> `sending` -> hermes send
  -> exactly one outcome: sent | failed | unknown | withheld

A run first resolves attempts a dead run left behind (recover(), table 7.3). The
rule that matters most: anything that reached `sending` becomes `unknown` and is
NEVER sent again. Outcomes follow 7.5: only a confirmed success is `sent`; any
error Hermes reports, a timeout, a crash or unreadable output is `unknown`.

Not solved here, and stated: a PRE-1B dispatcher that is still running with an old
snapshot takes neither the run lock nor the claim guard. Running old and new
dispatchers on one life directory at the same time is unsupported (7.8).
"""
from __future__ import annotations
import argparse, contextlib, datetime as dt, json, os, pathlib, secrets, subprocess, sys
...
def _hook(name):
    """Test seam at each crash boundary (PHASE1B_DESIGN 7.3). A no-op in production; a
    test replaces it to kill the process there. No environment variable is read (U6)."""

def run_lock_path(c):return pathlib.Path(c.life)/RUN_LOCK
...
def deliver(c,entry):
    ...
    if entry.get('media_path'):
        if not pathlib.Path(entry['media_path']).exists():
            return {'outcome':'failed','detail':'the file to send is gone','not_dispatched':True}
        if entry.get('content')=='image':
            from companion_media_review import ensure_delivery
            try:ensure_delivery(c,entry['media_path'],body)
            except Exception:
                return {'outcome':'failed','not_dispatched':True,
...
def classify(target,returncode=None,stdout='',stderr='',error=None):
    """PHASE1B_DESIGN 7.5 and the 7.6 delivery record. Only a confirmed success is `sent`;
    ...
    record={'platform':str(target).split(':',1)[0],'requested_target':target,'recipient_resolved':None,
            'message_id':None,'id_scope':'last_chunk','mirrored':None}
    if error is not None:
        code='timeout' if isinstance(error,subprocess.TimeoutExpired) else 'os_error'
        return {'outcome':'unknown','error_code':code,'delivery':record,
                'detail':f'delivery not confirmed ({code}: {str(error)[:160]}); the slot is kept and it is not retried'}
    try:payload=json.loads(stdout or '')
    ...
    if isinstance(payload,dict):
        for key in ('chat_id','recipient'):
            if isinstance(payload.get(key),(str,int)) and not isinstance(payload.get(key),bool):
                record['recipient_resolved']=payload[key];break
        if isinstance(payload.get('message_id'),(str,int)) and not isinstance(payload.get('message_id'),bool):
            record['message_id']=payload['message_id']
        if isinstance(payload.get('mirrored'),bool):record['mirrored']=payload['mirrored']
    if not isinstance(payload,dict):
        return {'outcome':'unknown','error_code':'no_json','delivery':record,
    ...
    if payload.get('success') is True and payload.get('skipped') and not error_text:
        return {'outcome':'withheld','delivery':record,'detail':f"Hermes skipped it: {str(payload.get('reason') or payload.get('skipped'))[:200]}"}
    ...
    if error_text and str(error_text) in PRE_PLATFORM_ERRORS:
        return {'outcome':'failed','error_code':'pre_platform','delivery':record,'not_dispatched':True,
    ...
    return {'outcome':'unknown','error_code':'hermes_error' if error_text else 'unconfirmed','delivery':record,
...
def _invoke(c,body,target):
    ...
    return subprocess.run(hermes_command('send','--to',target,'--json'),
        input=body,capture_output=True,text=True,encoding='utf-8',timeout=120,env=_env(c))

def hermes_send_result(c,body,target='telegram'):
    """One native Hermes send with this profile's own credentials, classified."""
    try:proc=_invoke(c,body,target)
    except (OSError,subprocess.SubprocessError) as exc:return classify(target,error=exc)
...
def _outcome(result):
    ...
    if isinstance(result,dict) and result.get('outcome') in ('sent','failed','unknown','withheld'):
        return result
    if isinstance(result,tuple) and len(result)==2:
        ok,detail=result
        if ok:return {'outcome':'sent','detail':str(detail),'delivery':{'message_id':None,'id_missing':True,
                                                                          'id_scope':'last_chunk','recipient_resolved':None,'mirrored':None}}
        return {'outcome':'unknown','detail':str(detail),'error_code':'unconfirmed'}
    return {'outcome':'unknown','detail':'the delivery result could not be read','error_code':'unreadable_result'}
...
def recover(c,run_id,now):
    ...
    for entry in outbox.fold(c):
        ...
        if status not in outbox.PHASES or not attempt or entry.get('run_id')==run_id:continue
        ident=entry['id']
        def move(to,**extra):
            r=outbox.transition(c,ident,to,attempt,run_id,status,now,**extra)
            if r.get('written',True) is not False:out.append((ident,to if to!='failed' else 'failed_not_dispatched'))
        if status=='dispatching':
            move('queued',release_reason='abandoned',detail='a dispatcher stopped before reserving a slot; nothing was charged or sent')
        elif status=='reserving_slot':
            try:n=outreach.charges_for(c,attempt)
            except (OSError,ValueError) as exc:
                move('reservation_unresolved',detail='definitely not dispatched; whether a daily slot was used is unknown '
                     f'(the outreach ledger could not be read: {str(exc)[:120]}). Not requeued, not refunded.')
                continue
            if n:move('failed',not_dispatched=True,detail='a dispatcher stopped after charging a slot and before sending; not sent, the slot stays used')
            else:move('queued',release_reason='abandoned',detail='a dispatcher stopped before charging a slot; nothing was charged or sent')
        elif status=='slot_reserved':
            move('failed',not_dispatched=True,detail='a dispatcher stopped after reserving a slot and before sending; not sent, the slot stays used')
        elif status=='sending':
            move('unknown',error_code='dispatcher_stopped',
...
def run(c,now=None,send=True):
    ...
    with contextlib.ExitStack() as stack:
        try:stack.enter_context(file_lock(run_lock_path(c),timeout=0))
        except TimeoutError:
            return {'at':now.isoformat(timespec='minutes'),'handled':[],'busy':True,
                    'reason':'another dispatcher is running','still_waiting':len(outbox.waiting(c,now))}
...
def _run_locked(c,now,send):
    ...
    for n,entry in enumerate(outbox.waiting(c,now)):
        ...
        if call['action']=='hold':
            handled.append({'id':ident,**call});continue
        if call['action'] in ('expire','withhold'):
            ok,why=step(ident,'expired' if call['action']=='expire' else 'withheld',attempt,'queued',detail=call['reason'])
            handled.append({'id':ident,**call} if ok else {'id':ident,'action':'skipped','reason':why});continue
        # A dry run decides and stops: claiming first used up a real daily slot for a
        # message it never sent.
        if not send:
            handled.append({'id':ident,'action':'would-send','reason':call['reason']});break
        ok,why=step(ident,'dispatching',attempt,'queued')          # 7.2 step 1: the claim
        if not ok:
            handled.append({'id':ident,'action':'skipped','reason':f'not claimed: {why}'});continue
        _hook('after_claim')
        ...
        if entry.get('content')=='image' and entry.get('media_path'):
            ...
            if held:
                ...
                handled.append({'id':ident,'action':'withhold','reason':held});continue
        ok,why=step(ident,'reserving_slot',attempt,'dispatching')  # 3a: intent BEFORE any charge
        if not ok:
            handled.append({'id':ident,'action':'stopped','reason':why});break
        ...
        slot=outreach.claim(c,entry.get('reason') or 'outbox',now=now,urgent=call.get('urgent',False),attempt=attempt)
        if not slot['allowed']:
            step(ident,'queued',attempt,'reserving_slot',detail=slot['reason'],
                 release_reason='ledger_unreadable' if slot.get('storage_error') else 'cap')
            handled.append({'id':ident,'action':'hold','reason':slot['reason']});break
        _hook('after_charge')
        ok,why=step(ident,'slot_reserved',attempt,'reserving_slot')
        if not ok:
            handled.append({'id':ident,'action':'stopped','reason':why});break
        _hook('after_slot_marker')
        ok,why=step(ident,'sending',attempt,'slot_reserved')       # 4: marker right before the send
        if not ok:
            handled.append({'id':ident,'action':'stopped','reason':why});break
        _hook('after_sending_marker')
        try:result=_outcome(deliver(c,entry))
        except Exception as exc:
            result={'outcome':'unknown','error_code':'dispatcher_error',
                    'detail':f'the delivery step raised {type(exc).__name__}; delivery is not known and it is not retried'}
        _hook('after_delivery')
        extra={k:result[k] for k in ('delivery','not_dispatched','error_code') if k in result}
        step(ident,result['outcome'],attempt,'sending',detail=result.get('detail',''),**extra)   # 5
        handled.append({'id':ident,'action':result['outcome'],'reason':result.get('detail','')})
        break
    return {'at':now.isoformat(timespec='minutes'),'run_id':run_id,'recovered':recovered,'handled':handled,
...
def main():
    ...
    if a.json:print(json.dumps(result,ensure_ascii=False,indent=2))
    elif result.get('busy'):pass                  # another run is going: a normal, silent tick
    else:
        ...
        bad=[h for h in result['handled'] if h['action'] in ('failed','unknown','expire')]
        for item in bad:print(f"{item['action']}: {item['reason']}")
        for ident,resolution in result.get('recovered',[]):
            if resolution in ('unknown','reservation_unresolved','failed_not_dispatched'):
                print(f'{resolution}: {ident} (left by a dispatcher that stopped mid-way)')
    return 0
