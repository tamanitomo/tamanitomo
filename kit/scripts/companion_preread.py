...
def preread(c, now=None):
    ...
    out = [
        "[Pre-read — assembled from your own records before this run. Everything below is already",
    ...
    if recent["text"]:
        out.append(
            "[Recent autonomy log tail, already read. Older entries remain in the file; "
    ...
    if scene:
        out.append(
            "[For a presence update, these are the current record token and worn wardrobe IDs; "
    ...
    if scene and not scene["state"].get("confirmed", True):
        ...
        out.append(
            f"[The present above was carried forward by a script, not confirmed by anyone since "
    ...
    if night.get("asleep"):
        ...
        out.append(
            ...
            + (
                " — the length you declared at wind-down."
                if night["source"] == "declared"
        ...
        out.append(
            f"[Quiet hours ({c.quiet_start}–{c.quiet_end}) are in force, but you are AWAKE."
    ...
    out.append(
        "[Routine anchors and interests — choose and record what happens; no automatic attendance]\n"
        + json.dumps(
            companion_life.routine(c.life, now.astimezone(_tz(c)), c.agent),
            ensure_ascii=False,
        )
    )
...
def day_ideas(c, now):
    ...
    try:
        ...
        cmd = terminal_python_command(
            pathlib.Path(__file__).with_name("companion_life.py"), "--home", c.home
        )
    ...
    for idea in ideas:
        bits = [
            x
            for x in (
                f"~{idea.get('hours')}h" if idea.get("hours") else "",
                idea.get("setting"),
                idea.get("social"),
            )
            if x
...
def fingerprint(c, now=None):
    ...
    deadlines = [
        (
            item["id"],
            now
            >= companion_day.timestamp(item["starts_at"])
            - dt.timedelta(minutes=item["buffer_minutes"]),
            now >= companion_day.timestamp(item["ends_at"]),
        )
...
def main():
    ...
    p.add_argument(
        "mode", choices=["preread", "fingerprint"], nargs="?", default="preread"
    )
    a = p.parse_args()
    c = cc.load(a.home)
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"[Pre-read unavailable: {e}. Read your state with companion_presence.py show.]"
        )
