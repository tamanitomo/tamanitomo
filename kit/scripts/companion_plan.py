...
def folder(c):
    return pathlib.Path(c.life) / FOLDER
...
def clean_item(raw):
    ...
    if not what:
        raise ValueError("An item needs a `what`")
    ...
    if a is None or b is None:
        raise ValueError(f"{what!r} needs a start and end as HH:MM")
    if b <= a:
        raise ValueError(f"{what!r} ends before it starts")
    item = {
        "id": str(raw.get("id") or "").strip() or _item_id(raw),
        "start": start,
        "end": end,
        "what": what[:160],
        "where": str(raw.get("where") or raw.get("setting") or "")[:160],
        "kind": kind,
        "ref": str(raw.get("ref") or "")[:80],
        "status": status,
...
def overlaps(one, other):
    return _minutes(one["start"]) < _minutes(other["end"]) and _minutes(
        other["start"]
    ) < _minutes(one["end"])
...
def conflicts(items, candidate, ignore=()):
    ...
    return [
        ...
        for x in items
        if x["status"] == "planned"
        and x["id"] != candidate["id"]
        and x["id"] not in ignore
        and overlaps(x, candidate)
    ]
...
def read_for(root, day):
    ...
    data["items"] = sorted(
        (x for x in data["items"] if isinstance(x, dict) and x.get("what")),
...
def _write(c, day, plan, note=None, now=None):
    ...
    plan["items"] = sorted(
        (clean_item(x) for x in plan.get("items", [])),
...
def settle(c, day, intent, theme="custom", items=(), author="companion", now=None):
    ...
    if previous:
        plan["history"] = previous.get("history", [])
    plan.update(
        intent=str(intent or "").strip()[:300],
        theme=str(theme or "custom"),
        items=list(items),
        settled_at=now.isoformat(),
    )
    if not plan["intent"]:
        raise ValueError("A day needs an intent written as a sentence")
...
def add(c, day, item, displaces=None, now=None):
    ...
    with file_lock(folder(c) / ".lock"):
        ...
        clash = conflicts(
            plan["items"], candidate, ignore={displaces} if displaces else ()
        )
        if clash:
            raise ValueError(
                f"{candidate['what']!r} overlaps {clash[0]['what']!r} "
        ...
        if displaces:
            ...
            if not found:
                raise ValueError(f"Nothing here called {displaces!r} to displace")
            found[0]["status"] = "moved"
            found[0]["reason"] = (
                candidate.get("reason") or f"displaced by {candidate['what'][:80]}"
            )[:300]
            note = f"{candidate['what'][:60]} displaced {found[0]['what'][:60]}"
        plan["items"] = [x for x in plan["items"] if x["id"] != candidate["id"]] + [
            candidate
        ]
...
def amend(c, day, item_id, status=None, reason=None, now=None):
    ...
    with file_lock(folder(c) / ".lock"):
        plan = read(c, day)
        if not plan:
            raise ValueError("No plan for that day")
        found = [x for x in plan["items"] if x["id"] == item_id]
        if not found:
            raise ValueError(f"Nothing here called {item_id!r}")
        if status:
            if status not in STATUSES:
                raise ValueError(f"Unknown status {status!r}")
            found[0]["status"] = status
        if reason is not None:
            found[0]["reason"] = str(reason)[:300]
        return _write(
            c, day, plan, f"{found[0]['what'][:60]} -> {found[0]['status']}", now
        )
    ...
    with file_lock(folder(c) / ".lock"):
        ...
        for item in plan["items"]:
            ...
            if item.get("status") == "planned" and end is not None and end <= cutoff:
                ...
                item["reason"] = (
                    item.get("reason")
                    or "its time passed with nothing recorded against it"
                )[:300]
                stale.append(item)
        if not stale:
            return []
        note = (
            "past their window, unrecorded: "
            + ", ".join(x["what"][:40] for x in stale[:4])
        )[:300]
...
def displacements(c, day=None, limit=4):
    ...
    out = []
    for item in plan["items"]:
        if item.get("status") not in ("moved", "dropped"):
            continue
        out.append(
            {
                "what": item.get("what", ""),
...
def render_displacements(rows):
    ...
    for row in rows:
        ...
        lines.append(line + ".")
    return (
        "[How today actually went — your own changes, already recorded. "
...
def timeline(c, day, routine=None):
    ...
    for row in routine or []:
        ...
        if a is None or b is None:
            continue
        anchors.append(
            {**row, "free": not any(a < end and start < b for start, end in busy)}
        )
    return {
        "date": day.isoformat(),
        "settled": bool(plan and plan.get("settled_at")),
        "intent": (plan or {}).get("intent", ""),
        "theme": (plan or {}).get("theme", "custom"),
...
def migrate(c, apply=False, now=None):
    ...
    for row in state.get("commitments") or []:
        ...
        if not end:
                ...
                dt.datetime.combine(day, dt.time.fromisoformat(start))
    ...
    for day in sorted(days):
        ...
        for raw in body["items"]:
            try:
                items.append(clean_item(raw))
            except ValueError as exc:
                report["skipped"].append({"day": day.isoformat(), "why": str(exc)})
                continue
        ...
        if existing and existing.get("settled_at"):
            report["skipped"].append(
                {
                    "day": day.isoformat(),
                    "why": "already has a settled plan; left alone",
                }
            )
            continue
        report["days"].append(
        ...
        if apply:
            plan_out = empty(day)
            plan_out.update(
                intent=body["intent"] or "a day carried over from the older plan files",
                theme=body["theme"],
                items=kept,
            ...
            with file_lock(folder(c) / ".lock"):
                _write(
                    ...
                    day,
                    plan_out,
                    "migrated from tomorrow.json and presence commitments",
                )
            report["written"] += 1
    return report
...
def sync_commitments(c, commitments, tz=None, now=None):
    ...
    for row in commitments or []:
        ...
        wanted = (
            "planned"
            if status == "planned"
            else ("done" if status == "completed" else "dropped")
        )
        try:
        ...
        with file_lock(folder(c) / ".lock"):
            ...
            existing = [
                x
                for x in plan["items"]
                if x["kind"] == "commitment" and ref and x["ref"] == ref
            ]
            if existing:
                existing[0].update(
                ...
                if promises and wanted == "planned":
                    ...
                    for other in clash:
                        other["status"] = "moved"
                        other["reason"] = (
                            f"displaced by {title[:80]}, which was promised"
                        )
...
def main():
    ...
    c = cc.load(a.home)
    if a.action == "migrate":
        print(json.dumps(migrate(c, apply=a.apply), ensure_ascii=False, indent=2))
        return
    if a.action == "reconcile":
        print(json.dumps(reconcile(c, a.day), ensure_ascii=False, indent=2))
        return
    print(
        json.dumps(timeline(c, a.day or dt.date.today()), ensure_ascii=False, indent=2)
    )
