...
def targets(c):
    ...
    return [
        ("staging inputs", c.life / ".inputs", DEFAULT_KEEP_DAYS),
...
def main():
    ...
    p.add_argument(
        "--apply", action="store_true", help="without this, only reports what would go"
    )
    a = p.parse_args()
    print(
        json.dumps(
            prune(cc.load(a.home), keep_days=a.keep_days, apply=a.apply), indent=2
        )
    )
