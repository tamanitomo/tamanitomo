...
TARGETS = {
    ...
    "monthly": "reflections/monthly.md",
    "autonomy": "continuity/Autonomy.md",
}


def path_for(c, target):
    if target not in TARGETS:
        raise ValueError("unknown journal target")
    path = (c.soul_dir / TARGETS[target]).resolve()
    if not path.is_relative_to(c.soul_dir.resolve()):
        raise ValueError("journal target escapes the companion soul directory")
    return path


def tail(c, target, limit=6000):
    if not 500 <= limit <= 16000:
        raise ValueError("limit must be 500–16000 characters")
    ...
    return {
        ...
        "omitted_chars": max(0, len(body) - len(text)),
        "text": text,
        "note": "Older text is retained in the file; this is only its tail.",
    }
...
def append(c, target, data):
    ...
    if not isinstance(text, str) or not text.strip() or len(text) > 8000:
        raise ValueError("text must be 1–8000 characters")
    text = text.strip()
    if re.search(r"^#{1,2}\s", text, re.M):
        raise ValueError("provide body text only; the helper writes the date heading")
    if "<!-- companion-journal:" in text:
        raise ValueError("journal markers are reserved")
    ...
    if not isinstance(ident, str) or not re.fullmatch(r"[A-Za-z0-9_.-]{1,100}", ident):
        raise ValueError(
            "autonomy requires a unique id of 1–100 letters, digits, dots, underscores or hyphens"
        )
    path = path_for(c, target)
    ...
    with file_lock(path.parent / ("." + path.name + ".lock")):
        ...
        if marker in old:
            ...
            if not saved:
                ...
                if marker + "\n" + end not in old:
                    raise ValueError(
                        "this date has an empty journal entry that is not a clean stub; "
            ...
            if saved != text:
                raise ValueError(
                    "entry id already exists with different text; original retained"
                )
            return {
                "written": False,
                "id": ident,
        ...
        if target != "autonomy" and re.search(
            r"^## " + re.escape(day) + r"(?:\s|$)", old, re.M
        ):
            return {
                "written": False,
                "id": ident,
                "path": str(path),
                "reason": "this date already has an entry; existing journal retained",
            }
...
def main():
    ...
    for name in ("tail", "append"):
        ...
        q.add_argument("--target", required=True, choices=TARGETS)
        if name == "tail":
            q.add_argument("--limit", type=int, default=6000)
        else:
            q.add_argument("--file", required=True, type=pathlib.Path)
    a = p.parse_args()
    c = cc.load(a.home)
    out = (
        tail(c, a.target, a.limit)
        if a.cmd == "tail"
        else append(c, a.target, json.loads(a.file.read_text(encoding="utf-8")))
    )
