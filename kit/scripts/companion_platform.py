...
def profile_name(value):
    ...
    if not re.fullmatch(r"[a-z0-9][a-z0-9_-]{0,47}", name) or name in WINDOWS_RESERVED:
        raise ValueError(
            "Profile names must be 1–48 letters, digits, hyphens or underscores, start with a letter/digit, and not be reserved."
        )
    return name
...
def profile_path(root, name):
    ...
    if base.is_symlink() or target.is_symlink() or target.resolve().parent != base:
        raise ValueError(
            "Refusing a profile path through a symlink or outside profiles."
        )
    return target
...
def command(argv):
    ...
    return (
        subprocess.list2cmdline([str(x) for x in argv])
        if os.name == "nt"
        else shlex.join([str(x) for x in argv])
    )
...
def hermes_command(*args):
    ...
    if not override:
        ...
        if config.is_file():
            override = json.dumps(
                json.loads(config.read_text(encoding="utf-8"))["command"]
            )
    ...
    if (
        ...
        raise ValueError(
            "COMPANION_HERMES_COMMAND must be a nonempty JSON array of arguments"
        )
...
def default_home():
    ...
    if os.environ.get("HERMES_HOME"):
        return pathlib.Path(os.environ["HERMES_HOME"]).expanduser()
    if os.name == "nt":
        return (
            pathlib.Path(
                os.environ.get(
...
def is_terminal(stream=None):
    ...
    if not (s and hasattr(s, "isatty") and s.isatty()):
        return False
    if os.name == "nt":
        try:
            import msvcrt
