#!/usr/bin/env python3
"""Structured reflection with deterministic periods and quote-backed writes.

The model selects and authors content; code owns journal dates, file paths,
human attribution and the evidence written to human ledgers. Evidence is an
exact trusted human quote selected by ID. For facts.jsonl, the model's readable
statement remains separate from that verbatim citation and its source. Keeping
the quote makes a claim auditable and correctable; vector similarity or a
plausible generated recollection cannot establish what the human actually said.

A fact statement is screened for obvious mismatches with its quote and held for
review when one is found. Accepted wording is marked as a model paraphrase,
not certified as equivalent to the source. The model cannot fabricate the
stored citation, choose arbitrary shell commands, or redirect writes elsewhere.
"""
...
def journal_period(c, target, start, end, limit=10000):
    ...
    last = (
        ...
        if end.time().replace(tzinfo=None) != dt.time(0)
        else end.date() - dt.timedelta(days=1)
    )
    selected = [
        entry
        for entry in split_entries(body)[1]
        if (day := entry_date(entry)) is not None and start.date() <= day <= last
    ]
    text = "\n".join(selected)
    return {
        "path": str(path),
        "entries_in_period": len(selected),
        "text": text[-limit:],
        "omitted_chars": max(0, len(text) - limit),
        "note": "Only dated entries inside the requested period are included.",
    }
...
def period(kind, now, flag=None):
    ...
    if kind == "daily":
        return (
            ...
            midnight,
            (midnight - dt.timedelta(days=1)).date().isoformat(),
        )
    if kind == "weekly":
        return midnight - dt.timedelta(days=6), now, now.date().isoformat()
    if kind == "monthly":
    ...
    start = (
        dt.datetime.fromisoformat(flag["last_reflected"])
        if flag.get("last_reflected")
...
def messages(
    c, start, end, human_id, limit_chars=24000, after_id=0, end_inclusive=True
):
    path = c.home / "state.db"
    if not path.exists():
        return [], False, end.isoformat(), 0
    if not human_id:
        return [], False, end.isoformat(), 0
    ...
    for row in rows:
        ...
        item = dict(row)
        item["id"] = str(item["id"])
        if len(item["content"]) + 150 > limit_chars:
            raise ValueError(
                "A conversation message exceeds the local reflection batch budget; pending evidence retained for review"
            )
        size = len(item["content"]) + 150
        if out and (used + size > limit_chars or len(out) >= 100):
            more = True
            break
        out.append(item)
        used += size
    through = (
        dt.datetime.fromtimestamp(out[-1]["timestamp"], end.tzinfo).isoformat()
        if more
        else end.isoformat()
    )
...
def trusted_messages(
    ...
    if human_id and str(human_id) not in binding.telegram:
        raise ValueError(
            "--human-user-id is not a Telegram identity in the chat owner binding; "
    ...
    def read(view):
        ...
        for record in view.forward(
            start.timestamp(), after_id, end.timestamp(), end_inclusive
        ):
            item = {
                "id": record.source_message,
                "session_id": record.source_session,
                "role": "user" if record.speaker == "owner" else "assistant",
                "content": record.content,
                "timestamp": record.occurred_at,
                "channel": record.source_kind,
            }
            size = len(item["content"]) + 150
            if size > limit_chars:
                raise ValueError(
    ...
    try:
        out, more, view = sources.owner_evidence(c, read, binding)
    except sources.SourceUnavailable as exc:
        raise ValueError(
            "The owner conversation cannot be read right now; nothing was reflected: "
            + str(exc)
        ) from exc
    ...
    for row in rows:
        ...
        for piece in pieces:
            ...
            while piece:
                ...
                quote = piece[:end].strip()
                piece = piece[end:].lstrip()
                if quote:
                    result[f"{row['id']}:{index}"] = {**row, "content": quote}
                    index += 1
    return result
...
def context(c, kind, start, end, day, rows):
    out = {
        ...
        "journal_date": day,
        "real_conversation_messages": rows,
        "existing_preferences": slf.summary(c)["preferences"],
    ...
    if kind == "daily":
        ...
        offset = 0
        while True:
            page = life.history_digest(c.life, day, offset)
            digest.extend(page["episodes"])
            if page["next_offset"] is None:
                break
            offset = page["next_offset"]
        # Ordinary days fit. Disclose exceptional oversize records; do not invent the omitted hours.
        out["imagined_episode_excerpts"] = digest[:200]
        out["omitted_episodes"] = max(0, len(digest) - 200)
        ...
        out["previous_reflections"] = journal_period(
            c, "weekly" if kind == "monthly" else kind, start, end, 8000
        )
...
def schema(kind, sources, question_ids):
    ...
    fields = {
        "reflection": (
            text(4000) if kind != "checkin" else {"type": "string", "enum": [""]}
        ),
        "preferences": arr(
            obj(
                {
                    "subject": text(120),
                    "text": text(600),
                    "valence": {"type": "string", "enum": list(slf.VALENCE)},
                }
            ),
            4,
        ),
        ...
        "standing": arr(obj(evidence), allowed),
        "moments": arr(obj(evidence), allowed),
        "answers": arr(
            obj(
                {
                    **evidence,
                    "question_id": {
                        "type": "string",
                        "enum": question_ids or ["no-question"],
                    },
                }
            ),
            4 if sources and question_ids else 0,
        ),
...
def request_plan(
    c, kind, data, sources, base_url, model, slot, allow_remote=False, api_key_env=""
):
    import companion_worker_model as worker

    companion_endpoint.verify(base_url, allow_remote, "Reflection")
    qs = [
        q["id"] for q in data["existing_questions"] if q["status"] in ("asked", "open")
    ]
    instructions = (
        c.soul.read_text(encoding="utf-8")
    ...
    payload = {
        ...
        "response_format": {
            ...
            "json_schema": {
                "name": "reflection",
                "strict": True,
                "schema": schema(kind, sources, qs),
            },
        },
    }
    route = {"provider": "", "model": model, "base_url": base_url, "direct": True}
    reply = worker.complete(
        c, payload, route, api_key_env, allow_remote, timeout=300, what="Reflection"
    )
...
def _transcript_wrapper(statement, human):
    ...
    who = r"the (?:human|user)" + (
        "|" + re.escape(human.strip()) if (human or "").strip() else ""
    )
    return bool(
        re.match(
            r"^\W*(?:" + who + r")\s+(?:said|mentioned|told (?:me|you|us))\b",
            statement.strip(),
            re.I,
        )
    )
...
def question_concern(question, human):
    ...
    of a reflection."""
    q = question.strip()
    if re.fullmatch(r"q-[a-f0-9]+", q):
        return ("omit", "an existing question ID, not a new question")
    if human is None:
        return None
    if re.search(r"\bthe (?:human|user)\b", q, re.I):
        return ("omit", 'calls the person "the human" or "the user"')
    name = (human or "").strip()
    if not name or name.lower() in ("the user", "the human"):
        return None
    named = re.compile(r"(?<!\w)" + re.escape(name) + r"(?!\w)")
    unaddressed = re.sub(
        r"^\W*" + re.escape(name) + r"\s*,|,\s*" + re.escape(name) + r"\W*$", "", q
    )
    if named.search(unaddressed) and not re.search(
        r"\b(?:you|your|yours|yourself)\b", q, re.I
    ):
...
def validate(
    plan, kind, sources, question_ids, human="", legacy=False, contract=PLAN_CONTRACT
):
    ...
    if not isinstance(plan, dict) or set(plan) != expected:
        raise ValueError("unexpected reflection fields")
    ...
    text(plan["reflection"], 4000, kind == "checkin")
    text(plan["soul_append"], 1000, True)
    if kind == "checkin" and plan["reflection"]:
        raise ValueError("checkin cannot write a journal")
    if kind not in ("weekly", "monthly") and plan["soul_append"]:
        raise ValueError(
            "soul_append must be empty for a "
            + kind
            + " reflection (only weekly/monthly may edit SOUL); drop it and keep the journal"
        )
    if any(line.startswith(("## ", "# ")) for line in plan["reflection"].splitlines()):
        raise ValueError("journal headings are supplied by code")
    for key, cap in [
        ("preferences", 4),
        ("questions", 2),
        ("facts", 6),
        ...
        if not isinstance(plan[key], list) or len(plan[key]) > cap:
            raise ValueError("invalid " + key + " list")
    for p in plan["preferences"]:
        if set(p) != {"subject", "text", "valence"} or p["valence"] not in slf.VALENCE:
    ...
    for q in plan["questions"]:
        ...
        if concern := question_concern(q, None if legacy else human):
            level, why = concern
            diagnostics["omitted" if level == "omit" else "warnings"].append(
                {"kind": "question", "text": q, "reason": why}
            )
        ...
        for p in plan[key]:
            allowed = {"quote_id"} | (
                ...
                if key == "facts"
                    ...
                    {"question_id"}
                    if key == "answers"
                    else {"title"} if key == "open_loops" else set()
                )
            )
            ...
            if key == "facts" and not legacy:
                ...
                if _transcript_wrapper(p["statement"], human):
                    if contract < 3:
                        raise ValueError(
                            'a fact statement is a proposition about the human, not a transcript ("X said: ...")'
                        )
            ...
            if key == "answers" and p["question_id"] not in question_ids:
...
def usable(plan, diagnostics):
    ...
    for d in diagnostics.get("held", ()):
        if d["kind"] == "fact":
            held.setdefault((d["quote_id"], d["statement"]), []).append(d["reason"])
    facts = [
        (
            {**f, "_hold": held[(f["quote_id"], f.get("statement"))]}
            if (f["quote_id"], f.get("statement")) in held
            else f
        )
        for f in plan["facts"]
    ]
    return {
        **plan,
        "questions": [q for q in plan["questions"] if q not in dropped],
        "facts": facts,
    }
...
def apply_plan(c, kind, day, plan, sources, now):
    ...
    for p in plan["preferences"]:
        ident = slf._mkid(
            "pref", p["valence"], p["subject"].lower(), now.date().isoformat()
        )
        if any(
            row.get("id") == ident for row in slf._read(c.life / "preferences.jsonl")
        ):
            save(
                "pref",
                {
                    "written": False,
                    "reason": "this subject and valence are already recorded today",
                },
            )
            continue
        save(
            "pref",
            slf.record_pref(
                c.life, p["text"], now, p["valence"], p["subject"], c.agent
            ),
        )
    for q in plan["questions"]:
        save("ask", slf.ask(c.life, q, now))
    for key in ("facts", "standing", "moments", "answers", "open_loops"):
        for p in plan[key]:
            s = sources[p["quote_id"]]
            when = dt.datetime.fromtimestamp(s["timestamp"], now.tzinfo).isoformat()
            source = f"session:{s['session_id']} message:{s['id']} {when}"
            quote = s["content"]
            evidence = f"{when}: {quote}"
            if key == "facts":
                ...
                out = notes.add_standing(
                    c, {"instruction": quote, "evidence": evidence, "scope": ""}, now
                )
            elif key == "moments":
                out = notes.add_moment(
                    c,
                    {
                        "moment": "note",
                        "text": f'{c.human} said: "{quote}"',
                        "happened_on": when[:10],
                    },
                    now,
                )
                ...
                current = next(
                    (q for q in slf.questions(c.life) if q["id"] == p["question_id"]),
                    {},
                )
                out = (
                    {"written": False, "reason": "answer already recorded"}
                    if current.get("status") == "answered"
                    and current.get("answer") == quote
                    else slf.resolve(c.life, p["question_id"], "answered", now, quote)
                )
            else:
                exists = any(
                    l["title"].lower() == p["title"].lower()
                    for l in loops.loops(c, None)
                )
                out = (
                    {"written": False, "reason": "this loop title already exists"}
                    if exists
                    else loops.add(
                        c,
                        {
                            "title": p["title"],
                            "detail": f"{c.human}: {quote} ({source})",
                            "gentle_use": f"Raise only when relevant; do not press {c.human} for a reply.",
                        },
                        now,
                    )
                )
    ...
    if plan["soul_append"]:
        ...
        save(
            "soul",
            (
                {"written": False, "reason": "insight already present"}
                if plan["soul_append"] in current
                else slf.soul_write(c, plan["soul_append"], "append", now)
            ),
        )
    if kind != "checkin":
        save(
            "journal",
            journal.append(c, kind, {"date": day, "text": plan["reflection"]}),
        )
    return results
...
def reflect(
    ...
    folder = c.life / "local-reflections"
    folder.mkdir(parents=True, exist_ok=True)
    with file_lock(folder / (kind + ".lock")):
        flag = checkin.read(c) if kind == "checkin" else None
        if kind == "checkin" and not flag.get("pending"):
            return {"status": "skipped", "reason": "no pending conversation"}
        start, end, day = period(kind, now, flag)
        if kind == "checkin" and not (c.home / "state.db").exists():
            raise ValueError("pending conversation database is missing")
        evidence = {"reader": "telegram"}
        if trusted:
            rows, more, through, through_id = trusted_messages(
                c,
                start,
                end,
                human_id,
                after_id=int((flag or {}).get("last_reflected_id", 0)),
                end_inclusive=kind == "checkin",
                report=evidence,
            )
        else:
            rows, more, through, through_id = messages(
                c,
                start,
                end,
                human_id,
            ...
            if kind == "checkin" and not human_id:
                raise ValueError(
                    "configure the trusted human user ID before reflecting conversations"
                )
        ...
        key = (
            ...
            + hashlib.sha256(
                (
                    start.isoformat() + end.isoformat() + through + str(through_id)
                ).encode()
            ).hexdigest()[:20]
        )
        ...
        budget_key = (
            ...
            + hashlib.sha256((watermark + "|" + str(after_id)).encode()).hexdigest()[
                :20
            ]
        )
        ...
        if saved and saved.get("complete"):
            return {
                "status": "skipped",
                "reason": "this reflection was already committed",
                "id": key,
            }
        data = context(c, kind, start, end, day, rows)
        data["more_conversation_messages"] = more
        data["evidence_quotes"] = [
            {"quote_id": key, "quote": value["content"]}
        ...
        if saved:
            ...
            recomputed = validate(
                plan, kind, sources, question_ids, c.human, contract < 2, contract
            )
            ...
            diagnostics = (
                {"omitted": [], "warnings": [], "held": [], **saved["diagnostics"]}
                if "diagnostics" in saved
                else recomputed
            )
        else:
            ...
            if attempts["count"] and attempts.get("last_at"):
                ...
                if now < ready:
                    return {
                        ...
                        "budget": budget_key,
                        "attempts": attempts["count"],
                        "retry_after": ready.isoformat(),
                        "reason": "the last attempt failed; waiting before asking again",
                    }
            # Counted before the request: a crash, timeout or kill mid-generation still spends it.
            attempts["count"] += 1
            attempts["last_at"] = now.isoformat()
            attempt = attempts["count"]
            attempts.setdefault("batches", []).append(
                {"attempt": attempt, "at": now.isoformat(), "plan_id": key}
            )
            atomic_write(
                attempts_path, json.dumps(attempts, ensure_ascii=False, indent=2)
            )
            plan = None
            try:
                plan, usage = (planner or request_plan)(
                    c,
                    kind,
                    data,
                    sources,
                ...
                if plan is not None:
                    atomic_write(
                        folder / f"{budget_key}.rejected-{attempt}.json",
                        json.dumps(
                            {"error": str(exc), "plan": plan},
                            ensure_ascii=False,
                            indent=2,
                        ),
                    )
        ...
        results = apply_plan(
            ...
            kind,
            day,
            usable(plan, diagnostics),
            sources,
            dt.datetime.fromisoformat(saved["authored_at"]),
        )
        if kind == "checkin":
            ...
            with file_lock(checkin.path_for(c).with_suffix(".json.lock")):
                ...
                state["last_reflected"] = through
                state["last_reflected_id"] = through_id
                if state.get("last_end") == flag.get("last_end") and not more:
                    state["pending"] = 0
                atomic_write(
                    checkin.path_for(c), json.dumps(state, ensure_ascii=False, indent=2)
                )
        saved.update(complete=True, results=results)
        atomic_write(path, json.dumps(saved, ensure_ascii=False, indent=2))
        ...
        return {
            ...
            "results": results,
            "usage": usage,
            "more_conversation_messages": more,
            "evidence": evidence,
...
def reset_attempts(c, budget_key, reason, now=None):
    ...
    if not re.fullmatch(r"[a-z]+-[\w-]+", budget_key or ""):
        raise ValueError("unknown attempt budget")
    reason = (reason or "").strip()
    if not reason:
        raise ValueError("a reason is required to reset an attempt budget")
    now = now or dt.datetime.now(ZoneInfo(c.timezone))
    folder = c.life / "local-reflections"
    path = folder / (budget_key + ".attempts.json")
    if not path.exists():
        raise ValueError("unknown attempt budget")
    kind = budget_key.split("-", 1)[0]
    with file_lock(folder / (kind + ".lock")):
        attempts = read_attempts(path)
        attempts.setdefault("resets", []).append(
            {
                "at": now.isoformat(),
                "reason": reason[:500],
                "spent": attempts["count"],
                "errors": attempts["errors"],
            }
        )
        attempts.update(count=0, errors=[])
        attempts.pop("last_at", None)
...
def main():
    ...
    p.add_argument(
        "--reset-attempts",
        metavar="BUDGET",
        help="give a held batch a fresh attempt budget (needs --reason)",
    )
    p.add_argument("--reason", default="")
    p.add_argument("--human-user-id", default="")
    ...
    if a.reset_attempts:
        print(
            json.dumps(
                reset_attempts(cc.load(a.home), a.reset_attempts, a.reason), indent=2
            )
        )
        return
    if not (a.base_url and a.model):
        p.error("--base-url and --model are required")
    print(
        json.dumps(
            reflect(
                cc.load(a.home),
