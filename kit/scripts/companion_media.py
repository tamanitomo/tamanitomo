...
def bare_term(term):
    ...
    if ":" in t:
        ...
        if head and tail.replace(".", "", 1).replace("-", "", 1).isdigit():
            t = head.strip()
...
def negative_prompt(preset, intimate=False):
    ...
    if not intimate:
        parts.append(str(preset.get("modesty_negative", "") or ""))
    seen = []
    known = set()
    for chunk in parts:
...
def adult_ready(c):
    ...
    return allowed, (
        [] if allowed else (blockers or ["Closeness has not reached that point."])
    )
...
def hermes_bridge(c, command, payload=None):
    ...
    run = subprocess.run(
        [
            str(python),
            str(Path(__file__).with_name("companion_image_provider.py")),
            command,
        ],
        ...
        capture_output=True,
        text=True,
        timeout=30 if command == "catalog" else 600,
    )
    try:
        reply = json.loads(
            next(
                ...
                if line.startswith("COMPANION_IMAGE=")
            )
        )
    except (StopIteration, ValueError):
        raise ValueError("Hermes image provider is unavailable; inspect its setup")
    if isinstance(reply, dict) and reply.get("error"):
        raise ValueError(str(reply["error"]))
    if run.returncode:
        raise ValueError("Hermes image provider failed")
    return reply
...
def discovered_presets(c):
    ...
    if not isinstance(rows, list):
        raise ValueError("Hermes returned an invalid image provider catalog")
    return [
        dict(
            id="hermes-" + r["id"],
            name="ChatGPT (OAuth)" if r["id"] == "openai-codex" else r["name"],
            provider="hermes",
            hermes_provider=r["id"],
            model=r.get("model", ""),
            category=(
                "realistic"
                if "chatgpt" in r["id"] or "openai" in r["id"]
                else "portrait"
            ),
            parts={},
            negative="",
        ...
        for r in rows
        if re.fullmatch("[a-z0-9][a-z0-9_-]{0,55}", r.get("id", ""))
    ]
...
def effective(c):
    ...
    if data.get("inherit") and c.profile:
        ...
        if own_source is not None:
            data["identity_source"] = own_source
        else:
...
def validate(data):
    if not isinstance(data, dict) or data.get("version") != 1:
        raise ValueError("Expected image preset format version 1")
    if data.get("identity_override") is not None and (
        not isinstance(data["identity_override"], str)
        or len(data["identity_override"]) > 20000
    ):
        raise ValueError("Identity prompt is too long")
    if "identity_source" in data and not isinstance(data["identity_source"], str):
        raise ValueError("Invalid identity source marker")
    if (
        not isinstance(data.get("quality_tags", ""), str)
        or len(data.get("quality_tags", "")) > 2000
    ):
        raise ValueError("Quality tags must be a short line of text")
    if not isinstance(data.get("inherit", False), bool):
        raise ValueError("Inheritance must be true or false")
    presets = data.get("presets", [])
    if not isinstance(presets, list) or len(presets) > 100:
        raise ValueError("At most 100 presets are supported")
    ids = set()
    for p in presets:
        ...
        if (
            ...
            or not re.fullmatch("[a-z0-9][a-z0-9_-]{0,63}", ident)
            or ident in ids
        ):
            raise ValueError("Preset IDs must be unique lowercase names")
        ids.add(ident)
        if not isinstance(p.get("name"), str) or not 1 <= len(p["name"]) <= 100:
            raise ValueError("Name each preset")
        if p.get("provider") not in ("comfyui", "openai", "hermes", "mistral"):
            raise ValueError(
                "Choose ComfyUI, Hermes, Mistral, or an OpenAI-compatible image API"
            )
        if p["provider"] == "hermes" and not re.fullmatch(
            "[a-z0-9][a-z0-9_-]{0,55}|", p.get("hermes_provider", "")
        ):
            raise ValueError("Invalid Hermes image provider")
        if not isinstance(p.get("model", ""), str) or len(p.get("model", "")) > 300:
            raise ValueError("Invalid image model")
        if p["provider"] not in ("hermes", "mistral") or (
            p["provider"] == "mistral" and p.get("endpoint")
        ):
            endpoint(
                p.get(
                    "endpoint",
                    "https://api.mistral.ai/v1" if p["provider"] == "mistral" else "",
                )
            )
        if not isinstance(p.get("include_identity", True), bool):
            raise ValueError("Include identity must be true or false")
        if p.get("category") not in CATEGORIES:
            raise ValueError("Choose an image category")
        for k, v in p.get("parts", {}).items():
            if k not in PARTS or not isinstance(v, str) or len(v) > 20000:
                raise ValueError("Invalid prompt component")
        for key, label in (
            ("negative", "negative prompt"),
            ("modesty_negative", "modesty negative prompt"),
        ):
            if not isinstance(p.get(key, ""), str) or len(p.get(key, "")) > 20000:
                raise ValueError("Invalid " + label)
        if p["provider"] == "comfyui":
            workflow = p.get("workflow", {})
            if not isinstance(workflow, dict) or not workflow or "nodes" in workflow:
                raise ValueError(
                    "Import a ComfyUI API-format workflow, not the visual editor format. Use Export (API) in ComfyUI."
                )
            ...
            if "prompt" not in mappings and "identity" not in mappings:
                raise ValueError("Map a positive prompt or identity input")
            for key, target in mappings.items():
                if key not in (
                    *PARTS,
                    ...
                    raise ValueError("Unknown workflow mapping")
                if (
                    not isinstance(target, list)
                    or len(target) != 2
                    or str(target[0]) not in workflow
                    or target[1] not in workflow[str(target[0])]["inputs"]
                ):
                    raise ValueError(
                        "Each mapping must name an existing node ID and input"
                    )
        ...
        if not isinstance(p.get("requires_reference", False), bool):
            raise ValueError("Reference requirement must be true or false")
        key = p.get("api_key_env", "")
        if key and (
            not isinstance(key, str) or not re.fullmatch("[A-Z][A-Z0-9_]{0,100}", key)
        ):
            raise ValueError("Specify an environment variable name for the API key")
        for key, lo, hi in (
            ("width", 64, 4096),
            ("height", 64, 4096),
            ("steps", 1, 150),
        ...
        if (
            not isinstance(p.get("cfg", 5), (int, float))
            or not 0 <= p.get("cfg", 5) <= 50
        ):
            raise ValueError("CFG must be 0–50")
    routes = data.get("routes", {})
    if not isinstance(routes, dict) or any(
        k not in CATEGORIES or v not in ids for k, v in routes.items()
    ):
        raise ValueError("Category routes must reference saved presets")
    if data.get("default_preset") and data["default_preset"] not in ids:
        raise ValueError("Default preset does not exist")
    if len(json.dumps(data)) > 4_000_000:
        raise ValueError("Preset file exceeds 4 MB")
    return data
...
def save(c, data, expected):
    ...
    with file_lock(c.home / ".companion-images.lock"):
        if revision(c) != expected:
            raise FileExistsError("Image settings changed. Reload before saving.")
        path = c.home / CONFIG
        if path.exists():
            atomic_write(
                c.home
                / "companion-config-backups"
                / ("images-" + uuid.uuid4().hex + ".json"),
                path.read_text(),
            )
...
def template():
    ...
    return {
        "id": "comfy-plantmilk",
        "name": "Comfy – PlantMilk",
        "category": "anime",
        "provider": "comfyui",
        "endpoint": "http://127.0.0.1:8188",
        "parts": {
            "quality": "anime illustration, detailed",
            "wardrobe": "",
            "lighting": "soft light",
            "camera": "portrait",
        },
        ...
        "workflow": {
            ...
            "11": {
                ...
                "inputs": {"images": ["10", 0], "filename_prefix": "Companion"},
            },
        },
        "mappings": {
            "prompt": ["4", "text"],
...
def visual_style(c):
    ...
    if getattr(c, "image_style", "") in NON_VISUAL_STYLES:
        return ""
...
def compile(
    c, preset_id="", category="portrait", overrides=None, draft=None, intimate=False
):
    import companion_portrait as portrait
    ...
    if draft is not None:
        ...
        ident = (
            preset_id
            or data.get("routes", {}).get(category)
            or data.get("default_preset")
        )
        preset = next((p for p in data["presets"] if p["id"] == ident), None)
    if not preset:
        raise ValueError("Choose and save an image preset first")
    p = copy.deepcopy(preset)
    parts = dict(p.get("parts", {}))
    ...
    parts["identity"] = (
        (parts.get("identity") or recorded["identity"])
        if p.get("include_identity", True)
        else ""
    )
    ...
    if not parts.get("quality"):
        parts["quality"] = visual_style(c)
    ...
    prompt = ", ".join(
        parts.get(k, "").strip().rstrip(",") for k in PARTS if parts.get(k, "").strip()
    )
    if not prompt:
        raise ValueError("Write a scene or identity before generating")
    ...
    labels = {
        ...
        "identity": "IDENTITY — KEEP CONSISTENT",
        "wardrobe": "WARDROBE — SHOW EXACTLY",
        "scene": "SCENE AND ACTION",
    ...
    if parts.get("identity", "").strip() and parts.get("wardrobe", "").strip():
        preamble += (
            " Where IDENTITY mentions clothing in general, WARDROBE is what she is"
    ...
    if intimate:
        ...
        if not allowed:
            raise ValueError(
                "An intimate render is not available yet: "
                + (
                    " ".join(blockers)
                    if blockers
                    else "closeness has not reached that point."
                )
            )
        if "negative" not in p.get("mappings", {}) and p["provider"] == "comfyui":
            raise ValueError(
                "This workflow has no negative prompt input, so the always-on "
                "negatives cannot reach it. Map one before rendering intimately."
            )
    values = {
        **parts,
        "prompt": prompt,
        "negative": negative_prompt(p, intimate),
        "seed": seed,
        **{
            k: p.get(k, d)
            for k, d in [
    ...
    for key, (node, field) in p.get("mappings", {}).items():
        if key in values:
            workflow[str(node)]["inputs"][field] = values[key]
    reference = portrait.portrait_path(c)
    if p.get("requires_reference") and not reference.is_file():
        raise ValueError(
            "Add a reference portrait before using this image-to-image preset"
        )
    ...
    return {
        ...
        "reference_image": (
            ...
            and p["provider"] == "comfyui"
            and p.get("mappings", {}).get("reference_image")
            else None
        ),
    }
    ...
    for attempt in range(attempts):
        try:
            with urllib.request.urlopen(req, timeout=120) as r:
                return json.loads(r.read(16_000_001))
        except urllib.error.HTTPError as exc:
            raise ValueError(
                f"Image server returned {exc.code}: "
                + exc.read(3000).decode(errors="replace")
            ) from exc
        except (
...
def generate(
    ...
    allow_nsfw = bool(
        (intimate or allow_nsfw or getattr(c, "adult_images_allowed", False))
        and adult_ready(c)[0]
    )
    if purpose not in PURPOSES:
        raise ValueError("A render is either a creation or a capture")
    try:
        return _generate(
            c,
            preset_id,
        ...
        if allow_nsfw or held.rating not in ("nsfw", "unknown"):
            raise
        import companion_media_review as review

        report(
            (
                "Unintended nudity detected."
                if held.rating == "nsfw"
                else "The scan was uncertain about this image."
            )
        ...
        try:
            replacement = _generate(
                c, preset_id, category, retry, report, False, draft, False, purpose
            )
            target = Path(replacement["path"])
            meta = review.metadata(target)
            if (
                meta.get("rating") != "safe"
                or meta.get("review", {}).get("status") != "passed"
            ):
            ...
            raise ValueError(
                ...
                "Use --allow-nsfw if the adult content was intended."
            ) from held
        review.write_metadata(
            held.path,
            {
                "replacement_status": "replaced",
                "superseded_by": str(target.relative_to(c.data)),
            },
        )
...
def _comfy_progress(base, client_id, ident, report):
    ...
    try:
        ...
        return None
    url = (
        base.replace("https://", "wss://", 1).replace("http://", "ws://", 1)
        + "/ws?clientId="
        + client_id
    )
        ...
        try:
            with connect(url, open_timeout=5, close_timeout=1) as socket:
                while True:
                    ...
                    if data.get("prompt_id") not in (None, ident):
                        continue
...
def _surface_held(c, path):
    ...
    try:
        ...
        for extra in (review.sidecar(path), path.with_suffix(".json")):
            if extra.is_file():
                os.replace(extra, folder / extra.name)
    except OSError:
        return path
    return target
...
def _generate(
    ...
    result = compile(c, preset_id, category, overrides, draft, intimate)
    p = result["preset"]
    base = endpoint(p["endpoint"]) if p["provider"] != "hermes" else ""
    provider_prompt = (
        result["prompt"] if p["provider"] == "comfyui" else result["structured_prompt"]
    )
    ...
    if p["provider"] == "hermes":
        reply = hermes_bridge(
            ...
            "generate",
            {
                ...
                "hermes_provider": p.get("hermes_provider", ""),
                "image_url": (
                ...
                "model": p.get("model", ""),
                "aspect_ratio": "landscape" if category == "landscape" else "portrait",
            },
        )
        if not reply.get("success"):
            raise ValueError(str(reply.get("error", "Hermes returned no image")))
        ...
        for node_id, node in graph.items():
            if node["class_type"] not in schema:
                raise ValueError("Missing ComfyUI node: " + node["class_type"])
            for key, value in node["inputs"].items():
                spec = (
                    schema[node["class_type"]]
                    .get("input", {})
                    .get("required", {})
                    .get(key)
                )
                if p.get("mappings", {}).get("reference_image") == [node_id, key]:
                    continue
                if (
                    spec
                    ...
                    raise ValueError(f"Choose an installed value for {key}: {value}")
        if "reference_image" in p.get("mappings", {}) and result["reference_image"]:
            ...
            node, field = p["mappings"]["reference_image"]
            graph[str(node)]["inputs"][field] = uploaded["name"]
        client_id = uuid.uuid4().hex
        queued = request_json(
            base + "/prompt", {"prompt": graph, "client_id": client_id}
        )
        if queued.get("node_errors"):
            raise ValueError(
                "Workflow validation failed: "
                + json.dumps(queued["node_errors"])[:3000]
            )
        ident = queued.get("prompt_id")
        if not ident:
            raise ValueError("ComfyUI did not return a prompt ID")
        ...
        while time.monotonic() < deadline:
            ...
            if history.get("status", {}).get("status_str") == "error":
                raise ValueError(
                    ...
                    + json.dumps(history.get("status"))[:2000]
                )
            ...
            if waited and waited % 3 == 0:
                try:
                    ...
                    ahead = sum(
                        1
                        for row in pending.get("queue_pending", [])
                        if row and str(row[1]) != str(ident)
                    )
                    if not any(
                        str(row[1]) == str(ident)
                        for row in pending.get("queue_running", [])
                        if row
                    ):
                        report(
                            f"Waiting in the ComfyUI queue; {ahead} ahead"
                            if ahead
                            else "Waiting for ComfyUI to start"
                        )
                except Exception:
                    pass
            waited += 1
        ...
        images = [
            ...
            for im in output.get("images", [])
            if im.get("type") == "output"
        ]
        if not images:
            raise ValueError(
                "No saved image returned; the job may still be queued. Check ComfyUI before retrying."
            )
        ...
        key = keys.get(p.get("api_key_env", "") or "MISTRAL_API_KEY") or os.environ.get(
            p.get("api_key_env", "") or "MISTRAL_API_KEY"
        )
        if not key:
            raise ValueError("No MISTRAL_API_KEY found in .env or environment")
        ...
        if not conv_model.startswith("mistral-") and not conv_model.startswith(
            "ministral-"
        ):
        ...
        payload = {
            ...
            "inputs": [
                {
                    "role": "user",
                    "content": f"Generate an image using this specification:{aspect_text}\n\n{provider_prompt}",
                }
            ],
        }
        headers = {"Authorization": "Bearer " + key}
        reply = request_json(
            "https://api.mistral.ai/v1/conversations", payload, headers
        )
        image_url = None
        for entry in reply.get("outputs") or []:
            if (
                entry.get("type") == "tool.execution"
                and entry.get("name") == "image_generation"
            ):
                ...
                if image_url:
                    break
        if not image_url:
            raise ValueError("Mistral Conversations API returned no image")
        ...
        keys = _env_values(c.home)
        key = keys.get(p.get("api_key_env", "")) or os.environ.get(
            p.get("api_key_env", "")
        )
        ...
        reply = request_json(
            base + "/images/generations",
            {
                "model": p.get("model", ""),
                "prompt": provider_prompt,
                "size": f'{p.get("width",1024)}x{p.get("height",1024)}',
                "n": 1,
            },
            headers,
        )
        ...
        if image.get("b64_json"):
            ...
            raise ValueError("Image API did not return an image")
    if len(raw) > 40_000_000:
        raise ValueError("Generated image exceeds 40 MB")
    import io
        ...
        json.dumps(
            {
                ...
                "active_prompt_type": active_type,
                "parts": result["parts"],
                "preset": p["name"],
                "seed": result["seed"],
            },
    ...
    generation = (
        (
            ...
            if p["provider"] == "comfyui"
                ...
                else p.get("hermes_provider") or p["provider"]
            )
        )
        + " · "
        + p["name"]
    ...
    if review.preferences(c)["review_before_delivery"]:
        ...
        try:
            decision = review.inspect(c, path, provider_prompt, allow_nsfw)
        except Exception:
            review.write_metadata(path, {"review": {"status": "unavailable"}})
            raise ValueError(
                "Image saved for inspection in Photos, but review is unavailable. Do not send it; check the reviewer in Media preferences."
            )
        if decision["status"] != "passed":
            ...
            raise ImageHeld(
                "Image held for inspection in Photos; do not send it. "
                + str(decision.get("reason", "It did not match the intended image."))[
                    :400
                ],
                path,
                review.metadata(path)["rating"],
            )
    ...
    return {
        ...
        "seed": result["seed"],
        "rating": meta["rating"],
        "blur": review.should_blur(review.preferences(c), meta),
...
if __name__ == "__main__":
    ...
    parser.add_argument("command", choices=["generate", "prompt"])
    parser.add_argument("--category", default="portrait", choices=CATEGORIES)
    parser.add_argument("--preset", default="")
    parser.add_argument("--scene")
    parser.add_argument(
        "--allow-nsfw",
        action="store_true",
        help="This image intentionally requests adult content",
    )
    parser.add_argument(
        "--intimate",
        action="store_true",
    ...
    c = cc.load(args.home)
    kwargs = (
        {"allow_nsfw": args.allow_nsfw, "intimate": args.intimate}
        if args.command == "generate"
        else {"intimate": args.intimate}
    )
    print(
        json.dumps(
            (generate if args.command == "generate" else compile)(
                c,
                args.preset,
                args.category,
                {"scene": args.scene} if args.scene else None,
                **kwargs,
            ),
            indent=2,
        )
