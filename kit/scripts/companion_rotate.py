...
def plan(text, today, keep_days=14, keep_head=50):
    ...
    for i, e in enumerate(entries):
        d = entry_date(e)
        if (d is None and i < keep_head) or (d is not None and d >= cutoff):
            keep.append(e)
        else:
            move.append((d, e))
...
def rotate(path, today=None, keep_days=14, keep_head=50, apply=False):
    ...
    with file_lock(path.parent / ("." + path.name + ".lock")):
        ...
        buckets = {}
        for d, e in move:
            buckets.setdefault(d.strftime("%Y-%m") if d else "undated", []).append(e)
        report = {
            "file": str(path),
            "entries": len(entries),
            "keep": len(keep),
            ...
            "after_chars": len(pre) + sum(map(len, keep)),
            "applied": False,
        }
        ...
        for month, items in buckets.items():
            ...
            if any(saved[item] < count for item, count in needed.items()):
                raise ValueError("Archive verification failed; source retained")
        out = pre + "".join(keep)
        if len(split_entries(out)[1]) != len(keep):
            raise ValueError("Kept-entry mismatch; source retained")
        if path.read_text(encoding="utf-8") != text:
            raise ValueError(
                "Source changed during rotation; source retained, retry safely"
            )
        atomic_write(path, out)
        report["applied"] = True
        report["after_chars"] = len(out)
        return report
...
def main():
    ...
    p.add_argument("--keep-head", type=int, default=50)
    p.add_argument(
        "--apply",
        action="store_true",
        help="without this, only reports what would move",
    )
    a = p.parse_args()
    import json
    ...
    print(
        json.dumps(
            [
                rotate(x, keep_days=a.keep_days, keep_head=a.keep_head, apply=a.apply)
                for x in a.paths
            ],
            indent=2,
        )
