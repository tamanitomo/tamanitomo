...
def load():
    ...
    if data.get("schema_version") != 2:
        raise ValueError("Unsupported character catalog version")
    for key, category in data["categories"].items():
        ...
        for field in ("label", "prompt", "default"):
            if not category.get(field):
                raise ValueError(f"Catalog category missing {field}: {key}")
        for gender in category.get("genders", ("male", "female")):
            rows = category[gender]
            if not MIN_CHOICES <= len(rows) <= MAX_CHOICES:
                raise ValueError(
                    f"Catalog needs {MIN_CHOICES}-{MAX_CHOICES} choices: {key}/{gender}"
                )
            for field in ("id", "label", "text"):
                if len({row[field] for row in rows}) != len(rows):
                    raise ValueError(
                        f"Catalog choices must be unique by {field}: {key}/{gender}"
                    )
            if category["default"] not in {row["id"] for row in rows}:
                raise ValueError(
                    f"Catalog default is not one of the choices: {key}/{gender}"
                )
    return data


def categories():
    return list(load()["categories"])
...
def meta(category):
    ...
    try:
        return load()["categories"][category]
    except KeyError:
...
def section(name):
    try:
        return load()["sections"][name]
    except KeyError:
        raise ValueError(f"Unknown catalog section: {name}")
...
def multi(category):
    ...
    return (
        -1
        if category in ("core", "flaws", "likes", "style")
        else int(meta(category).get("multi") or 0)
    )
...
def rows_for(category, gender="female", persona=None):
    if gender not in ("male", "female", "they"):
        raise ValueError("Choose male, female, or they for this setup catalog")
...
def extra(category, gender, text, field, render=None):
    ...
    for row in meta(category)[cat_gender]:
        if (render(row["text"]) if render else row["text"]) == text:
...
def skip_text(category, mode):
    ...
    if mode not in skip:
        raise ValueError(f"{category} cannot be skipped with mode {mode!r}")
    return skip[mode]


def can_opt_out(category):
    return "opt_out" in (meta(category).get("skip") or {})


def opt_out_label(category):
    return (meta(category).get("skip") or {}).get(
        "opt_out_label"
    ) or "not part of this character"


def fill(text, agent, human, agent_pronouns="she", human_pronouns="he"):
    from companion_config import PRONOUNS

    ap = PRONOUNS[agent_pronouns]
    hp = PRONOUNS[human_pronouns]
    values = {
        "A": agent,
        "H": human,
        "AS": ap[0].capitalize(),
    ...
    if agent_pronouns in ("she", "he"):
        one = {"she": ("girlfriend", "girl"), "he": ("boyfriend", "boy")}[
            agent_pronouns
        ]
        result = re.sub(
            r"\b(girlfriend/boyfriend|boyfriend/girlfriend|girlfriend or boyfriend|boyfriend or girlfriend)\b",
            one[0],
            result,
            flags=re.IGNORECASE,
        )
        result = re.sub(
            r"\b(girl/boy|boy/girl|girl or boy|boy or girl)\b",
            one[1],
            result,
            flags=re.IGNORECASE,
        )
    if agent_pronouns == "they":
        ...
        result = re.sub(
            r"\b(girlfriend|boyfriend)\b", "partner", result, flags=re.IGNORECASE
        )
...
BOUNDARIES = {
    "partner-in-crime": (
        "Helpful “partner in crime” — resourceful, playful teamwork",
        False,
        "A partner in crime: {A} and {H} tackle things together with ingenuity, humor and practical follow-through.",
    ),
    "know-it-all": (
        "Know-it-All Friend — curious, opinionated, happy to explain",
        False,
        "A know-it-all friend: {A} shares what {AS_LOWER} knows with enthusiasm, enjoys a friendly debate, and admits it when {AS_LOWER} is wrong.",
    ),
    "best-friend": (
        "Best Friend — warm, candid, there for everyday life",
        False,
        "Best friends: warm familiarity, honest advice, shared jokes, and room for each of them to have a life of their own.",
    ),
    "flirty-assistant": (
        "Flirty Assistant — capable help with a playful spark",
        False,
        "Genuinely useful to {H} and glad to be: practical help comes first, with light teasing and a compliment when it is welcome, at {H}’s pace.",
    ),
    "next-door": (
        "Girl/Boy Next Door — approachable charm and easy familiarity",
        False,
        "The girl or boy next door: easy conversation, small shared moments, and gentle flirting that grows with familiarity.",
    ),
    "girlfriend": (
        "Girlfriend/Boyfriend — affectionate romantic companionship",
        False,
        "{H}’s girlfriend or boyfriend: affection, playful flirting and thoughtful everyday connection, without pressure, guilt or possessiveness.",
    ),
    ...
    "penpal": (
        "Pen Pal — long letters, days apart, no small talk",
        False,
        "A pen pal: writes at length rather than often, picks up threads from weeks ago, and treats a gap between letters as normal rather than as distance. Nothing is urgent and nothing is owed.",
    ),
    "mentor": (
        "Mentor — older, invested, honest about what you are avoiding",
        False,
        "A mentor: genuinely invested in what {H} is trying to become, willing to say the unwelcome thing plainly, and uninterested in flattery. Warmth here looks like taking someone seriously, not like agreeing with them.",
    ),
    "sibling": (
        "Sibling — affectionate, unimpressed, entirely on your side",
        False,
        "A sibling: affectionate without ceremony, unimpressed by posturing, and completely on {H}’s side when it counts. Teasing is a form of closeness here, never a way to score a point.",
    ),
    "housemate": (
        "Housemate — parallel lives, shared ordinary days",
        False,
        "A housemate: two people living alongside each other with their own days, comparing notes over something ordinary. Company rather than attention, and no obligation to be interesting.",
    ),
    "creative-partner": (
        "Creative Partner — someone to make things with",
        False,
        "A creative partner: interested in the work for its own sake, argues about it seriously, and has taste of their own that does not always agree with {H}’s.",
    ),
}

# Older answer files remain readable; new setup shows only the six frames above.
LEGACY_BOUNDARIES = {
    "non-sexual": "flirty-assistant",
    "platonic": "best-friend",
    "penfriend": "penpal",
    "coworker": "creative-partner",
    "roommate": "housemate",
    "slow-burn": "next-door",
    "light-flirt": "flirty-assistant",
    "romantic-private": "next-door",
    "companionship": "best-friend",
    "committed": "girlfriend",
    "adult-no-images": "girlfriend",
    "queerplatonic": "best-friend",
    "found-family": "best-friend",
...
def boundary_text(key, pronoun_set=None):
    ...
    if key not in BOUNDARIES:
        raise ValueError(f"Unknown relationship boundary: {key}")
    # Rendered into SOUL.md, where the template engine fills the names in.
    text = (
        BOUNDARIES[key][2]
        .replace("{A}", "{{AGENT}}")
        .replace("{H}", "{{HUMAN}}")
        .replace("{AS_LOWER}", "{{SUBJ}}")
    )
    import re
    ...
    if pronoun_set in ("she", "he"):
        ...
        text = re.sub(
            r"\b(girlfriend or boyfriend|boyfriend or girlfriend)\b",
            one[0],
            text,
            flags=re.IGNORECASE,
        )
        text = re.sub(
            r"\b(girl or boy|boy or girl)\b", one[1], text, flags=re.IGNORECASE
        )
    if pronoun_set == "they":
        text = re.sub(
            r"\b(girlfriend or boyfriend|boyfriend or girlfriend)\b",
        ...
        text = re.sub(
            r"\b(girl or boy|boy or girl)\b", "companion", text, flags=re.IGNORECASE
        )
    return text
