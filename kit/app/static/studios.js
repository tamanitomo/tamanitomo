...
workspaceHandlers['companion-edit']=async()=>{
 ...
 $('companion-edit').innerHTML=heading('Edit '+d.display_name,'Your companion’s current configuration and authored identity, prefilled from their files.')+`<div class="actions"><button class="quiet" data-editor-route="roster">← All companions</button><button class="quiet" data-editor-route="voice">Voice studio</button><button class="quiet" data-editor-route="image-studio">Image studio</button><button class="quiet" data-editor-route="environment">Hermes settings</button></div><form id="companion-edit-form"><div class="card"><label>Display name <span class="dim small">The name shown in this workspace</span><input id="editor-display-name" value="${esc(d.display_name)}" required maxlength="100"></label></div>${groups.map(([label,keys],i)=>`<details class="card" ${i<2?'open':''}><summary>${label}</summary><div class="form-grid">${keys.map(k=>fieldHTML(k,d.config[k],choices[k]||[],d.fixed.includes(k))).join('')}</div></details>`).join('')}<div class="card"><h2>Personality, appearance, boundaries & shared history</h2><p class="dim">These sections are prefilled from the current SOUL, including custom sections.</p><div id="editor-soul-sections"></div></div><details class="card"><summary>Full SOUL document</summary><p class="dim">This is the actual SOUL document, including writing created outside the kit. Review its wording when changing identity fields above. Saving preserves this text exactly and keeps a backup.</p><textarea id="editor-soul" aria-label="Companion SOUL document" class="soul-editor">${esc(d.soul)}</textarea></details><div class="actions"><button class="act">Save companion</button><button type="button" class="quiet" id="reload-companion">Reload current files</button></div></form>`;
 for(const b of $('companion-edit').querySelectorAll('[data-editor-route]'))b.onclick=()=>showTab(b.dataset.editorRoute);
 const drawSoulSections=()=>{const text=$('editor-soul').value;const headings=[...text.matchAll(/^## (.+)$/gm)];$('editor-soul-sections').innerHTML=headings.length?headings.map((m,i)=>`<details ${i===0?'open':''}><summary>${esc(m[1])}</summary><textarea data-soul-section="${i}" aria-label="${esc(m[1])}">${esc(text.slice(m.index+m[0].length,headings[i+1]?.index??text.length).trim())}</textarea></details>`).join(''):'<p class="dim">This document has no section headings. Use the full-document editor below.</p>';for(const input of $('editor-soul-sections').querySelectorAll('[data-soul-section]'))input.oninput=()=>{const raw=$('editor-soul').value,matches=[...raw.matchAll(/^## (.+)$/gm)],i=Number(input.dataset.soulSection),m=matches[i];if(m)$('editor-soul').value=raw.slice(0,m.index+m[0].length)+'\n\n'+input.value+'\n\n'+raw.slice(matches[i+1]?.index??raw.length);};};drawSoulSections();$('editor-soul').onchange=drawSoulSections;
