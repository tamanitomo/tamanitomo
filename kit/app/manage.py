...
def register(app, select, load, operations):
    ...
    def vault_links_resolve(source:str,target:str,kind:str='wiki'):
        ...
        if kind not in ('wiki','md'):raise HTTPException(400,'kind must be wiki or md')
        entries,incomplete=vli.build(c)
    ...
    def vault_links_embed_note(source:str,target:str,heading:str=''):
        ...
        def get_text(rel):
            try:return vault.read(c,rel)['text']
    ...
    def vault_links_embed_image(source:str,target:str):
        ...
        c=load()
        vault.resolve(c,source)
        lookup=vli.asset_lookup(c)     # images are not in the note index; a plain file walk
        candidates=vli.resolve_wiki(target,lookup)
        if len(candidates)!=1:raise HTTPException(404,'Image not found or ambiguous')
        resolved=candidates[0]
        if not vli.is_image_target(resolved):raise HTTPException(400,'Not a supported embeddable image type')
