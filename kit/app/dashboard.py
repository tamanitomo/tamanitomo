...
class Dashboards:
    ...
    def start(self, rt):
        with self.lock:
            ...
            if not python.is_file():
                raise ValueError("Hermes Python is unavailable. Install Hermes first.")
            folder = rt.root / ".companion-dashboard"
            ...
            raise ValueError(
                "Hermes dashboard could not start. " + redact(output)[-2000:]
            )
...
def register(app, select, access_token=""):
    ...
    @app.api_route(
        "/api/hermes/{installation}/{path:path}",
    ...
    async def proxy(installation: str, path: str, request: Request):
        ...
        client = httpx.AsyncClient(
            timeout=httpx.Timeout(180, connect=10), follow_redirects=False
        )
        try:
        ...
        if "location" in upstream.headers:
            ...
            response_headers["location"] = (
                prefix + location if location.startswith("/") else location
            )
    ...
    async def websocket_proxy(websocket: WebSocket, installation: str, path: str):
        dash_cookie = websocket.cookies.get(
            "tamanitomo_dashboard"
        ) or websocket.cookies.get("companion_dashboard", "")
        cookie_ok = secrets.compare_digest(dash_cookie, app.state.dashboard_cookie)
        ws_token = websocket.headers.get("x-tamanitomo-token") or websocket.headers.get(
            "x-companion-token", ""
        )
        ...
        query = [
            ...
            if k not in ("token", "session_token")
        ]
        query.append(("token", row["token"]))
        uri = f'ws://127.0.0.1:{row["port"]}/{path}?' + urlencode(query)
        async with websockets.connect(
            uri, origin=f'http://127.0.0.1:{row["port"]}', max_size=16_000_000
        ) as upstream:
