...
MODELS = [
    {
        "id": "qwen3:4b-q4_K_M",
        "name": "Qwen3 4B · compact",
        "gb": 2.6,
        "memory": "8 GB RAM minimum; 12+ GB preferred for the full stack",
        "context": 8192,
    },
    {
        "id": "qwen3:8b-q4_K_M",
        ...
        "gb": 5.2,
        "memory": "16+ GB RAM; roughly 8 GB free GPU memory for acceleration",
        "context": 8192,
    },
    {
        "id": "qwen3:14b-q4_K_M",
        "name": "Qwen3 14B · more capable",
        "gb": 9.3,
        "memory": "24–32+ GB RAM; roughly 12–16 GB free GPU memory for acceleration",
        "context": 8192,
    },
]
...
MOBILE_MODELS = [
    {
        ...
        "name": "SmolLM2 1.7B · ultra-compact mobile",
        "gb": 1.0,
        "memory": "4 GB RAM; fast & battery-efficient on mobile CPU",
        "context": 4096,
    },
    {
        "id": "qwen2.5:1.5b-instruct-q4_K_M",
        "name": "Qwen 2.5 1.5B · lightweight mobile",
        "gb": 1.1,
        "memory": "4–6 GB RAM; excellent reasoning for mobile footprint",
        "context": 4096,
    },
    {
        "id": "llama3.2:1b-instruct-q4_K_M",
        "name": "Llama 3.2 1B · compact mobile",
        "gb": 0.8,
        "memory": "4 GB RAM; minimal battery & memory overhead",
        "context": 4096,
    },
    {
        "id": "llama3.2:3b-instruct-q4_K_M",
        "name": "Llama 3.2 3B · high-capability mobile",
        "gb": 2.0,
        "memory": "6–8 GB RAM; balance of capability and memory",
        "context": 4096,
    },
]
...
def is_mobile() -> bool:
    ...
    override = os.environ.get("TAMANITOMO_IS_MOBILE") or os.environ.get(
        "COMPANION_IS_MOBILE"
    )
...
def validate_model_safety(
    model_size_gb: float, mem_info: dict | None = None
) -> tuple[bool, str]:
...
def default_llama_service():
    return os.environ.get("TAMANITOMO_LLAMA_SERVICE") or os.environ.get(
        "COMPANION_LLAMA_SERVICE", "companion-llama"
    )
...
def _service_status(unit_name=None):
    ...
    sv_bin = shutil.which("sv") or (
        Path(prefix) / "bin/sv" if (Path(prefix) / "bin/sv").is_file() else None
    )
    sv_dir = Path(prefix) / "var/service"
    if sv_bin and (sv_dir / unit_name).exists():
        try:
            res = subprocess.run(
                [str(sv_bin), "status", str(sv_dir / unit_name)],
                capture_output=True,
                text=True,
                timeout=3,
            )
        ...
        try:
            for p in d.rglob("*.gguf"):
                try:
                    ...
                    family = "GGUF Model"
                    nl = p.name.lower()
                    if "gemma" in nl:  # Public model family: google/gemma
                        family = "Gemma"  # Public model family: google/gemma
                    elif "qwen" in nl:
                        family = "Qwen"
                    elif "smollm" in nl:
                        family = "SmolLM"
                    ...
                    mmprojs = [
                        m for m in p.parent.glob("*.gguf") if "mmproj" in m.name.lower()
                    ]
...
def status(rt):
    ...
    try:
        ...
        if data:
            ...
            loaded_model = {
                ...
                or (first.get("details") or {}).get("quantization_level")
                or "",
    ...
    if not online:
        error = (
            "Local model server is offline. Use Start to launch the background engine."
        )
    ...
    for m in disk_models:
        alias = (
            ...
            .replace("-it", "")
            .replace("-abliterated", "")
            .replace("-uncensored", "")[:24]
        )
        m["alias"] = alias
        m["active"] = bool(
            m["path"] in exec_start
            or (loaded_model and loaded_model.get("id") == alias)
        )
    ...
    return {
        ...
        "models": models,
        "installed": bool(
            srv.get("found") or binary(rt) or shutil.which("llama-server")
        ),
...
def asset_name():
    ...
    if arch is None:
        raise ValueError(
            "This processor needs a manual Ollama installation from ollama.com/download"
        )
    if system == "Linux":
        return f"ollama-linux-{arch}.tar.zst"
    if system == "Windows":
        return f"ollama-windows-{arch}.zip"
    if system == "Darwin":
        return "ollama-darwin.tgz"
    raise ValueError(
        "Install Ollama manually on this operating system, then use Start / refresh here."
    )
    ...
    if archive.name.endswith(".zip"):
        with zipfile.ZipFile(archive) as bundle:
            for item in bundle.infolist():
                ...
                if not target.is_relative_to(dest.resolve()) or "\\" in item.filename:
...
def install(rt, report):
    if binary(rt):
        return {
            "note": "Ollama is already installed. Use Start / refresh, then choose model weights."
        }
    name = asset_name()
    root = directory(rt)
    ...
    if not asset:
        raise ValueError(
            "No official Ollama archive for this host. See ollama.com/download."
        )
    digest = asset.get("digest", "")
    if not re.fullmatch(r"sha256:[0-9a-f]{64}", digest):
        raise ValueError(
            "The official release has no SHA-256 digest; install it manually instead."
        )
    url = asset["browser_download_url"]
    if not url.startswith("https://github.com/ollama/ollama/releases/download/"):
        raise ValueError("Unexpected Ollama release source")
    size = int(asset["size"])
    if shutil.disk_usage(root).free < size * 4:
        raise ValueError(
            "Free more disk space before installing Ollama (archive plus extracted runtime)."
        )
    with tempfile.TemporaryDirectory(prefix=".install-", dir=root) as temp:
        ...
        with (
            ...
            while chunk := response.read(1024 * 1024):
                ...
                if total > size:
                    raise ValueError(
                        "Ollama archive is larger than its release metadata"
                    )
                sha.update(chunk)
                out.write(chunk)
                progress = total * 100 // size
                if progress // 10 != last:
                    report(f'Downloading Ollama {release["tag_name"]}: {progress}%')
                    last = progress // 10
        if total != size or sha.hexdigest() != digest[7:]:
            raise ValueError(
                "Ollama archive integrity check failed; retry the download"
            )
        ...
        if not any(
            (stage / p).is_file() for p in ("bin/ollama", "ollama", "ollama.exe")
        ):
            raise ValueError("Ollama archive has an unexpected layout")
        stage.rename(root / "runtime")
    return {
        "note": "Ollama installed on the Hermes host. Start it, then download a recommended model."
    }
...
def start(rt, report):
    ...
    if status(rt)["online"]:
        return {"note": "Using the local model server already running on this host."}
    ...
    if not executable and llama_server_bin:
        ...
        for _ in range(35):
            if status(rt)["online"]:
                return {
                    "note": f"Local llama-server is ready and serving {m['alias']}."
                }
            if proc.poll() is not None:
                raise ValueError(
                    "Local server could not start. Check "
                    + str(root / "llama-server.log")
                )
            time.sleep(1)
        raise ValueError("Local llama-server startup timed out.")

    if not executable:
        raise ValueError("Install Ollama or configure a local model service first")
    root = directory(rt)
    root.mkdir(parents=True, exist_ok=True)
    env = dict(
        os.environ,
    ...
    if Path(executable).is_relative_to(root):
    ...
    for _ in range(60):
        if status(rt)["online"]:
            return {"note": "Local server is ready."}
        if proc.poll() is not None:
            raise ValueError(
                "Local server could not start. Check " + str(root / "server.log")
            )
        time.sleep(1)
    ...
    if srv.get("found"):
        ...
        if srv.get("service_mgr") == "runit":
            ...
            cmd_action = (
                "up"
                if action == "start"
                else ("down" if action == "stop" else "restart")
            )
            if report:
                report(f"Running sv {cmd_action} {unit}")
            res = subprocess.run(
                [sv_bin, cmd_action, str(sv_dir / unit)], capture_output=True, text=True
            )
            if res.returncode != 0:
            ...
            return {
                "ok": True,
                "action": action,
                "note": f"{unit} ({action}) completed via termux-services.",
            }

        # Handle systemd
        if report:
            report(f"Running systemctl --user {action} {unit}")
        res = subprocess.run(
        ...
        if action in ("start", "restart"):
            if report:
                report("Waiting for model server to initialize on Vulkan...")
            for _ in range(35):
                time.sleep(1)
                try:
                    ...
                    if m:
                        return {
                            "ok": True,
                            "action": action,
                            "note": f"{unit} is online and serving models.",
                        }
                except Exception:
                    pass
            return {
                "ok": True,
                "action": action,
                "note": f"{unit} command issued; server is initializing.",
            }
...
def switch_model(
    rt, model_path: str, alias: str = "", ctx_size: int = 131072, report=None
):
...
"""
    if report:
        report(f"Configuring local server for {alias}...")
    dropin_file.write_text(content, encoding="utf-8")

    if shutil.which("systemctl"):
        subprocess.run(["systemctl", "--user", "daemon-reload"], check=True)
        if report:
            report("Restarting model server on Vulkan GPU...")
...
def pull(rt, ident, report):
    ...
    if not choice:
        raise ValueError("Choose a recommended model weight variant")
    mem = get_host_memory()
    ...
    root = directory(rt)
    root.mkdir(parents=True, exist_ok=True)
    if shutil.disk_usage(root).free < choice["gb"] * 1_000_000_000 * 1.2:
        raise ValueError("Not enough free disk space for these model weights")
    req = urllib.request.Request(
        BASE + "/api/pull",
        data=json.dumps({"model": ident, "stream": True}).encode(),
        headers={"Content-Type": "application/json"},
    )
    last = ""
    success = False
    with urllib.request.build_opener(urllib.request.ProxyHandler({})).open(
        ...
        for line in response:
            ...
            if row.get("total"):
                message += f' · {int(row.get("completed", 0) * 100 / row["total"])}%'
            if message != last:
                report(message)
                last = message
            success = row.get("status") == "success"
    if not success:
        raise ValueError("Model download was interrupted; retry to resume it")
    return {
        "note": ident
        + " downloaded. Select Use for this companion to change its model."
    }
...
def download_gguf(rt, model_id: str, report):
    ...
    if not model:
        raise ValueError(
            f"Unknown GGUF model '{model_id}'. Choose a recommended model variant."
        )
    ...
    if target_file.is_file() and target_file.stat().st_size > int(
        model["gb"] * 1024 * 1024 * 1024 * 0.8
    ):
    ...
    if free_bytes < required_bytes:
        raise ValueError(
            f"Not enough free disk space. Required: {model['gb'] * 1.15:.1f} GB, available: {free_bytes / (1024**3):.1f} GB."
        )
    ...
    if existing_bytes > 0:
        ...
        report(
            f"Resuming download of {model['name']} from {existing_bytes // (1024 * 1024)} MB..."
        )
    else:
    ...
    try:
        with opener.open(req, timeout=60) as resp:
            ...
            total_size = (
                ...
                if content_length
                else int(model["gb"] * 1024 * 1024 * 1024)
            )
            ...
            with open(temp_file, mode) as f:
                while True:
                    ...
                    if pct // 5 != last_pct // 5:
                        report(
                            f"Downloading {model['name']}: {pct}% ({downloaded // (1024 * 1024)} MB / {total_size // (1024 * 1024)} MB)"
                        )
                        last_pct = pct
...
def assign(rt, home, ident):
    ...
    try:
        ...
        if (
            ident in names
            or any(m["id"] == ident for m in MODELS)
            or any(m["id"] == ident for m in MOBILE_MODELS)
        ):
            is_ollama = True
    except Exception:
        pass
    ...
    if is_ollama:
        ...
        if (
            ...
            or ident not in names
            or ident.endswith(("-cloud", ":cloud"))
        ):
        ...
        if "tools" not in info.get("capabilities", []):
            raise ValueError(
                "This model does not advertise tool calling, which Hermes needs. Choose a recommended model."
            )
        ...
        with cp.file_lock(home / ".companion-profile-editor.lock"):
            ...
            if raw is not None:
                cp.atomic_write(
                    ...
                    / ("local-model-" + uuid.uuid4().hex + ".json"),
                    path.read_text(),
                )
        ...
        return {
            "note": "Local model tested and saved for this profile; cloud fallbacks cleared. Review background job models and use Apply job models, then restart the gateway to reload workers."
        }
    ...
    return {
        "note": f"Model {ident} assigned to this companion for chat, loops, and reflection."
    }
...
def register(app, select):
    def op(label, fn):
        ...
        return app.state.operations.submit(
            str(rt.root), label, lambda report: fn(rt, home, report), profile=p
        )

    @app.get("/api/local-models")
    def get():
        return status(select()[0])

    @app.post("/api/local-models/server/control")
    def control_route(payload: dict):
        action = payload.get("action", "restart")
        return op(
            f"{action.title()} model server",
            lambda rt, h, r: server_control(rt, action, r),
        )
    ...
    def switch_route(payload: dict):
        ...
        return op(
            ...
            lambda rt, h, r: switch_model(rt, path, alias, ctx_size, r),
        )

    @app.post("/api/local-models/install")
    def install_route():
        return op("Install Ollama", lambda rt, h, r: install(rt, r))

    @app.post("/api/local-models/start")
    def start_route():
        return op("Start local server", lambda rt, h, r: start(rt, r))

    @app.post("/api/local-models/pull")
    def pull_route(payload: dict):
        return op(
            "Download local model", lambda rt, h, r: pull(rt, payload.get("model"), r)
        )
    ...
    def download_gguf_route(payload: dict):
        ...
        return op(
            "Download GGUF model", lambda rt, h, r: download_gguf(rt, model_id, r)
        )

    @app.post("/api/local-models/assign")
    def assign_route(payload: dict):
        return op(
            "Use local model", lambda rt, h, r: assign(rt, h, payload.get("model"))
        )
