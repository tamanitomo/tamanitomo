...
def build(home=None, token="", state_dir=None):
    ...
    public_origin = (
        os.environ.get("TAMANITOMO_PUBLIC_ORIGIN")
        or os.environ.get("COMPANION_PUBLIC_ORIGIN", "")
    ).rstrip("/")
    if public_origin:
    ...
    default_home_state = (
        initial.hermes_root / ".companion-app"
        if (initial.hermes_root / ".companion-app").exists()
        and not (initial.hermes_root / ".tamanitomo-app").exists()
        else initial.hermes_root / ".tamanitomo-app"
    )
    state = (
        pathlib.Path(state_dir)
        if state_dir
        else (default_home_state if home else app_directory())
    )
    runtimes = {
        "existing": Runtime(initial.hermes_root),
        "managed": Runtime(state / "managed-hermes", managed=True),
    }
    ...
    if linked_file.exists():
        ...
        for ident, root in linked.items():
            if re.fullmatch(r"linked-[a-f0-9]{12}", ident) and isinstance(root, str):
                runtimes[ident] = Runtime(root)
    selection = ContextVar(
        "companion_selection", default=("existing", initial.profile or "default")
    )

    def select():
        installation, profile = selection.get()
        if installation not in runtimes:
            raise ValueError("Unknown Hermes installation")
    ...
    def check_pin(pin, expected, client):
        ...
        with pin_attempt_lock:
            for host, (_, expiry) in list(pin_attempts.items()):
                if expiry <= now:
                    pin_attempts.pop(host, None)
            count, expiry = pin_attempts.get(client, (0, now + 60))
            if count >= 5 or (client not in pin_attempts and len(pin_attempts) >= 1024):
                return None
    ...
    async def guard(request: Request, call_next):
        ...
        if (
            not is_local
            and request.url.path.startswith(("/api", "/media"))
            ...
            try:
                ...
                if c.remote_pin:
                    ...
                    expected = hashlib.sha256(
                        f"{c.remote_pin}:{app.state.pin_salt}".encode()
                    ).hexdigest()
                    cookie_token = request.cookies.get(
                        "tamanitomo_pin_session"
                    ) or request.cookies.get("companion_pin_session", "")
                    header_pin = request.headers.get(
                        "x-tamanitomo-pin"
                    ) or request.headers.get("x-companion-pin", "")
                    pin_ok = (
                        bool(cookie_token)
                        and secrets.compare_digest(cookie_token, expected)
                    ) or (
                        bool(header_pin)
                        and check_pin(header_pin, c.remote_pin, client_host) is True
                    )
                    if not pin_ok:
                        return JSONResponse(
                            {
                                "pin_required": True,
                                "error": "Remote access locked. Enter 4-digit PIN.",
                            },
                            status_code=401,
                        )
            except Exception:
                return JSONResponse(
                    {"error": "Workspace access configuration could not be read"},
                    status_code=503,
                )
        ...
        if token and request.url.path.startswith(("/api", "/media")):
            supplied = (
                request.headers.get("x-tamanitomo-token")
                or request.headers.get("x-companion-token")
                or request.query_params.get("token", "")
            )
            dash_cookie = request.cookies.get(
                "tamanitomo_dashboard"
            ) or request.cookies.get("companion_dashboard", "")
            dashboard_cookie = request.url.path.startswith(
                "/api/hermes/"
            ) and secrets.compare_digest(dash_cookie, app.state.dashboard_cookie)
            if not dashboard_cookie and not secrets.compare_digest(
        ...
        if (
            ...
            and origin
            and origin not in {str(request.base_url).rstrip("/"), public_origin}
        ):
            return JSONResponse(
                {"detail": "Cross-origin writes are refused"}, status_code=403
            )
        current = selection.set(
            (
                request.query_params.get("installation", "existing"),
        ...
        try:
            if request.method not in (
                ...
                "HEAD",
                "OPTIONS",
            ) and not request.url.path.startswith("/api/terminal/"):
                ...
                if str(runtime.root) in app.state.operations.busy:
                    return JSONResponse(
                        {
                            "detail": "An action is running for this installation. Wait for it to finish."
                        },
                        status_code=409,
                    )
                if request.url.path != "/api/terminal" and any(
                    row.scope[0] == str(runtime.root) and not row.finished
                    for row in list(app.state.consoles.rows.values())
                ):
                    return JSONResponse(
                        {
                            "detail": "Close the native Hermes setup console before changing settings."
                        },
                        status_code=409,
                    )
        ...
        if (
            request.url.path.startswith(("/api", "/media"))
    ...
    def overview():
        ...
        state = (
            display_scene.get("state", {}) if isinstance(display_scene, dict) else {}
        )
        commitments = [
            dict(item)
        ...
        return {
            ...
            "timezone": c.timezone,
            "age": c.current_age(),
            "birthday_in": c.birthday_in(),
            "state": display_scene,
            "confirmed_at": anchor["recorded_at"] if anchor else None,
            "moods": presence.mood_history(c, 40),
            "loops": loops.loops(c),
            "missions": missions.missions(c, "open", now),
            ...
            "level_event": (
                ...
                if c.bars and c.relationship_progression != "off"
                else None
            ),
    ...
    def timeline():
        ...
        attempts = []
        for _, row in sorted(
            tl.records(c), key=lambda item: item[1]["created_at"], reverse=True
        ):
                ...
                attempts.append(
                    {
                        ...
                        "error": redact(row.get("error", ""))[:600],
                    }
                )
                continue
        ...
        return {
            ...
            "budget_gb": c.timeline_budget_gb,
    ...
    def schedule(day: str = ""):
        ...
        try:
            target = _dt.date.fromisoformat(day) if day else now.date()
        except ValueError:
            raise HTTPException(400, "Use a YYYY-MM-DD date")
        try:
            return life.expected_day(c.life, target, now)
    ...
    def media(name: str, request: Request):
        ...
        if not tl.IMAGE.fullmatch(name):
            raise HTTPException(404)
        path = tl.root(c) / "images" / name
        if (
            path.is_symlink()
            or not path.is_file()
            or not path.resolve().is_relative_to(c.data.resolve())
        ):
            raise HTTPException(404)
        ...
        if etag in [
            x.strip() for x in (request.headers.get("if-none-match") or "").split(",")
        ]:
    ...
    def keep(capture: str, album: str = "Favorites"):
        ...
        try:
            return tl.favorite(c, capture, album)
        except ValueError as exc:
            raise HTTPException(400, str(exc))
    ...
    def rerender_timeline(capture: str, payload: dict | None = None):
        ...
        if row.get("status") != "saved":
            raise HTTPException(400, "Can only rerender a saved capture")
        payload = payload or {}
        ...
        if not preset:
            preset_id = recipe.get("routes", {}).get("portrait") or recipe.get(
                "default_preset"
            )
            preset = next((p for p in presets if p["id"] == preset_id), None)
        if not preset:
        ...
        def run(rt, companion, report):
            ...
            variant = (
                updated.get("variants", [])[-1] if updated.get("variants") else None
            )
            if variant:
                ...
                updated = dict(
                    updated, variants=with_etags(companion, updated.get("variants", []))
                )
            return {
                "status": "saved",
                "capture_id": capture,
        ...
        return app.state.operations.submit(
            str(runtime.root),
            "Regenerate photo",
            lambda report: run(runtime, c, report),
    ...
    def select_timeline_variant(capture: str, payload: dict):
        ...
        try:
            ...
            return {
                ...
                "primary_filename": updated["primary_filename"],
                "capture": updated,
            }
        except ValueError as exc:
            raise HTTPException(400, str(exc))
    ...
    def closet_view():
        ...
        init_st = (
            lifestyle.initial(curr, c) if lifestyle.enabled(c) else {"clothes": {}}
        )
        clothes_status = dict(init_st.get("clothes", {}))
        wearing_ids = (
            {item["id"] for item in curr["state"].get("outfit", [])} if curr else set()
        )
        for wid in wearing_ids:
            clothes_status[wid] = "wearing"
        ...
        for w in wardrobe:
            ...
            if wid in wearing_ids:
                eff = "wearing"
            elif wid in laid_out and status != "dirty":
                eff = "laid_out"
            elif status in ("dirty", "hamper"):
                eff = "hamper"
            elif status == "washing":
                eff = "washing"
            else:
        ...
        return {
            ...
            "wearing": filter_wardrobe_items(
                [i for i in items if i["status"] == "wearing"], stage
            ),
            "laid_out": (
                {
                    "plan": tomorrow,
                    "items": filter_wardrobe_items(
                        [i for i in items if i["status"] == "laid_out"], stage
                    ),
                }
                if tomorrow
                else None
            ),
            "hamper": filter_wardrobe_items(
                [i for i in items if i["status"] == "hamper"], stage
            ),
            "washing": filter_wardrobe_items(
                [i for i in items if i["status"] == "washing"], stage
            ),
            "clean": filter_wardrobe_items(
                [i for i in items if i["status"] == "clean"], stage
            ),
    ...
    def ledgers():
        ...
        return {
            "facts": slf.facts(c.human_dir),
            "standing": notes.standing(c),
            "relationship": notes.moments(c),
            "questions": slf.questions(c.life, "open"),
            "preferences": slf._read(
                c.life / "preferences.jsonl", kind="companion_preference"
            )[-40:],
        }
    ...
    def forget(fact_id: str):
        ...
        if not any(f["id"] == fact_id for f in slf.facts(c.human_dir)):
            raise HTTPException(404, "no such active fact")
        slf.retract_fact(
            c.human_dir,
            fact_id,
            "Marked incorrect by " + c.human + " in the app",
            dt.datetime.now(dt.timezone.utc),
        )
        return {
            "retired": fact_id,
            "note": "Retracted, not deleted. The original stays in the ledger.",
        }
    ...
    def held_fact_decide(held_id: str, body: dict = Body(...)):
        ...
        if decision not in slf.HELD_DECISIONS:
            raise HTTPException(400, "decision must be accept or dismiss")
        try:
        ...
        except slf.HeldDecisionConflict as exc:
    ...
    def get_settings():
        ...
        return {
            ...
            "outreach": c.outreach,
            "outreach_per_day": c.outreach_per_day,
            "adaptive_quiet": c.adaptive_quiet,
            ...
            "autonomy_windows": c.autonomy_windows,
            "share_people": c.share_people,
            "image_interval_minutes": c.image_interval_minutes,
            "schedule_offset_minutes": c.schedule_offset_minutes,
            "timeline_budget_gb": c.timeline_budget_gb,
            "image_timeline": c.image_timeline,
            "bars": c.bars,
            "agent_type": c.agent_type,
            "relationship_progression": c.relationship_progression,
            "relationship_pace": c.relationship_pace,
            "peer_interaction": c.peer_interaction,
            "image_style": c.image_style,
            "image_styles": {k: v["label"] for k, v in render.load_styles().items()},
            "explicit": c.explicit,
            "adult_images": c.adult_images,
            # The choice exists only once the relationship has reached Bonded.
            "adult_images_available": bool((intimacy or {}).get("can_intimate")),
            "integrity_lockout": False,
            "integrity_warning": None,
            "intimacy": intimacy,
            "remote_pin": c.remote_pin,
        }
    ...
    def set_settings(payload: dict):
        ...
        if payload.get("explicit") is False and c.explicit:
            ...
            raise HTTPException(
                400,
                "Adult themes were permanently turned off for this companion and cannot be re-enabled.",
            )
        ...
        if payload.get("adult_images") is True and not c.adult_images:
            ...
            if not companion_intimacy.compute(c).get("can_intimate"):
                raise HTTPException(
                    400,
                    "Adult images become available once your relationship reaches Bonded.",
                )
        adult_confirmed = payload.pop("adult_confirmed", False)
        if (
            payload.get("explicit") is True
            and not c.explicit
            and adult_confirmed is not True
        ):
            raise HTTPException(
                400,
                "Confirm that both you and the companion are adults before enabling adult themes",
            )
        unknown = sorted(set(payload) - ALLOWED)
        if unknown:
            raise HTTPException(400, f'not settable from here: {", ".join(unknown)}')
        for key, value in payload.items():
            setattr(c, key, value)
        try:
            c.__post_init__()  # the same validation the CLI gets
        except ValueError as exc:
            raise HTTPException(400, str(exc))
        ...
        if "remote_pin" in payload:
            save_access_pin(initial.hermes_root, c.remote_pin)
    ...
    def verify_pin(request: Request, payload: dict = Body(...)):
        ...
        matched = check_pin(
            pin, c.remote_pin, request.client.host if request.client else "unknown"
        )
        if matched is None:
            return JSONResponse(
                {"ok": False, "detail": "Too many attempts. Try again in a minute."},
                status_code=429,
        ...
        if matched:
            ...
            return resp
        return JSONResponse(
            {"ok": False, "detail": "Incorrect 4-digit PIN"}, status_code=403
        )

    @app.get("/api/network")
    def network_info(request: Request):
        c = load()
        port = request.url.port or int(
            os.environ.get("TAMANITOMO_PORT")
            or os.environ.get("COMPANION_PORT")
            or os.environ.get("PORT", "38439")
        )
        client_host = request.client.host if request.client else "127.0.0.1"
        is_local = client_host in ("127.0.0.1", "::1", "localhost", "testclient")
        ...
        expected = (
            hashlib.sha256(f"{c.remote_pin}:{app.state.pin_salt}".encode()).hexdigest()
            if c.remote_pin
            else ""
        )
        cookie_token = request.cookies.get(
        ...
        authenticated = (
            is_local
            or (not c.remote_pin)
            or (bool(expected) and secrets.compare_digest(cookie_token, expected))
        )
        return {
            "port": port,
            "is_local": is_local,
    ...
    def drop_mission(mission: str):
        ...
        try:
            return missions.update(c, mission, "dropped", "withdrawn in the app")
        except ValueError as exc:
            raise HTTPException(400, str(exc))
    ...
    def calendar_ics():
        ...
        for m in all_missions:
            ...
            uid = m.get(
                "id", hashlib.sha256(m.get("title", "").encode()).hexdigest()[:16]
            )
            status = (
                "CONFIRMED"
                if m.get("status") == "open"
                else "CANCELLED" if m.get("status") == "dropped" else "COMPLETED"
            )
            lines.extend(
                [
                    "BEGIN:VEVENT",
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f'attachment; filename="{c.agent}-calendar.ics"'
            },
        )
    ...
    def document_paths(c):
        ...
        for parent in (c.home, c.home / "memories"):
            ...
            for path in parent.glob("*.md"):
                if path.is_file() and not path.is_symlink() and path.name != "SOUL.md":
                    paths[str(path.relative_to(c.home))] = path
        return paths

    @app.get("/api/documents")
    def documents():
        return {"documents": sorted(document_paths(load()))}
    ...
    def soul_document(document: str = "SOUL.md"):
        ...
        if path.stat().st_size > 2_000_000:
            raise HTTPException(400, "Document exceeds 2 MB")
    ...
    def save_soul_document(payload: dict):
        ...
        c = load()
        path = document_paths(c).get(payload.get("document", "SOUL.md"))
        if path is None:
            raise HTTPException(404, "Unknown document")
        body = payload.get("text")
        if not isinstance(body, str) or len(body.encode()) > 2_000_000:
            raise HTTPException(400, "Document must be text under 2 MB")
        with cp.file_lock(path.parent / ("." + path.name + ".lock")):
            original = path.read_text(encoding="utf-8")
            if hashlib.sha256(original.encode()).hexdigest() != payload.get("revision"):
                raise HTTPException(409, "Document changed. Reload before saving.")
            cp.atomic_write(
                pathlib.Path(c.soul_backups) / (path.name + "." + uuid.uuid4().hex),
                original,
            )
    ...
    def repair_identity(payload: dict):
        ...
        with cp.file_lock(path.parent / ("." + path.name + ".lock")):
            original = path.read_text(encoding="utf-8")
            try:
                repaired = ident.retrofit(original)
            except ValueError as exc:
                raise HTTPException(400, str(exc))
            found = ident.sections(repaired)
            missing = [
                name
                for name in ("core", "appearance", "relationship", "voice")
            ...
            for name in missing:
                ...
                if (
                    ...
                    or ident.OPEN.search(body)
                    or "/COMPANION-SECTION" in body
                ):
                    raise HTTPException(400, "Use plain section text")
                addition += (
                    "\n\n<!-- COMPANION-SECTION:"
                    + name
                    + (" LOCKED" if name in ("appearance", "relationship") else "")
                    + " -->\n"
                    + body
                    + "\n<!-- /COMPANION-SECTION:"
                    + name
            ...
            if addition or repaired != original:
                cp.atomic_write(
                    pathlib.Path(c.soul_backups) / ("SOUL.md." + uuid.uuid4().hex),
                    original,
                )
    ...
    def identity():
        ...
        try:
            ...
            boundaries = render.BOUNDARIES if hasattr(render, "BOUNDARIES") else {}
            ...
            profile = {
                ...
                "timezone": c.timezone,
                "persona": c.persona,
                "persona_label": persona.get("label", c.persona),
                "persona_blurb": persona.get("blurb", ""),
                "boundary": c.boundary,
                "boundary_label": frame[0] if frame else c.boundary,
                "relationship_pace": c.relationship_pace,
                "image_style": c.image_style,
            }
                ...
                "sections": [
                    {
                        **s,
                        "body": s["body"],
                        "keeper": (by.get(s["name"]) or {}).get("keeper"),
                ...
                "private": [
                    {
                        ...
                        "opens": r.get("opens"),
                        "open": r["her"]["can"]
                        or "cooldown" in r["her"]
                        or r["id"] in companion_soul.private_notes(c),
                        "written": r["id"] in companion_soul.private_notes(c),
                    }
                    for r in keepers["sections"]
                    if r["keeper"] == "private"
                ],
                "stage_name": keepers["stage_name"],
                "days_together": keepers["days_together"],
                "portrait": portrait.compile_prompt(c),
                "profile": profile,
            }
    ...
    def lock_section(payload: dict):
        ...
        try:
            return companion_soul.set_lock(
                load(), str(payload.get("section", "")), payload.get("locked") is True
            )
        except ValueError as exc:
            raise HTTPException(400, str(exc))
    ...
    def write_section(section: str, payload: dict):
        ...
        if keeper == "hers":
            raise HTTPException(
                403,
                f"{c.agent} writes this section; it can be read but not edited here.",
            )
        if keeper in ("fixed", "system"):
            raise HTTPException(403, "This section is maintained by the app.")
        import companion_identity as ident
        ...
        try:
            return ident.replace(c, section, payload.get("body", ""))
        except ValueError as exc:
            raise HTTPException(400, str(exc))
    ...
    def upload_portrait(data: bytes = Body(..., media_type="application/octet-stream")):
        ...
        if not data:
            raise HTTPException(400, "no image in the request body")
        if len(data) > tl.MAX_BYTES:
            raise HTTPException(400, "Image exceeds 32 MB")
        import tempfile
        import os as _os
        ...
        try:
            ...
            handle.close()
            try:
                return pt.save_reference(c, handle.name)
    ...
    def portrait_from_content(payload: dict):
        ...
        if KINDS[p.suffix.lower()] != "image":
            raise HTTPException(400, "Choose an image file")
        if p.stat().st_size > tl.MAX_BYTES:
            raise HTTPException(400, "Image exceeds 32 MB")
        try:
            return pt.save_reference(c, str(p))
        except (ValueError, OSError) as exc:
            raise HTTPException(400, str(exc))
    ...
    def forget_portrait():
        ...
        path = pt.portrait_path(c)
        if not path.is_file():
            raise HTTPException(404, "no stored likeness")
        path.unlink()
    ...
    def health():
        ...
        return {
            "problems": watch.problems(c, now),
            "memory": memory.status(c),
            "storage": prune.survey(c),
            "vault_repo": vault.is_repo(c.vault),
            "jobs": _jobs(c),
        }
    ...
    def cost():
        ...
        try:
            ...
            pass
        return {
            "days": [{"day": k, **v} for k, v in sorted(by_day.items())[-30:]],
            "source": str(path),
            "available": bool(by_day),
        }

    def _jobs(c):
        try:
            rows = json.loads((c.home / "cron/jobs.json").read_text(encoding="utf-8"))[
                "jobs"
            ]
        ...
        return [
            ...
            for j in rows
            if str(j.get("name", "")).startswith(c.agent + " ")
        ]
