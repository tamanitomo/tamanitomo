...
def _zone(c):
    try:
        return ZoneInfo(c.timezone or "UTC")
    except Exception:
...
def parse_day(value):
    ...
    if not isinstance(value, str) or not DAY.fullmatch(value):
        raise ValueError("Choose a date as YYYY-MM-DD")
    try:
        return dt.date.fromisoformat(value)
    except ValueError:
        raise ValueError("Choose a real calendar date")
...
def local_moment(value, tz):
    ...
    """
    if not isinstance(value, str) or DAY.fullmatch(value):
        return None
    try:
...
def _episodes(c, day, tz):
    ...
    for offset in (-1, 0, 1):
        ...
        if path.is_symlink():
            ...
            continue
        try:
            read = life.read_events(c.life, name, limit=0)
        except OSError:
            if offset == 0 or path.exists():
                unavailable = True
            continue
        except ValueError:
            continue
    ...
    timed = sorted(
        (x for x in rows if x[0] is not None),
        key=lambda x: x[0].astimezone(dt.timezone.utc),
    )
...
def scenes(c, day, tz):
    ...
    state = (
        "unavailable"
        if unavailable
        else ("available" if out else ("empty" if found else "none"))
    )
    return out, state
...
def attach_photos(c, day, scene_rows, tz):
    ...
    for scene in scene_rows:
        for ident in scene["ids"]:
            by_id[ident] = scene
    wanted = {}
    unplaced = []
    try:
        records = list(timeline.records(c))
    except (OSError, ValueError):
        return "unavailable", []
    for _, row in records:
        if not isinstance(row, dict) or row.get("status") != "saved":
            continue
        ...
        if owner is None:
            if local_day(scene.get("recorded_at"), tz) == day.isoformat():
                state = (
                    scene.get("state") if isinstance(scene.get("state"), dict) else {}
                )
                unplaced.append(
                    {
                        ...
                        "activity": _text(state.get("activity"), 120),
                        "photos": [],
                    }
                )
    ...
    for _, row in records:
        if row.get("id") not in wanted:
            continue
        for name in [row.get("primary_filename"), row.get("filename")] + [
            v.get("filename") for v in row.get("variants", []) if isinstance(v, dict)
        ]:
            if isinstance(name, str) and name:
                paths.add("image-timeline/images/" + name)
    try:
        found = catalog(c, kind="image", reference_paths=paths)
    except (OSError, ValueError):
        return "unavailable", unplaced
    items = found["items"]
    ...
    for item in items:
        ...
        if target is None:
            continue
        target["photos"].append(item)
        seen.add(item.get("capture_id"))
    for capture, target in wanted.items():
        if capture not in seen and "missing_photos" in target and not partial:
            target["missing_photos"] += 1
...
def plan(c, day):
    ...
    items = [
        {
            "start": _text(x.get("start"), 10),
            "end": _text(x.get("end"), 10),
            "what": _text(x.get("what"), 200),
            "where": _text(x.get("where"), 200),
            "status": _text(x.get("status"), 20) or "planned",
        }
        for x in data["items"]
    ]
    return {
        ...
        "items": items,
        "intent": _text(data.get("intent"), 400),
    }
...
def reflection(c, day):
    ...
    entries = [
        {k: e[k] for k in ("id", "day", "excerpt", "words", "source", "truncated")}
        for e in data["entries"]
        if e["day"] == day.isoformat()
    ]
...
def together(c, day):
    ...
    if path.is_symlink():
        return {"state": "unavailable", "moments": []}
    try:
    ...
    rows = [
        {
            "id": r.get("id"),
            "moment": r.get("moment"),
            "text": _text(r.get("text"), 500),
            "happened_on": r.get("happened_on"),
            "status": r.get("status"),
        }
        for r in rows
        if r.get("happened_on") == day.isoformat()
    ]
...
def archive_day(c, value):
    ...
    return {
        ...
        "human": c.human,
        "scenes": rows,
        "unplaced_photos": unplaced,
        "plan": plan(c, day),
        "reflection": reflection(c, day),
        "together": together(c, day),
...
def archive_index(c):
    ...
    try:
        names = sorted(
            p.stem for p in folder.glob("????-??-??.jsonl") if not p.is_symlink()
        )
    except OSError:
        names = []
    unavailable = False
    for name in names[-1500:]:
        try:
            rows = life.read_events(c.life, name, limit=0)
        except (OSError, ValueError):
            unavailable = True
            continue
        for row in rows:
            key = (
                local_day(row.get("recorded_at"), tz) if isinstance(row, dict) else None
            )
            if key is None and isinstance(row, dict):
                key = name
            if key:
                days.setdefault(key, {"scenes": False, "reflection": False})[
                    "scenes"
                ] = True
    from .content import journals

    data = journals(c, limit=1000)
    warnings = data["warnings"]
    while True:
        for e in data["entries"]:
            days.setdefault(e["day"], {"scenes": False, "reflection": False})[
                "reflection"
            ] = True
...
def register(app, load):
    ...
    def journal_archive_index():
        return archive_index(load())

    @app.get("/api/journal/archive/{day}")
    def journal_archive_day(day: str):
        return archive_day(load(), day)
