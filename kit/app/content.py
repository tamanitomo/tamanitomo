...
KINDS={'.png':'image','.jpg':'image','.jpeg':'image','.webp':'image','.gif':'image',
       ...
       '.mp4':'video','.webm':'video','.mov':'video','.md':'writing','.txt':'writing','.pdf':'document'}
EXCLUDED={'soul-backups','memory-archive','__pycache__','captures','node_modules','.git'}

def resolve(c,relative):
    if not isinstance(relative,str) or '\\' in relative or '\x00' in relative:raise ValueError('Invalid content path')
    rel=Path(relative)
    if rel.is_absolute() or not rel.parts or any(p in ('..','.') or p.startswith('.') or p in EXCLUDED for p in rel.parts) or peer.is_secret(rel) or re.search(r'(?i)(?:^|[._-])(credentials?|secrets?|passwords?|tokens?|api[_-]?keys?)(?:[._-]|$)',rel.name):
        raise ValueError('Choose a visible companion content file')
    if c.is_root and rel.parts[0]=='agents':raise ValueError('Choose this companion’s own content')
    root=c.data.resolve();path=root/rel
    for parent in [path,*path.parents]:
        if parent==root:break
        if parent.is_symlink():raise ValueError('Linked content files are not served')
    path=path.resolve()
...
def unique_images(rows):
    ...
    for digest,copies in groups.items():
        copies.sort(key=lambda r:({'creation':0,'photo session':1,'album':2}[r['source']],r['at'],r['path']))
        row=dict(copies[0])
        row['content_id']=digest
        row['at']=min(r['at'] for r in copies)
        row['copies']=[{k:r[k] for k in ('path','source','url','etag','deletable')} for r in copies]
        row['blur']=any(r.get('blur',False) for r in copies)
        # A saved copy with a stricter review must not become exposed by grouping.
        row['rating']=next((rating for rating in ('nsfw','unknown','safe') if any(r.get('rating','unknown')==rating for r in copies)),'unknown')
        result.append(row)
    return result
...
def catalog(c,limit=1500,before=None,kind='',q='',day='',collection='all',content_id=None,reference_paths=None):
    from .runtime import _page_cursor,_encode_cursor
    if type(limit)!=int or not 1<=limit<=1500:raise ValueError('Content page size must be 1–1500')
    if kind and kind not in set(KINDS.values()):raise ValueError('Unknown content kind')
    if len(q)>200 or len(collection)>100:raise ValueError('Content filter is too long')
    if day:
        try:dt.date.fromisoformat(day)
        except ValueError:raise ValueError('Choose a valid photo day')
    cursor=_page_cursor(before)
    if cursor and not isinstance(cursor[1],str):raise ValueError('Invalid content cursor')
    prefs=review.preferences(c)
    rows=[];scanned=0;limited=False
    for folder,dirs,names in os.walk(c.data,followlinks=False):
        dirs[:]=sorted(d for d in dirs if not d.startswith('.') and d not in EXCLUDED and not (Path(folder)/d).is_symlink())
        if c.is_root and Path(folder)==c.data and 'agents' in dirs:dirs.remove('agents')
        for name in sorted(names):
            scanned+=1
            if scanned>10000:limited=True;break
            p=Path(folder)/name
            if p.suffix.lower() not in KINDS:continue
            rel=p.relative_to(c.data).as_posix()
            try:p=resolve(c,rel);stat=p.stat()
            ...
            if media_kind=='image':
                ...
                if superseded:
                    try:
                        ...
                        if review.metadata(replacement).get('rating')=='safe':continue
                    except (OSError,ValueError):pass
            # Protocol/configuration and rendered state belong in the Vault, not Creations.
            if media_kind=='writing' and (rel.startswith('soul/') or p.name in ('README.md','PROTOCOL.md','PRESENCE.md')):continue
            try:digest=_image_digest(str(p),stat.st_mtime_ns,stat.st_ctime_ns,stat.st_size,stat.st_ino) if media_kind=='image' else None
            except OSError:continue
            rows.append({'path':rel,'title':p.stem.replace('_',' ').replace('-',' '),'kind':media_kind,
                'at':dt.datetime.fromtimestamp(stat.st_mtime,dt.timezone.utc).isoformat(),'bytes':stat.st_size,
                'etag':str(stat.st_mtime_ns)+':'+str(stat.st_size),'deletable':deletable(rel),
                **({'_digest':digest} if media_kind=='image' else {}),
                **({**review.metadata(p),'blur':review.should_blur(prefs,review.metadata(p))} if media_kind=='image' else {}),
                'url':'/api/content/file?path='+quote(rel,safe=''),'source':'photo session' if rel.startswith('image-timeline/') else 'album' if rel.startswith('albums/') else 'creation'})
        if limited:break
    ...
    if q:rows=[r for r in rows if q.casefold() in (r['title']+' '+r['path']+' '+str(r.get('mood',''))).casefold()]
    if day:rows=[r for r in rows if dt.datetime.fromisoformat(r['at']).astimezone(ZoneInfo(c.timezone)).date().isoformat()==day]
    if collection!='all':
        rows=[r for r in rows if any(x['path'].startswith('albums/'+collection[6:]+'/') if collection.startswith('album:') else x['source']==collection for x in r.get('copies',[r]))]
    ...
    more=len(rows)>page_limit;rows=rows[:page_limit]
    return {'timezone':c.timezone,'items':rows,'limited':limited or more,'scan_limited':limited,'scanned':scanned,'total':total,
            'next_cursor':_encode_cursor(*key(rows[-1])) if more else None}


def journals(c,limit=1000,before=None,q='',month='',entry_id=None):
    if type(limit)!=int or not 1<=limit<=1000:raise ValueError('Journal page size must be 1–1000')
    if len(q)>200:raise ValueError('Journal search must be at most 200 characters')
    if month and not re.fullmatch(r'\d{4}-(?:0[1-9]|1[0-2])',month):raise ValueError('Choose a valid journal month')
    if before and not re.fullmatch(r'\d{4}-\d{2}-\d{2}:[a-f0-9]{20}',before):raise ValueError('Invalid journal cursor')
    if entry_id and not re.fullmatch(r'[a-f0-9]{20}',entry_id):raise ValueError('Invalid journal entry')
    roots=list(dict.fromkeys([c.soul_dir/'Lifelog.md',c.home/'Lifelog.md',*sorted((c.soul_dir/'continuity/archive').glob('Lifelog-*.md')),*sorted((c.home/'continuity/archive').glob('Lifelog-*.md'))]))
    result=[];warnings=[];remaining=16_000_000
    if len(roots)>500:warnings.append('Only the first 500 journal files were scanned. Additional archives remain in the Vault.')
    for path in roots[:500]:
        ...
        for base in (c.data,c.home):
            ...
            if not any(base.joinpath(*parts[:i]).is_symlink() for i in range(1,len(parts)+1)):
                contained=True;break
        if not contained:
            warnings.append('A linked journal archive was excluded. Open it from your local vault.');continue
        if path.is_symlink() or path.stat().st_size>8_000_000:
            warnings.append('A journal file is linked or too large to preview. Open it from your local vault.');continue
        size=path.stat().st_size
        if size>remaining:
            warnings.append('Journal preview reached its 16 MB limit. Browse the Vault for additional archives.');break
        remaining-=size
        try:body=path.read_text(encoding='utf-8')
        except (OSError,UnicodeError):warnings.append('A journal file could not be read.');continue
        matches=list(re.finditer(r'^##\s+(\d{4}-\d{2}-\d{2})[^\n]*$',body,re.M))
        for i,m in enumerate(matches):
            text=body[m.end():matches[i+1].start() if i+1<len(matches) else len(body)].strip()
            if not text:continue
            try:dt.date.fromisoformat(m.group(1))
            except ValueError:continue
            ident=hashlib.sha256((str(path)+str(m.start())).encode()).hexdigest()[:20]
            row={'id':ident,'day':m.group(1),'text':text if entry_id else text[:100000],
                 'excerpt':re.sub(r'[#*_`>\[\]]','',text)[:240],'words':len(text.split()),'source':path.name,
                 'truncated':not entry_id and len(text)>100000}
            if entry_id:
                if ident==entry_id:return {'timezone':c.timezone,'entry':row}
                continue
            if month and not row['day'].startswith(month):continue
            if q and q.casefold() not in (row['day']+' '+text).casefold():continue
            result.append(row)
    if entry_id:raise ValueError('This journal entry changed or is no longer available. Refresh the journal list.')
    result.sort(key=lambda r:(r['day'],r['id']),reverse=True);total=len(result)
    if before:result=[r for r in result if r['day']+':'+r['id']<before]
    more=len(result)>limit;result=result[:limit]
    return {'timezone':c.timezone,'entries':result,'warnings':warnings,'total':total,
...
def register(app,load):
    ...
    def save_media_preferences(payload:dict):
        c=load();result=review.save_preferences(c,payload)
    ...
    def rate_content(payload:dict):
        c=load();p=resolve(c,payload.get('path',''))
        if KINDS[p.suffix.lower()]!='image' or payload.get('rating') not in ('safe','nsfw','unknown'):raise ValueError('Choose an image rating')
        paths=[p]
        stat=p.stat()
    ...
    def read_journals(limit:int=1000,before:str|None=None,q:str='',month:str=''):return journals(load(),limit,before,q,month)
    @app.get('/api/journals/{ident}')
    def read_journal_entry(ident:str):return journals(load(),entry_id=ident)
    @app.post('/api/content/album')
    def save_album(payload:dict):
        import companion_timeline as timeline
        c=load();source=resolve(c,payload.get('path',''));album=payload.get('album','Favorites')
        if KINDS[source.suffix.lower()]!='image':raise ValueError('Choose an image to keep')
    ...
    def text_content(path:str):
        p=resolve(load(),path)
        if p.suffix.lower() not in ('.md','.txt'):raise ValueError('Choose a text document')
