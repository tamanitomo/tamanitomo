...
def family(label):
    v = str(label).lower()
    if any(s in v for s in ("sdxl", "pony", "illustrious", "noobai")):
        return "sdxl"
    if "sd 1" in v or "sd1" in v:
        return "sd15"
    if "krea 2" in v or "krea2" in v:
        return "krea2"
    if "zimage" in v or "z-image" in v or "z image" in v:
        return "zimage"
    return "unknown"
...
def settings(root):
    ...
    if p.is_symlink():
        raise ValueError("Comfy settings must not be a symbolic link")
    return (
        ...
        if p.exists()
            ...
            "mode": "local",
            "host": "",
            "directory": str(root / "companion-engines/comfyui"),
...
def public_settings(d):
    return {
        **{k: v for k, v in d.items() if k != "api_key"},
        "api_key_configured": bool(d.get("api_key")),
    }
...
def save_settings(root, payload):
    ...
    directory = payload.get("directory", current["directory"])
    if mode not in ("local", "ssh"):
        raise ValueError("Choose local or SSH Comfy host")
    if mode == "ssh" and (
        not isinstance(host, str)
        or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.@-]{0,150}", host)
    ):
    ...
    if (
        ...
        raise ValueError("Choose a ComfyUI directory")
    if mode == "ssh" and not directory.startswith("/"):
        raise ValueError("Remote ComfyUI directory must be absolute")
    if mode == "local" and not Path(directory).expanduser().is_absolute():
        raise ValueError("ComfyUI directory must be absolute")
    key = payload.get("api_key")
    if key is not None and (
        not isinstance(key, str) or len(key) > 2000 or any(ord(c) < 32 for c in key)
    ):
    ...
    current.update(
        ...
        host=host,
        directory=directory,
        endpoint=media.endpoint(payload.get("endpoint", current["endpoint"])),
    )
    if "hash_lookup" in payload:
        current["hash_lookup"] = bool(payload["hash_lookup"])
    if key:
        current["api_key"] = key
    if payload.get("clear_key"):
        current["api_key"] = ""
    path = root / "companion-comfy-private.json"
    cp.atomic_write(path, json.dumps(current, indent=2))
    path.chmod(0o600)
...
def api_json(path, key):
    try:
        with open_url("https://civitai.com/api/v1/" + path, key, 30) as r:
            return json.load(r)
    except Exception as exc:
        raise ValueError(
            "Civitai could not return this model. Check the URL, API key and access permissions."
        ) from exc


def identify(url):
    if not isinstance(url, str):
        raise ValueError("Paste a Civitai model page URL")
    u = urllib.parse.urlsplit(url)
    if (
        u.scheme != "https"
        or u.hostname not in ("civitai.com", "www.civitai.com")
        or u.username
        or u.password
    ):
        raise ValueError("Paste an HTTPS Civitai model page URL")
    match = re.fullmatch(r"/models/(\d+)(?:/[^/]*)?/?", u.path)
    if not match:
        raise ValueError("Use a Civitai /models/ page")
...
def inspect_model(root, url, version_id=None):
    model_id, linked = identify(url)
    key = settings(root).get("api_key", "")
    model = api_json("models/" + str(model_id), key)
    versions = model.get("modelVersions") or []
    wanted = version_id or linked
    selected = (
        next((v for v in versions if v["id"] == wanted), None)
        if wanted
        else next(iter(versions), None)
    )
    if selected is None:
        raise ValueError("The selected model version is unavailable")
    files = []
    for f in selected.get("files", []):
        if f.get("type") != "Model" or Path(f.get("name", "")).suffix.lower() not in (
            ".safetensors",
            ".gguf",
        ):
            continue
        sha = (f.get("hashes") or {}).get("SHA256", "")
        size = float(f.get("sizeKB") or 0) * 1024
        if (
            not re.fullmatch("[a-fA-F0-9]{64}", sha)
    ...
    return {
        ...
        "base_model": selected.get("baseModel", "Unknown"),
        "family": family(selected.get("baseModel")),
        "versions": [
            {
                "id": v["id"],
                "name": v.get("name", ""),
        ...
        "files": files,
        "trigger_words": selected.get("trainedWords") or [],
        "license": {
            k: model.get(k)
            for k in (
                "allowNoCredit",
...
def download(root, payload, report):
    ...
    if f is None:
        raise ValueError("Choose an available weight file")
    ...
    if slot not in permitted.get(data["type"], set()):
        raise ValueError("This Civitai model type does not fit the selected slot")
    ...
    return {
        **result,
        ...
        "note": "Verified weights installed. Refresh the model list before building.",
    }
...
def run_scan(config, folders, hash_unknown=False, known=None):
    ...
    result = None
    error = "Could not read the model files on the Comfy host"
    try:
        proc.stdin.write(
            json.dumps(
                {
                    "folders": folders,
                    "hash_unknown": bool(hash_unknown),
                    "known": known or {},
                }
            )
        )
...
def read_scan_cache(root):
    try:
        return json.loads(scan_cache_path(root).read_text(encoding="utf-8"))
...
def write_scan_cache(root, data):
    try:
        scan_cache_path(root).write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
def run_worker(config, request, report=lambda _: None):
    ...
    result = None
    error = "Model downloader failed; check the Comfy path and SSH access"
    try:
...
def register(app, select, load):
    ...
    def get():
        rt, _ = select()
        return {
            "settings": public_settings(settings(rt.root)),
            "families": {
    ...
    def install(payload: dict):
        ...
        return app.state.operations.submit(
            str(rt.root),
            "Download Comfy weights",
            lambda report: download(rt.root, payload, report),
            profile=p,
        )
    ...
    def build(payload: dict):
        rt, _ = select()
        p = create_recipe(payload)
        p["endpoint"] = settings(rt.root)["endpoint"]
        media.validate(
            {"version": 1, "presets": [p], "routes": {}, "default_preset": p["id"]}
        )
        schema = media.request_json(p["endpoint"] + "/object_info")
        for node in p["workflow"].values():
            if node["class_type"] not in schema:
                raise ValueError(
                    "Install the required Comfy node first: " + node["class_type"]
                )
            for key, value in node["inputs"].items():
                ...
                if (
                    ...
                    raise ValueError("Choose an installed " + key + ": " + value)
        return p
