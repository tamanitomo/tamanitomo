...
class Console:
    def __init__(self, runtime, home, action):
        if action not in COMMANDS:
            raise ValueError("Unknown Hermes setup console")
        import pyte
        ...
        env = runtime.env(home)
        env.update(TERM="xterm-256color", COLUMNS="110", LINES="32")
        argv = runtime.command() + COMMANDS[action]
        if os.name == "nt":
            from winpty import PtyProcess

            self.process = PtyProcess.spawn(
                argv, cwd=str(home), env=env, dimensions=(32, 110)
            )
        else:
            import pty
            import fcntl
            ...
            try:
                fcntl.ioctl(
                    slave, termios.TIOCSWINSZ, struct.pack("HHHH", 32, 110, 0, 0)
                )
        ...
        threading.Thread(
            target=self._expiry, daemon=True, name="hermes-console-expiry"
        ).start()
    ...
    def _read(self):
        ...
        try:
            while True:
                data = (
                    self.process.read(16384)
                    if os.name == "nt"
                    else decoder.decode(os.read(self.master, 16384))
                )
                if not data:
                    break
                with self.lock:
    ...
    def write(self, data):
        self.last_access = time.monotonic()
        if self.finished:
            raise ValueError("This Hermes console has finished")
        if not isinstance(data, str) or len(data) > 12000:
            raise ValueError("Invalid console input")
        if os.name == "nt":
...
class Consoles:
            ...
            for row in self.rows.values():
                if row.scope[0] == str(rt.root) and not row.finished:
                    if row.scope == (str(rt.root), str(home)):
                        return row
                    raise ValueError(
                        "Close the other setup console for this installation first"
                    )
    ...
    def get(self, ident, rt, home):
        ...
        if not row or row.scope != (str(rt.root), str(home)):
            raise ValueError("Console not found for this profile")
        return row
