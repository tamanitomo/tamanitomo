...
TIER_HELP={
 'loops':('The pulse and the autonomy loop. These run every 15 and 30 minutes, so this is the choice '
 ...
 'reflection':('The daily journal and the weekly and monthly reflections. A handful of runs a week, '
               'each deciding what the companion keeps of itself. Worth your best model.'),
}

def current_rows(c,config):
    default=((config.get('model') or {}) if isinstance(config.get('model'),dict) else {})
    rows=[]
...
def ask_tier(c,tier,config):
    ...
    print();print(wiz.C.dim('  '+TIER_HELP[tier]));print()
    model=ask(f"Model for {tier} (blank to follow the profile's own model, "
              f"{default.get('default') or 'unset'})",pick.get('model',''))
    provider=ask('Provider for that model (blank if Hermes already knows it)',pick.get('provider','')) if model.strip() else ''
    effort=pick_key('How much reasoning should these runs do?',
                    [(k,f'{k} — {v}') for k,v in EFFORT_HELP.items()]+[('','leave it to Hermes')],
                    default=1)
    out={}
    if model.strip():out['model']=model.strip()
    if provider.strip():out['provider']=provider.strip()
...
def screen(c):
    while True:
        ...
        print(wiz.C.dim(f'  {c.home}'));print()
        ...
        print(f"  {'fallbacks':<12}"+(', '.join(f"{r.get('model')}" for r in chain) if chain else 'none set'))
        print()
        action=wiz.choose('What would you like to change?',
            [('The loops — pulse and autonomy, every 15 and 30 minutes','loops'),
             ('Reflection — the daily, weekly and monthly jobs','reflection'),
             ('The fallback chain, when the first provider will not answer','fallbacks'),
             ('Re-pin the installed jobs to these choices','apply'),
             ('Back','back')],allow_write=False,allow_skip=False)
        if action=='back':return 0
        if action in ('loops','reflection'):
            picked=ask_tier(c,action,config)
            models=dict(c.models)
            if picked:models[action]=picked
            else:models.pop(action,None)
            c=cc.dataclasses.replace(c,models=models);c.save()
            print(wiz.C.dim(f'  saved. Run "re-pin" below to move the installed jobs onto it.'))
            ...
            print(wiz.C.dim(f'  Updated {len(result["updated"])} jobs. Their run history is preserved.'))
        input('\n  '+wiz.C.dim('press Enter to return'));wiz.clear()

def cmd_models(args):
    """Choose what each kind of scheduled job runs on, and a fallback chain."""
    c=resolve(args,require_config=True)
    if getattr(args,'apply',False):
        print(json.dumps(apply_job_models(c),indent=2));return 0
    if getattr(args,'show',False):
        config=read_config(c)
        print(json.dumps({'tiers':{t:{'model':m,'reasoning_effort':e} for t,m,e in current_rows(c,config)},
                          'fallback_providers':config.get('fallback_providers') or []},indent=2))
        return 0
    return screen(c)
...
def apply_job_models(c,run=None):
    ...
    if run is None:
        def run(args):
            result=subprocess.run(cp.hermes_command(*args),capture_output=True,text=True,timeout=60,
                                  env={**os.environ,'HERMES_HOME':str(c.home)})
            if result.returncode:raise ValueError(result.stderr or 'Hermes cron edit failed')
            return result
    ...
    with cp.file_lock(c.home/'.companion-jobs.lock'):
        for job in _read_jobs(c.home/'cron/jobs.json')['jobs']:
            ...
            if not spec or spec.get('no_agent'):continue
    ...
    return {'updated':updated,'note':'Job IDs, run history, and enabled state are preserved.'}
