...
def cmd_catalog(args):
    ...
    elif key=='boundaries':rows=[(v[0],v[2]) for v in BOUNDARIES.values()]
    elif key:rows=options(key,gender)
    else:
        rows=[(v['label'],f"{len(v[gender])} {gender} choices") for v in cat.values() if gender in v]
        rows.extend([('Personas',str(len(cr.load_personas()))),('Image styles',str(len(cr.load_styles())-2)+' plus text-only and undecided'),('Boundaries',str(len(BOUNDARIES)))])
    wiz.banner('Character catalog',f'{gender.capitalize()} · '+(key or 'all categories'))
    ...
    for i,(label,detail) in enumerate(rows,1):
        print(wiz.C.cyan(f'  {i:2}. ')+wiz.C.bold(fill(label,'<<agent>>','<<user>>',pron,pron)))
        detail=fill(detail,'<<agent>>','<<user>>',pron,pron)
        for line in textwrap.wrap(detail,wiz.width()-6):print('      '+wiz.C.dim(line))
        print()
    return 0
