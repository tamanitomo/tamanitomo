...
def ask(prompt,default='',choices=None,answers=None,key=None):
    ...
    if choices and wiz._tty():
        return wiz.choose(prompt,[(label,val) for val,label in choices],allow_write=False,allow_skip=False,default=int(default or 1))
    while True:
        if choices:
            print(f'\n{prompt}')
            ...
            if not raw:raw=str(default or 1)
            if raw.isdigit() and 1<=int(raw)<=len(choices):return choices[int(raw)-1][0]
            print('  -> pick a number from the list')
            continue
        raw=input(f'{prompt}'+(f' [{default}]' if default else '')+': ').strip()
        if raw or default:return raw or default
        print('  -> required')
def pick_key(prompt,pairs,answers=None,key=None,default=1,note=''):
    ...
    got=wiz.choose(prompt,[(label,k) for k,label in pairs],answers,key,
                   allow_write=False,allow_skip=False,default=default,note=note)
    if got not in keys:raise ValueError(f'Invalid choice for {key}: {got}')
    return got
...
def questionnaire(c,answers=None,vault_default=None):
    ...
    if isinstance(a.get('cron_active'),bool):a={**a,'cron_active':'active' if a['cron_active'] else 'paused'}
    if isinstance(a.get('share_people'),bool):a={**a,'share_people':'yes' if a['share_people'] else 'no'}
    personas=cr.load_personas();styles=cr.load_styles()
    out={}
    if wiz._tty():
        wiz.clear()
        wiz.banner('Create your companion','Identity · Appearance · Relationship · Everyday life · Setup')
        print(wiz.C.dim('Skip optional story questions; review everything before applying.'))
    # First, because it decides what the rest of these questions mean — and what
    # machinery gets installed at all.
    out['agent_type']=ask('What kind of agent is this?',1,
        [('companion','A companion — a life of their own, moods, a relationship, may write first'),
         ('colleague','A colleague — a personality that grows and remembers you, but never reaches '
                      'out socially; it speaks first only when something is broken'),
         ('worker','A quiet worker — does the work, keeps records, no relationship layer at all')],
        a,'agent_type')
    ...
    if wiz._tty():wiz.rule('05 / 05   Daily life · Time, contact and memory')
    ...
    if 'image_style' in a:
        out['image_style']=pick_key('If you make images of them, what style?',
            [(k,f"{v['label']} — {v['blurb']}") for k,v in styles.items()],a,'image_style')
        ...
        out['image_style']=pick_key('If you make images of them, what style?',
            [(k,f"{v['label']} — {v['blurb']}") for k,v in styles.items() if k not in ('none','unset')]
            +[('none',styles['none']['label']+' — '+styles['none']['blurb']),
              ('unset','Skip: '+styles['unset']['label'])],a,'image_style',
            note='Consistency matters more than the style itself; the same look every time.')
    timeline_answer=a.get('image_timeline',False)
    if 'image_timeline' in a:out['image_timeline']=wiz.as_bool(timeline_answer)
    else:
        out['image_timeline']=wiz.choose('Keep a visual timeline of their day?',[
            ('No automatic images — everyday life and outfits still continue','off'),
            ('One local image every 15 minutes — retain 30 days','on')],allow_write=False,allow_skip=False,
            note='Uses your configured Hermes image provider: up to 96 images/day, including overnight. Images stay local. Copy favorites elsewhere to keep them.',default=1)=='on'
    if out['image_timeline'] and out['image_style'] in ('none','unset'):
        if not wiz._tty():raise ValueError('Choose an image style before enabling the image timeline')
        out['image_style']=pick_key('Choose a style for your timeline images',
            [(k,f"{v['label']} — {v['blurb']}") for k,v in styles.items() if k not in ('none','unset')])
    if out['agent_type']=='worker':
        ...
        out['outreach']=ask('May they message you first?',2,
            [('free','Yes — socially, whenever they want to'),
             ('updates_only','Only with something real to report or ask'),
             ('never','No, replies only')],a,'outreach')
    out['outreach_per_day']=0 if out['outreach']=='never' else ask_outreach_cap(a)
    out['cron_active']=ask('Start the scheduled background routine after setup?',2,
        [('active','Yes — authorize these recurring jobs'),('paused','Create paused (recommended); enable after hook approval and gateway restart')],a,'cron_active')=='active'
    out['quiet_start']=hhmm(ask('Start of your do-not-disturb hours (HH:MM)','23:00',None,a,'quiet_start'),'23:00')
    out['quiet_end']=hhmm(ask('End of your do-not-disturb hours (HH:MM)','08:00',None,a,'quiet_end'),'08:00')
    permissions={}
    if out['outreach']!='never':
        out['adaptive_quiet']=confirm(
            'Let those hours drift toward the ones you actually keep?',a,'adaptive_quiet',False)
        if wiz._tty():
            print(wiz.C.dim('  If you consistently write inside the window for a fortnight, it moves by half an\n'
        ...
        for kind,question,default in (
            ...
            permissions[kind]=ask(question,default,
                [('yes','Yes, whenever it fits'),
                 ('ask','They may mention it and offer; not attach it unasked'),
                 ('no','No')],a,'permit_'+kind)
    out['content_permissions']=permissions
    out['share_people']=ask('Should other agents here share what they learn about you?',1,
        [('yes','Yes — one shared record of me, so anything one learns, they all know'),
         ('no','No — each one gets to know me from scratch')],a,'share_people')=='yes'
    out['location']=ask('Where do you live? (a place name, for weather and daylight; blank to skip)',
                        '',None,a,'location').strip()[:120]
    out['sensors']=ask_sensors(a,bool(out['location']))
    import yaml
    existing_cfg=yaml.safe_load((c.home/'config.yaml').read_text(encoding='utf-8')) or {} if (c.home/'config.yaml').exists() else {}
    if not isinstance(existing_cfg,dict):raise ValueError('config.yaml must be a mapping')
    detected_timezone=existing_cfg.get('timezone') or os.environ.get('TZ') or c.timezone
    if a and 'timezone' in a:out['timezone']=str(a['timezone'])
    elif not wiz._tty():out['timezone']=detected_timezone
    else:out['timezone']=pick_timezone(detected_timezone)
    from zoneinfo import ZoneInfo
    ZoneInfo(out['timezone'])

    vdef=str(vault_default or c.vault or (pathlib.Path.home()/'vault'))
    sub=(pathlib.Path(vdef)/'agents'/c.profile) if c.profile else pathlib.Path(vdef)
    if wiz._tty():
        print('\n'+wiz.C.bold('Shared vault')+wiz.C.dim(' — the one folder every agent keeps its life in.'))
        print(wiz.C.dim(f'  {out["agent"]} gets its own subfolder; you are not naming that part.'))
    ...
    if 'context_tokens' in a:
        ...
        print(f"  -> {cc.Companion(context_tokens=tokens).budgets()['total']} chars of continuity per turn.")
        if wiz._tty() and not confirm('  Does that look right?',a,'context_ok',True):
            out['context_mode']='fixed'
            out['context_tokens']=int(ask('Context window in tokens',str(tokens),None,a,'context_tokens'))
    if not 2048<=out['context_tokens']<=10_000_000:raise ValueError('Context window must be between 2,048 and 10,000,000 tokens')
    for key in ('agent','human'):
        if not out[key].strip() or len(out[key])>100 or any(ord(ch)<32 for ch in out[key]) or '{{' in out[key]:
...
def ask_sensors(answers=None,has_location=False):
    ...
    if answers and 'sensors' in answers:
        ...
        if isinstance(chosen,str):chosen=[x.strip() for x in chosen.split(',') if x.strip()]
        return [name for name in companion_sensors.REGISTRY if name in (chosen or [])]
    default=[name for name in companion_sensors.REGISTRY
             if name!='weather' or has_location]
    if not wiz._tty():return default
    print('\n'+wiz.C.bold('Senses')+wiz.C.dim(' — cheap, optional, and silent when they have nothing.'))
    chosen=[]
    for name,spec in companion_sensors.REGISTRY.items():
        if name=='weather' and not has_location:
            print(wiz.C.dim(f'  weather — skipped, since no location was given'));continue
        print()
        print('  '+wiz.C.bold(name)+wiz.C.dim(' — '+spec['blurb']))
        if confirm('  Switch it on?',None,None,True):chosen.append(name)
    return chosen
...
def apply_answers(c,ans):
    return cc.Companion(agent=ans['agent'],pronoun_set=ans['pronoun_set'],human=ans['human'],
        ...
        context_tokens=int(ans['context_tokens']),context_mode=ans.get('context_mode','auto'),image_mode=ans['image_mode'],
        image_style=ans['image_style'],image_timeline=ans.get('image_timeline',False),boundary=ans['boundary'],outreach=ans['outreach'],
        outreach_per_day=int(ans.get('outreach_per_day',3)),
        names=list(ans.get('names') or []),age=int(ans.get('age',24)),
...
SETUP_KEYS=frozenset({'agent','human','human_names','persona','image_mode','image_style',
...
def parse_cap(value):
    ...
    if isinstance(value,bool) or value is None:raise ValueError('Give an explicit numeric cap or no limit')
    text=str(value).strip().lower()
    if text in ('0','no limit','unlimited'):return 0
    match=re.fullmatch(r'([0-9]{1,3})(?:\s*(?:a day|/day|per day|(?:times?|messages?)(?: (?:a|per) day)?))?',text)
    if not match:raise ValueError(f'{value!r} is not a daily message cap; use 0 for no limit or replies-only for no outreach')
    n=int(match.group(1))
    if not 0<=n<=100:raise ValueError('pick between 0 and 100 messages a day')
    return n
def ask_outreach_cap(answers=None):
    ...
    while True:
        got=wiz.choose('How many times a day may they message you first?',OUTREACH_CAPS,
            ...
            write_label='some other number',write_hint='messages a day (0 for no limit)',
            note='Enforced in code, outside the model, along with your do-not-disturb hours.')
        try:return parse_cap(got)
        except ValueError as e:print(wiz.C.red('  '+str(e)))
def read_answers(args):
    ...
    if not isinstance(raw,dict):raise ValueError('--answers must be a JSON object')
    unknown=sorted(set(raw)-known_answer_keys())
    if unknown:
        raise ValueError('Unknown answer key(s): '+', '.join(unknown)+
    ...
    if getattr(args,'vault',None):raw['vault']=str(args.vault)
    return raw
def strict_hhmm(value):
    got=hhmm(value,'')
    if not got:raise ValueError(f'{value!r} is not a time of day; use HH:MM, e.g. 23:00')
    return got
def check_timezone(name):
    from zoneinfo import ZoneInfo,ZoneInfoNotFoundError
    try:ZoneInfo(name)
    except (ZoneInfoNotFoundError,ValueError,OSError) as exc:
        raise ValueError(f'{name!r} is not an IANA timezone; use one like Europe/London') from exc
    return name
...
US_TIMEZONES=[('America/New_York','Eastern — New York'),
...
def pick_timezone(current=''):
    ...
    got=pick_key('Timezone',rows,note='Used for quiet hours, scheduled jobs and the companion’s own clock.')
    if got!=ELSEWHERE:return got
    regions=zones_by_region()
    region=pick_key('Which region?',[(name,f'{name} ({len(zones)})') for name,zones in sorted(regions.items())])
