...
def collect(c, now=None):
    ...
    return {
        ...
        "state_age": (
            context.age_phrase(dt.datetime.fromisoformat(scene["recorded_at"]), now)
            if scene
            else None
        ),
        "loops": loops.loops(c),
...
def render(c, data):
    ...
    if not scene:
        ...
        out.append(
            f"  right now   {state.get('activity','')} at {state.get('location','')}"
            f"   ({data['state_age']}{'' if confirmed else ', UNCONFIRMED'})"
        )
        out.append(f"  mood        {state.get('mood','')}")
        if state.get("wants"):
            out.append("  wants       " + "; ".join(state["wants"]))
        if not confirmed and data["confirmed_at"]:
            out.append(
                f"  last seen   a model last confirmed the present at {data['confirmed_at'][:16]}"
            )
    loops = data["loops"]
    out.append(
        f"  open loops  {len(loops)}"
        + (": " + "; ".join(l["title"] for l in loops[:3]) if loops else "")
    )
    active = [j for j in data["jobs"] if j.get("enabled")]
    failing = [j for j in data["jobs"] if j.get("last_status") == "error"]
    out.append(
        f"  jobs        {len(active)} of {len(data['jobs'])} running"
    ...
    for job in data["jobs"]:
        ...
        out.append(
            f"    {mark:<7}{job['name'][len(c.agent)+1:]:<30.29}{last:<12}next {job.get('next_run_at') or '—'}"
        )
    if data["problems"]:
        out.append("")
        out.append("  Not well:")
