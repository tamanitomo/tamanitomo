...
def menu(hermes_root):
    while True:
        ...
        opts = []
        if unset:
            opts.append(("adopt", "Set up an existing agent"))
        opts += [
            ("manage", "Manage an installed companion"),
            ("add", "Create a new agent"),
            ("catalog", "Explore character options"),
            ("doctor", "Check an agent"),
            ("remove", "Remove an agent"),
            ("quit", "Quit"),
        ]
        descriptions = {
            "manage": "Quiet hours, how often they message first, timezone, images.",
            "catalog": "Browse the wardrobe, personality and art catalogs.",
            "adopt": "Preserve an identity and add continuity.",
            "add": "Build a personality in a separate Hermes profile.",
            "doctor": "Check memory, hooks and scheduled jobs.",
            "remove": "Archive a profile; keep its vault data.",
            "quit": "Return to your terminal.",
        }
        roster = "\n".join(
            f"{r['agent']} ({r['name']}) — "
            + ("installed" if r["set_up"] else "ready to set up")
            for r in rows
        )
        action = wiz.choose(
            "Companion setup",
            [(label + " — " + descriptions[key], key) for key, label in opts],
            allow_write=False,
            allow_skip=False,
            note=roster,
        )
        try:
            ...
            if action == "catalog":
                ...
                name = input(
                    "\n  " + wiz.C.bold("Short name for the new agent (lowercase): ")
                ).strip()
                if name:
                    cmd_add(
                        argparse.Namespace(
                            home=hermes_root, name=name, answers=None, vault=None
                        )
                    )
            elif action == "manage":
                r = menu_pick(rows, "Which companion?", only_set_up=True)
                if r:
                    settings_screen(cc.load(r["home"]))
            elif action == "doctor":
                r = menu_pick(rows, "Which agent?", only_set_up=True)
                if r:
                    cmd_doctor(argparse.Namespace(home=r["home"]))
            elif action == "remove":
                ...
                if r and r["is_root"]:
                    ...
                    cmd_remove(
                        argparse.Namespace(
                            home=hermes_root, name=r["name"], force=True, purge=False
                        )
                    )
                elif r:
                    print("\n  " + wiz.C.dim("  cancelled."))
...
def cmd_menu(args):
    ...
    if not cp.is_terminal(sys.stdin):
        for r in agent_rows(root):
            print(
                f"{r['name']}\t{r['agent']}\t{'set-up' if r['set_up'] else 'not-set-up'}"
            )
        return 0
    return menu(root)
