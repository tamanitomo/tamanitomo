...
def cmd_schedule(args):
    ...
    jobs = [
        j
        for j in _read_jobs(c.home / "cron/jobs.json")["jobs"]
    ...
    if args.state == "status":
        for j in jobs:
            ...
            print(
                f"  next: {j.get('next_run_at') or 'not scheduled'} · last: {j.get('last_status') or 'not run yet'}"
            )
        return 0
    if len(jobs) != len(names) or {j["name"] for j in jobs} != names:
        raise ValueError("Incomplete or duplicate job set; run doctor and repair first")
    action = "resume" if args.state == "active" else "pause"
    scripted = script_job_names(c)
    for job in jobs:
        if job["name"] in scripted:
            continue
        result = subprocess.run(
            cp.hermes_command("cron", action, job["id"]),
            capture_output=True,
            text=True,
            encoding="utf-8",
            timeout=60,
        ...
        if result.returncode:
            raise ValueError(
                f"Could not {action} {job['name']}; inspect schedule status before retrying"
            )
        print(f"{args.state}: {job['name']}")
    c.cron_active = args.state == "active"
    c.save()
    print(
        "Jobs that run without a model stay active: they cost nothing and they are what keeps the\npresent honest and retention enforced while the schedule is paused."
    )
    print(
        "This changes recurring scheduling only; it does not grant tool permissions or start a gateway."
    )
    return 0
