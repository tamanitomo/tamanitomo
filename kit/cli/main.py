    ...
    for name, fn in (
        ...
        if name == "pin":
            s.add_argument(
                "--set",
                metavar="PIN",
                help="Set a 4-digit PIN for remote network access",
            )
            s.add_argument(
                "--clear",
                action="store_true",
                help="Clear/remove the remote PIN (unlocks network access)",
            )
        if name == "backup":
            s.add_argument(
                "--output",
                "-o",
                metavar="PATH",
                help="Custom output path or directory for the zip backup",
            )
        if name == "timeline":
            s.add_argument(
                "state",
                choices=["status", "on", "off", "prune"],
                nargs="?",
                default="status",
            )
        if name == "gateway":
            ...
            s.add_argument(
                "--action",
                choices=["status", "setup", "install", "start", "restart", "preflight"],
            )
            s.add_argument(
                "--root-restarted",
                action="store_true",
                help="Confirm the existing root gateway was restarted after routing changes",
            )
        if name == "catalog":
            ...
            s.add_argument(
                "--category",
                choices=list(load_catalog()["categories"])
                + ["personas", "images", "boundaries"],
            )
        if name == "app":
            s.add_argument(
                "--host",
                default="0.0.0.0",
                help="Listening address (default: local network; use 127.0.0.1 for host-only access)",
            )
            s.add_argument("--port", type=int, default=8770)
            s.add_argument(
                "--token",
                default="",
                help="require this token; generated if you bind wider",
            )
            s.add_argument(
        ...
        if name == "identity":
            s.add_argument(
                "section", nargs="?", help="which section; omit to list them"
            )
            s.add_argument(
                "--rerender",
                action="store_true",
        ...
        if name == "models":
            s.add_argument(
                "--show", action="store_true", help="print the current choices as JSON"
            )
            s.add_argument(
                "--apply",
                action="store_true",
                help="apply model choices to existing jobs without recreating them",
            )
        if name == "repair":
            s.add_argument(
                "--prompts",
        ...
        if name == "schedule":
            s.add_argument(
                "state",
                choices=["status", "active", "paused", "history"],
                nargs="?",
                default="status",
            )
        if name == "settings":
            s.add_argument(
                ...
                action="append",
                metavar="KEY=VALUE",
                help="non-interactive change, repeatable: "
                + ", ".join(sorted(SETTINGS_KEYS)),
            )
        if name in ("add", "remove"):
            s.add_argument("name")
        if name in ("init", "add", "upgrade"):
            s.add_argument("--answers", help="JSON, for non-interactive setup")
        if name in ("init", "upgrade"):
            s.add_argument("--force", action="store_true")
        if name in ("init", "add", "upgrade"):
            s.add_argument("--vault", help="data root (default ~/vault)")
        if name == "upgrade":
            s.add_argument("--soul", choices=["append", "replace", "keep"])
        if name == "remove":
            s.add_argument(
                "--force", action="store_true", help="required; archives the profile"
            )
            s.add_argument(
                "--purge", action="store_true", help="delete instead of archiving"
            )
    a = p.parse_args()
...
def run():
    ...
    try:
        ...
        print(
            "Missing dependency. Install requirements.txt using the Python interpreter "
            "that runs this kit.",
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        print(
            "Invalid YAML configuration; setup stopped. Correct config.yaml and retry.",
            file=sys.stderr,
        )
        sys.exit(1)
