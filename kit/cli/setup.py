...
def cmd_init(args):
    ...
    if (c.home / cc.CONFIG_NAME).exists() and not args.force:
        sys.exit(
            f"{c.home} is already a companion — use `tamanitomo doctor`, or --force to re-scaffold"
        )
    ...
    if getattr(args, "vault", None):
        c = cc.dataclasses.replace(
            c, vault=pathlib.Path(args.vault).expanduser().absolute()
        )
...
def hermes_create_profile(name, hermes_root, report, description=""):
    ...
    if os.environ.get("COMPANION_APP_CLIENT") == "1":
        cmd += ["--no-alias"]
    if description:
    ...
    if r.returncode == 0:
        report.append(
            f"  ! `hermes profile create` reported success but {expected} does not exist;"
        )
...
def cmd_add(args):
    ...
    if home.exists():
        sys.exit(f"{name} already exists at {home}; use upgrade to adopt it")
    ...
    if c.canonical_soul.exists():
        raise ValueError(
            f"A vault identity already exists at {c.canonical_soul}; resolve it before reusing this profile name"
        )
    if not home.exists():
        if not hermes_create_profile(
            name, base.hermes_root, report, f"{ans['agent']}: companion agent"
        ):
            raise ValueError(
                "Hermes profile creation failed; no companion scaffold was written. Check your Hermes installation."
            )
    scaffold(c, ans, report, soul_mode="create", raw=raw)
    finish(c, report)
...
def removal_plan(home):
    ...
    if not (home / cc.CONFIG_NAME).exists():
    ...
    inside = None
    try:
        c.data.resolve().relative_to(home.resolve())
        inside = c.data
    except (ValueError, OSError):
        pass
...
def removal_screen(name, agent, home, data, inside, purge):
    ...
    if data:
        print(
            ...
            + wiz.C.dim("its life  ")
            + str(data)
            + ("" if inside else "   " + wiz.C.dim("(outside the profile)"))
        )
    print()
    if purge:
        print(
            "  "
            + wiz.C.red(wiz.C.bold("PURGE — this deletes the profile permanently."))
        )
        print(
            "  " + wiz.C.red("  Its SOUL, memories, sessions and ledgers go with it.")
        )
        ...
        print(
            "  "
            + wiz.C.yellow(
                wiz.C.bold("ARCHIVE — the profile is moved aside, not deleted.")
            )
        )
        print(
            "  "
            + wiz.C.dim(
                "  You can move it back, or delete it yourself once you are sure."
            )
        )
...
def cmd_remove(args):
    ...
    home = cp.profile_path(base.hermes_root, name)
    if not home.exists():
        sys.exit(f"no profile at {home}")
    if not args.force:
        sys.exit(
            f"refusing to remove {name} without --force.\n"
            f"  This holds its SOUL, memories, sessions and ledgers at {home}"
        )
    data, vault, inside = removal_plan(home)
    agent = ""
    try:
        agent = cc.load(home).agent
    except (ValueError, OSError):
        pass
    if inside is not None:
        sys.exit(
            f"refusing to remove {name}: its vault data lives inside the profile.\n"
            f"  {inside}\n"
    ...
    if args.purge and cp.is_terminal(sys.stdin):
        typed = input(
            "  "
            + wiz.C.bold(
                f"Type {name} to purge it permanently, or anything else to cancel: "
            )
        ).strip()
        if typed != name:
            print("\n  " + wiz.C.dim("cancelled. Nothing was removed."))
            return 1
    stamp = dt.datetime.now().strftime("%Y%m%dT%H%M%S")
    dest = (
        base.hermes_root / "profiles-removed" / f"{name}-{stamp}-{uuid.uuid4().hex[:8]}"
    )
    ...
    if args.purge:
        ...
        print(
            f"archived {name} -> {dest}\nNothing was deleted. Remove that directory yourself when sure."
        )
    if data:
        print(f"its life is still at {data}")


def cmd_upgrade(args):
    if not args.home and not os.environ.get("COMPANION_HOME"):
        root = pathlib.Path(cp.default_home())
        args.home = pick_agent(
            root, json.loads(args.answers) if args.answers else None, "upgrade"
        )
    c = resolve(args)
    if not c.soul.exists():
        sys.exit(
            f"no SOUL.md at {c.soul} — this does not look like a Hermes agent.\n"
    ...
    print(
        f'Its SOUL.md is {len(c.soul.read_text(encoding="utf-8"))} chars and will NOT be overwritten by default.'
    )
    shared = existing_vault(c.hermes_root, exclude=c.home)
    raw = read_answers(args)
    ans = questionnaire(c, raw, vault_default=shared)
    mode = args.soul or ask(
        "What should happen to the existing SOUL.md?",
        3,
        [
            ...
            (
                "keep",
                "Keep your SOUL text (recommended; only the self-authored block marker is added)",
            ),
        ],
        json.loads(args.answers) if args.answers else None,
        "soul",
    )
    c = apply_answers(c, ans)
    if args.vault:
    ...
    scaffold(
        ...
        ans,
        report,
        soul_mode={"append": "append", "replace": "replace", "keep": "keep"}[mode],
        raw=raw,
    )
    extra = []
    if mode == "append":
        extra.append(
            (
                "Merge the appended scaffold into your original SOUL by hand — two identities "
                "in one file will read as an inconsistent character",
                c.soul,
            )
        )
