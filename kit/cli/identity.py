...
def cmd_identity(args):
    ...
    if not args.section:
        ...
        if not rows:
            print(
                "This SOUL has no section markers — it predates them, or was rewritten by hand."
            )
            print(
                wiz.C.dim(
                    "  Everything still works; only per-section re-rendering needs them."
                )
            )
            return 1
        ...
        print(
            wiz.C.dim(
                "  locked = not editable by the companion. You can still edit the file yourself."
            )
        )
        print(wiz.C.dim("  companion identity <section>            show it"))
        print(
            wiz.C.dim("  companion identity <section> --rerender rebuild just that one")
        )
        return 0
    if not args.rerender:
        ...
        if args.section not in found:
            raise ValueError(
                f'no section {args.section!r}; known: {", ".join(sorted(found))}'
            )
    ...
    print(
        wiz.C.dim(
            "  every other section is byte-for-byte unchanged; that is verified, not assumed"
        )
    )
    return 0
