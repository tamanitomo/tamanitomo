...
def ensure_vault_gitignore(vault,report=None):
    ...
    try:
        ...
        with cp.file_lock(path.with_name('.gitignore.companion.lock')):
            ...
            block+=newline+'# companion-kit: editor and sync artifacts'+newline+newline.join(missing)+newline
...
def _create_job_via_hermes(c,name,expr,prompt,toolsets=(),script=None,spec=None,report=None):
    ...
    if not c.cron_active and not script:cmd+=['--paused','--paused-reason','Paused by companion-kit setup choice']
...
def write_preread_script(c,spec,mode):
    ...
    cp.atomic_write(path,'"""Generated by companion-kit. Prints context; calls no model."""\n'
        'import runpy,sys\n'
...
def write_job_script(c,spec):
    ...
    cp.atomic_write(path,'"""Generated by companion-kit. Runs one kit helper with no model."""\n'
        'import runpy,sys\n'
        f'sys.path.insert(0,{str(KIT/"kit/scripts")!r})\n'
        f'sys.argv={argv!r}\n'
        f'runpy.run_module({module!r},run_name="__main__")\n')
...
def install_hook(c,m,report):
    ...
    legacy=old.exists() and 'companion-kit continuity hook' in old.read_text(encoding='utf-8')
    cleaned=[]
    for entry in pre:
    ...
    source=("# companion-kit continuity hook; generated for this profile.\n"
    ...
    end_source=("# companion-kit session-end flag; generated for this profile.\n"
        "import json, os, pathlib, runpy, sys\n"
        f"home=pathlib.Path({str(c.home)!r}).resolve()\n"
        "active=pathlib.Path(os.environ.get('HERMES_HOME',str(home))).expanduser().resolve()\n"
...
def offer_multiplex(c,report,answers=None):
    ...
    if action!='later':
        ...
        if code:report.append(f'  ! gateway {action} returned {code}; run companion gateway to resolve it')
    report.append('  gateway controls: companion --home "'+str(c.home)+'" gateway')
...
SOUL_MARKER='Generated by companion-kit'
...
def scaffold(c,ans,report,soul_mode='create',raw=None):
    ...
    if soul_mode=='keep':
        ...
        banner=('\n\n---\n\n<!-- companion-kit appended the scaffold below on '
