"""Install profile files and separate maintenance from inference jobs.

Quiet rhythms must keep working without a provider, token budget or model
interpretation. The cron manifest declares six maintenance routines with
``no_agent``: timeline_cleanup, watch, dispatch, sensors, quiet and vault.
Their generated wrappers run the named Python helper with ``--script`` and
``--no-agent``; no language-model turn is needed to prune files, check health,
deliver already-written messages, read sensors, adjust an opted-in quiet window
or commit the local vault. Availability still depends on each helper's actual
filesystem, device or network dependencies.

The manifest also declares rollover as a seventh no-agent session-lifecycle
job. It preserves old transcripts and flags reflection work rather than doing
that reflection itself. Jobs that author presence or reflections remain a
separate inference path. Keeping this distinction in job configuration, rather
than asking an agent to do nothing, prevents routine ticks from consuming
model calls or failing solely because inference is unavailable.
"""
...
def ensure_vault_gitignore(vault, report=None):
    ...
    try:
        if path.is_symlink():
            raise ValueError(
                "vault .gitignore is a symlink; refusing to edit another target"
            )
        with cp.file_lock(path.with_name(".gitignore.companion.lock")):
            ...
            missing = [
                rule
                for rule in (*VAULT_IGNORES, "/.gitignore.companion.lock")
                if rule not in have
            ]
            if not missing:
                return False
            ...
            block += (
                newline
                + "# tamanitomo: editor and sync artifacts"
                + newline
        ...
        if report is not None:
            report.append("  ! could not update vault .gitignore: " + str(exc))
        return False
    if report is not None:
        report.append(f"  vault .gitignore: {len(missing)} rule(s) added ({path})")
    return True
...
def hermes_supports(command, home, flag):
    ...
    if key in SUPPORTED_FLAG_CACHE:
        return SUPPORTED_FLAG_CACHE[key]
    try:
        r = subprocess.run(
            list(command) + ["cron", "create", "--help"],
            capture_output=True,
...
def _create_job_via_hermes(
    c, name, expr, prompt, toolsets=(), script=None, spec=None, report=None
):
    ...
    """
    if os.environ.get("COMPANION_NO_HERMES_CRON", "").lower() in ("1", "true", "yes"):
        return False, False
    spec = spec or {}
    report = report if report is not None else []
    cmd = cp.hermes_command(
        "cron", "create", expr, prompt, "--name", name, "--deliver", "local"
    )
    ...
    if script:
        cmd += ["--script", pathlib.Path(script).name, "--no-agent"]
    if not script:
        ...
        if pick.get("base_url"):
            if not hermes_supports(base, c.home, "--base-url"):
                raise ValueError("Update Hermes to use custom job endpoints")
        ...
        if spec.get("preread"):
            ...
            if hermes_supports(base, c.home, "--script"):
                cmd += ["--script", path.name]
        if spec.get("monitor"):
            path = write_preread_script(c, spec, "fingerprint")
            if hermes_supports(base, c.home, "--monitor-script"):
                cmd += ["--monitor-script", path.name]
    if not c.cron_active and not script:
        cmd += ["--paused", "--paused-reason", "Paused by tamanitomo setup choice"]
    env = {
        **os.environ,
    ...
    flag = (
        cron_toolset_flag(tuple(cp.hermes_command()), str(c.home)) if toolsets else None
    )
    if flag:
        cmd += [flag, ",".join(toolsets)]
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, encoding="utf-8", timeout=180, env=env
        )
    except (OSError, subprocess.SubprocessError):
        return False, False
...
def seed_open_loops(c, report):
    ...
    if loops.loops(c, None):
        return
    soul = c.soul.read_text(encoding="utf-8") if c.soul.exists() else ""
    if not edit_count(soul):
        return
    try:
        loops.add(
            c,
            {
                "title": f"Finish {c.agent}'s SOUL.md",
...
def write_preread_script(c, spec, mode):
    ...
    """
    module = (
        spec.get("monitor_module", "companion_preread")
        if mode == "fingerprint"
        else "companion_preread"
    )
    path = c.home / "scripts" / f"companion-{spec['key'].replace('_','-')}-{mode}.py"
    argv = [module + ".py", "--home", str(c.home), mode]
    cp.atomic_write(
        path,
        '"""Generated by tamanitomo. Prints context; calls no model."""\n'
        "import runpy,sys\n"
...
def write_job_script(c, spec):
    ...
    args = [str(a) for a in (detail.get("args") or [])]
    if not module:
        return None
    ...
    cp.atomic_write(
        path,
        '"""Generated by tamanitomo. Runs one kit helper with no model."""\n'
        "import runpy,sys\n"
        f'sys.path.insert(0,{str(KIT/"kit/scripts")!r})\n'
        f"sys.argv={argv!r}\n"
        f'runpy.run_module({module!r},run_name="__main__")\n',
...
def _install_jobs_locked(c, m, report):
    ...
    for spec in load_manifest(c)["jobs"]:
        ...
        if name in existing:
            ...
            if (
                not spec.get("no_agent")
                and job.get("no_agent")
                and legacy_script.startswith("companion-local-")
            ):
                rendered = cr.render(template, m)
                backups = c.home / "cron/prompt-backups"
                backups.mkdir(parents=True, exist_ok=True)
                stamp = dt.datetime.now().strftime("%Y%m%dT%H%M%S")
                (backups / f"{name}-{stamp}-legacy-worker.md").write_text(
                    job.get("prompt", ""), encoding="utf-8"
                )
                ...
                if spec.get("monitor"):
                    ...
                    updates += ["--monitor-script", ""]
                if spec.get("continuity"):
                    updates += ["--continuity"]
                result = subprocess.run(
                    cp.hermes_command("cron", "edit", job["id"], *updates),
                    capture_output=True,
                    text=True,
                    encoding="utf-8",
                    timeout=60,
                ...
                if result.returncode:
                    report.append(
                        f"  ! could not migrate {name} away from its legacy provider-pinned worker"
                    )
                else:
                    job = {
                        **job,
            ...
            for marker in (
                ...
                if marker not in job.get("prompt", ""):
                    ...
                    if check:
                        additions.append(cr.render(check, m))
            updates = []
            if additions:
                prompt = "\n\n".join(additions) + "\n\n" + job.get("prompt", "")
            ...
            if (
                not spec.get("no_agent")
                and spec.get("preread")
                and legacy_pre.startswith("companion-local-")
                and legacy_pre.endswith("-preread.py")
            ):
                old_file = c.home / "scripts" / legacy_pre
                if old_file.exists():
                    keep = c.home / "scripts/legacy-backup"
                    keep.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(old_file, keep / legacy_pre)
                updates += ["--script", write_preread_script(c, spec, "preread").name]
                report.append(
                    f"  {name}: legacy pre-read {legacy_pre} replaced (backed up in scripts/legacy-backup)"
                )
            if (
                spec["key"] == "pulse"
                and job.get("schedule", {}).get("expr") == "*/15 6-23 * * *"
            ):
            ...
            if spec["key"] == "wake" and job.get("schedule", {}).get(
                "expr"
            ) == cr.cron_at(c.quiet_end, offset=10):
            ...
            if (
                ...
                and not job.get("monitor_script")
                and hermes_supports(
                    tuple(cp.hermes_command()), c.home, "--monitor-script"
                )
            ):
                updates += [
                    "--monitor-script",
            ...
            if updates:
                try:
                    ...
                    if result.returncode:
                        raise ValueError("Hermes cron edit failed")
        ...
        created, applied = _create_job_via_hermes(
            c, name, expr, prompt, toolsets, script=script, spec=spec, report=report
        )
        if created:
            made += 1
            record_fingerprint(c, name, prompt)
            if toolsets and not applied:
                without_toolsets.append(name)
        else:
            pending.append(
                {"name": name, "expr": expr, "prompt": prompt, "toolsets": toolsets}
            )
    if not c.image_timeline:
        job = existing.get(c.agent + " image timeline")
        if job and job.get("enabled"):
            result = subprocess.run(
                cp.hermes_command("cron", "pause", job["id"]),
                capture_output=True,
                text=True,
                timeout=30,
            ...
            if result.returncode:
                report.append("  ! could not pause image timeline; retry repair")
    staging = c.home / "companion-pending-jobs.json"
    if pending:
        cp.atomic_write(staging, json.dumps(pending, ensure_ascii=False, indent=2))
        report.append(
            f"  ! cron: {len(pending)} pending; Hermes CLI failed or was disabled. Run repair after fixing Hermes."
        )
    elif staging.exists():
        staging.unlink()
    ...
    if without_toolsets:
        report.append(
            f"  ! this Hermes exposes no supported per-job toolset flag; {len(without_toolsets)} job(s) "
...
def _registered(entries, hook: pathlib.Path, cmd: str) -> str:
    ...
    for entry in entries or []:
        ...
        if not (value and _runs_hook(value, hook)):
            continue
        try:
            interpreter = shlex.split(value, posix=os.name != "nt")[0].strip('"')
        except (ValueError, IndexError):
            continue
        if pathlib.Path(interpreter).is_file() or shutil.which(interpreter):
            return value
    return cmd
...
def read_fingerprints(c):
    try:
        return json.loads((c.home / FINGERPRINT_FILE).read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return {}


def record_fingerprint(c, name, prompt):
    data = read_fingerprints(c)
    data[name] = _fingerprint(prompt)
    path = c.home / FINGERPRINT_FILE
...
def _edit_prompt(c, job, prompt):
    result = subprocess.run(
        cp.hermes_command("cron", "edit", job["id"], "--prompt", prompt),
        capture_output=True,
        text=True,
        encoding="utf-8",
        timeout=60,
...
def refresh_templates(c, m, report, force=False):
    ...
    for spec in load_manifest(c)["jobs"]:
        ...
        if current.strip() == rendered.strip():
            if stored.get(name) != _fingerprint(rendered):
                record_fingerprint(c, name, rendered)
            continue
        import hashlib
        ...
        if not (unedited or force):
            why = (
                "it was edited after the kit wrote it"
                if stored.get(name)
                else "this kit never recorded writing it, so it cannot be told from your own edits"
            )
            report.append(
                f"  ! {name} runs an older prompt and {why}; it was left alone. "
        ...
        try:
            ...
            record_fingerprint(c, name, rendered)
            report.append(
                f"  {name}: prompt re-rendered from the current template"
                + ("" if unedited else f" (previous text saved under {backups})")
            )
        except (OSError, ValueError, subprocess.SubprocessError):
            report.append(
                f"  ! could not re-render {name}; it still runs the older prompt"
            )
...
def refresh_prose(old, new, report):
    ...
    for spec in load_manifest(new)["jobs"]:
        ...
        for key in keys:
            ...
            if was and was in updated:
                updated = updated.replace(was, now)
            elif now not in updated:
                stale.append(key)
        if updated != prompt:
            try:
                result = subprocess.run(
                    cp.hermes_command("cron", "edit", job["id"], "--prompt", updated),
                    capture_output=True,
                    text=True,
                    encoding="utf-8",
                    timeout=30,
                    env={
                        **os.environ,
                        "HERMES_HOME": str(new.home),
                        "HERMES_TIMEZONE": new.timezone,
                    },
                )
                if result.returncode:
                    raise ValueError("Hermes cron edit failed")
                ...
                report.append(
                    f"  {job['name']}: updated {', '.join(k.lower() for k in keys)}"
                )
            except (OSError, ValueError, subprocess.SubprocessError):
                report.append(
                    f"  ! could not update {job['name']}; its prompt still states the old value. Retry, or edit the job in Hermes"
                )
        if stale:
            report.append(
                f"  ! {job['name']} no longer contains the {'/'.join(k.lower() for k in stale)} wording this kit wrote; "
...
def refresh_schedules(old, new, report, run_cmd=None):
    ...
    """
    if not (new.home / "cron/jobs.json").exists():
        return
    m_old, m_new = mapping(old, {}), mapping(new, {})
    specs_old = {
        cr.render(spec["name"], m_old): spec for spec in load_manifest(old)["jobs"]
    }
    specs_new = {
        cr.render(spec["name"], m_new): spec for spec in load_manifest(new)["jobs"]
    }
            ...
            if was != now and current_expr == was:
                try:
                    ...
                    if getattr(result, "returncode", 0):
                        report.append(
                            f"  ! could not update schedule for {job.get('name',job['id'])}; retry repair"
                        )
                    else:
                        report.append(
                            f"  {job.get('name',job['id'])}: schedule updated {was} -> {now}"
                        )
                except (OSError, ValueError, subprocess.SubprocessError) as exc:
                    report.append(
                        f"  ! could not update schedule for {job.get('name',job['id'])}: {exc}"
                    )


def install_hook(c, m, report):
    guide = c.home / "skills/companion-image-workflows/SKILL.md"
    guide_source = (T / "skills/companion-image-workflows/SKILL.md").read_text(
        encoding="utf-8"
    )
    if not guide.exists():
    ...
    if not isinstance(hooks, dict):
        raise ValueError("Invalid Hermes hooks configuration")
    ...
    legacy = old.exists() and any(
        k in old.read_text(encoding="utf-8")
        for k in ("tamanitomo continuity hook", "companion-kit continuity hook")
    )
    cleaned = []
    for entry in pre:
        value = entry.get("command", "") if isinstance(entry, dict) else ""
        ...
        if value == cmd or _runs_hook(value, hook):
            continue
        if "inject-continuity.sh" in value:
            if legacy and value in (str(old), "~/.hermes/hooks/inject-continuity.sh"):
                continue
            report.append(
                "  ! unrelated continuity hook already registered; inspect config.yaml before installing this hook"
            )
            return
        cleaned.append(entry)
    source = (
        "# tamanitomo continuity hook; generated for this profile.\n"
    ...
    if not hook.exists() or hook.read_text(encoding="utf-8") != source:
        cp.atomic_write(hook, source)
    cleaned.append({"command": cmd})
    hooks["pre_llm_call"] = cleaned
    ...
    end_source = (
        "# tamanitomo session-end flag; generated for this profile.\n"
        "import json, os, pathlib, runpy, sys\n"
        f"home=pathlib.Path({str(c.home)!r}).resolve()\n"
        "active=pathlib.Path(os.environ.get('HERMES_HOME',str(home))).expanduser().resolve()\n"
    ...
    end_cmd = _registered(
        hooks.get("on_session_end"), end_hook, cp.python_command(end_hook)
    )
    ends = [
        e
        for e in (hooks.get("on_session_end") or [])
        if not (
            isinstance(e, dict)
            and (
                e.get("command") == end_cmd
                or _runs_hook(e.get("command", ""), end_hook)
            )
        )
    ]
    ends.append({"command": end_cmd})
    hooks["on_session_end"] = ends
    ...
    try:
        current = int(spill.get("max_chars") or 0)
    except (TypeError, ValueError):
        current = 0
    if spill.get("enabled", True) is not False and current < need:
        spill["max_chars"] = need
        hooks["output_spill"] = spill
        report.append(
            f"  hook output limit raised to {need:,} chars so the vault map is not spilled"
        )
    data["hooks"] = hooks
    ...
    if "show_reasoning" not in display:
        ...
        report.append(
            "  display: model thinking hidden from replies (display.show_reasoning: false)"
        )
    if cfg.exists():
        shutil.copy2(
            cfg, cfg.with_name("config.yaml.pre-companion-" + uuid.uuid4().hex[:8])
        )
    ...
    if any(isinstance(e, dict) and e.get("command") == cmd for e in pre):
        ...
        report.append(
            "  ! approve this hook when Hermes asks on the first interactive chat; unattended runs require that consent"
        )
        report.append(
            "    with nobody at the keyboard: hermes -p <profile> chat -q 'hello' --oneshot --accept-hooks"
        )
...
def offer_multiplex(c, report, answers=None):
    ...
    report.append(
        "  preflight: "
        + (
            "existing gateway and Telegram setup detected"
            if health["ready"]
            else "some gateway/Telegram checks need attention"
        )
    )
    for note in health["notes"]:
        report.append("  ! " + note)
    if "multiplex" in answers and "gateway_mode" not in answers:
        answers["gateway_mode"] = (
            "shared" if wiz.as_bool(answers["multiplex"]) else "later"
        )
    if wiz._tty():
        wiz.banner(
            "Gateway ownership",
            "Each agent keeps its own identity, jobs, memory and bot.",
        )
        print(
            "  Shared: one service runs several profiles. Less maintenance and overhead;"
        )
        print(
            "          one restart affects everyone. Each Telegram agent still needs its own bot."
        )
        print(
            "  Dedicated: one service per profile. Independent restarts and troubleshooting;"
        )
        print(
            "             more services to maintain. Exclude it from the root multiplexer."
        )
        print(
            "  Neither option isolates secrets from processes sharing the same OS user."
        )
    mode = ask(
        "Who should run this agent? Keep current routing preserves an existing working setup.",
        3,
        [
            ("shared", "Shared root gateway"),
            ("dedicated", "Dedicated profile gateway"),
            ("later", "Keep current routing; decide later"),
        ],
        answers,
        "gateway_mode",
    )
    ...
    if result["restart_required"]:
        report.append(
            "  ! routing changed: restart the existing root gateway before starting a separate profile gateway"
        )
    action = ask(
        "Gateway next step",
        1,
        [
            ("later", "Finish first; configure gateway later"),
            ("status", "Inspect native gateway status"),
            ("setup", "Open Hermes bot/platform setup"),
            ("install", "Install a dedicated service through Hermes"),
            ("start", "Start the dedicated service"),
        ],
        answers,
        "gateway_action",
    )
    if action != "later":
        code = cg.native(
            c, action, root_restarted=wiz.as_bool(answers.get("root_restarted", False))
        )
        if code:
            report.append(
                f"  ! gateway {action} returned {code}; run tamanitomo gateway to resolve it"
            )
...
def place_soul(c, report, answers=None):
    ...
    if not c.soul_in_vault:
        return
    if c.soul_linked:
        return
    here = c.soul
    target = c.canonical_soul
    if here.is_symlink():
        raise ValueError(
            "SOUL.md already links elsewhere; preserve and resolve it before adopting this profile"
        )
    if here.exists() and here.read_text(encoding="utf-8").strip() and c.is_root:
        if not confirm(
            "Move the existing SOUL into the vault (preserving a backup)?",
            answers,
            "move_soul",
            True,
        ):
            c.soul_in_vault = False
            report.append("  SOUL.md: retained in Hermes home")
            return
    if target.exists():
        raise ValueError(
            f"A canonical SOUL already exists at {target}; resolve the conflict before setup"
        )
    ...
    try:
        ...
        c.soul_in_vault = False
        report.append(
            "  SOUL.md: symlinks unavailable; canonical file stays in Hermes home. Back up home AND vault."
        )
        return
    finally:
        if probe.is_symlink():
    ...
    if original is not None:
        backup = c.soul_backups / "SOUL.md.pre-move"
    ...
    try:
        ...
        if here.is_symlink():
            here.unlink()
        if not here.exists() and target.exists():
            shutil.move(str(target), str(here))
        raise
...
def scaffold(c, ans, report, soul_mode="create", raw=None):
    ...
    for d in (
        ...
        c.human_dir,
        c.soul_dir,
        c.soul_dir / "continuity",
        c.soul_dir / "reflections",
        c.soul_dir / "ambient",
        c.home / "hooks",
        c.home / "cron",
    ):
    ...
    if soul_mode == "keep":
        ...
        report.append(
            "  SOUL.md: left exactly as it was; only the self-authored block is added"
        )
        ...
        if write(c.soul, soul_text, overwrite=blank or stock):
            report.append(
                "  SOUL.md: written"
                + (f" (Hermes's default saved to {backup})" if stock else "")
            )
        else:
            report.append(
                "  ! SOUL.md already had content and was left alone; the companion identity "
        ...
        pathlib.Path(c.soul).resolve().write_text(
            prev.rstrip("\n") + banner + soul_text, encoding="utf-8"
        )
        report.append(
            f"  SOUL.md: scaffold appended below your original (backup: {backup})"
        )
    else:
        if c.soul.exists():
            backup = c.soul_backups / "SOUL.md.pre-companion"
            if backup.exists():
                backup = backup.with_name(backup.name + "-" + uuid.uuid4().hex[:8])
            backup.parent.mkdir(parents=True, exist_ok=True)
            backup.write_text(c.soul.read_text(encoding="utf-8"), encoding="utf-8")
            report.append(f"  SOUL.md: replaced (previous saved to {backup})")
        write(c.soul, soul_text, overwrite=True)
    for tmpl, dest in (
        ("PROTOCOL.md.tmpl", c.life / "PROTOCOL.md"),
    ...
    try:
        ...
        report.append(
            f"  wardrobe: {seeded['added_items']} starter pieces; daily care routine enabled"
        )
    ...
    if locks:
        ...
        for section, value in locks.items():
            try:
                companion_soul.set_lock(c, section, value)
            except ValueError as exc:
                report.append(f"  ! soul lock {section}: {exc}")
        report.append(
            "  soul: "
            + ", ".join(f"{k} {'locked' if v else 'open'}" for k, v in locks.items())
        )
    ...
    try:
        ...
        if state.get("created"):
            report.append(f"  vault: now a local git repo at {c.vault}")
        elif not state.get("ready"):
            report.append(f"  ! vault history unavailable: {state.get('reason')}")
    except (OSError, ValueError, ImportError) as exc:
        report.append(
            f"  ! could not start the vault history ({exc}); companion restore will not work"
        )
    write(
        c.human_dir / "README.md",
    ...
    try:
        if slf.BEGIN not in c.soul.read_text(encoding="utf-8"):
            slf.soul_init(c)
            report.append("  SOUL self-authored block: created")
    except OSError:
        pass
    ...
    try:
        ...
        report.append(
            f"  memory: room for {plan['caps']['MEMORY.md']:,} and "
            f"{plan['caps']['USER.md']:,} characters ({plan['reason']})"
        )
    except (OSError, ValueError, ImportError) as exc:
        report.append(
            f"  ! could not size the memory caps ({exc}); Hermes defaults stand, "
    ...
    try:
        ...
        report.append(
            f"  ! could not assemble ActiveContext.md ({exc}); the template placeholder stands"
        )
...
def finish(c, report, extra_actions=None):
    ...
    if edits:
        actions.append(
            (f"{edits} section(s) still marked ✎ EDIT — the parts you skipped", c.soul)
        )
    over = len(soul) - c.soul_warn
    if soul and over > 0:
        actions.append(
            (
                f"SOUL.md is {over:,} chars above the kit’s early-warning target; review available headroom before growing it",
                c.home / "config.yaml",
            )
        )
    if actions:
...
def next_steps(c):
    cli = lambda *args: cp.python_command(
        KIT / "bin/companion", "--home", c.home, *args
    )
    steps = [
        ("Open this profile in Hermes:", cli("chat")),
        (
            "Send a short greeting. If Hermes asks about the companion hook, review and approve it. Then exit the chat with /quit.",
            None,
        ),
        (
            "With nobody at the keyboard, give that consent non-interactively instead:",
            (
                "hermes chat -q 'hello' --oneshot --accept-hooks"
                if c.is_root
                else f"hermes -p {c.profile} chat -q 'hello' --oneshot --accept-hooks"
            ),
        ),
        (
            "Restart the gateway to load this profile’s configuration:",
            cli("gateway", "--action", "restart"),
        ),
        (
            "Run the installation check. Resolve any ! items before continuing:",
            cli("doctor"),
        ),
    ]
    if not c.cron_active:
        steps.append(
            ("Enable the scheduled jobs when you are ready:", cli("schedule", "active"))
        )
    steps.extend(
        [
            (
                f'Check all {len(load_manifest(c)["jobs"])} schedules and their next run times:',
                cli("schedule", "status"),
            ),
            (
                "After the next due time, check that Hermes recorded a successful run:",
    ...
    if c.image_timeline:
        steps.append(
            (
                "Open the local image timeline after the first successful capture:",
                str(c.data / "image-timeline/index.html"),
            )
        )
    return steps
