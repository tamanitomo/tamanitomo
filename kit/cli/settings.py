...
OUTREACH_LABELS = {
    "free": "yes, socially whenever they want",
...
def apply_settings(c, changes):
    ...
    for key in ("quiet_start", "quiet_end"):
        if key in values:
            values[key] = strict_hhmm(values[key])
    if "timezone" in values:
        values["timezone"] = check_timezone(str(values["timezone"]).strip())
    if "outreach" in values:
        if values["outreach"] not in OUTREACH_LABELS:
            raise ValueError(
                "outreach must be one of: " + ", ".join(sorted(OUTREACH_LABELS))
            )
    if "outreach_per_day" in values:
        values["outreach_per_day"] = parse_cap(values["outreach_per_day"])
    if "image_style" in values:
        styles = cr.load_styles()
        if values["image_style"] not in styles:
            raise ValueError(
                "unknown image style; run `tamanitomo catalog --category images` to see them"
            )
    ...
    if (
        updated.outreach == "never"
        and "outreach" in values
        and "outreach_per_day" not in values
    ):
...
def commit_settings(c, updated, report):
    ...
    if updated.timezone != c.timezone:
        ...
        report.append(
            f"  timezone {c.timezone} -> {updated.timezone}; Hermes config.yaml updated"
        )
    if updated.image_style != c.image_style:
        style = cr.load_styles()[updated.image_style]
        report.append(f"  image style -> {style['label']}")
        report.append(
            "  ! SOUL.md still describes the previous style. It is your file, so this does not "
    ...
    if (updated.quiet_start, updated.quiet_end) != (c.quiet_start, c.quiet_end):
        report.append(
            f"  quiet hours {c.quiet_start}-{c.quiet_end} -> {updated.quiet_start}-{updated.quiet_end}, "
    ...
    if (updated.outreach, updated.outreach_per_day) != (c.outreach, c.outreach_per_day):
        ...
        cap = (
            ""
            if updated.outreach == "never"
            else (
                " (no daily limit)"
                if not updated.outreach_per_day
                else f" (at most {updated.outreach_per_day} a day)"
            )
        )
...
def settings_rows(c):
    ...
    rows = [
        ("quiet", "Change quiet hours", f"{c.quiet_start} to {c.quiet_end}"),
        (
            "outreach",
            "May they message you first",
            OUTREACH_LABELS.get(c.outreach, c.outreach),
        ),
    ]
    if c.outreach != "never":
        rows.append(("cap", "How often they may message first", cap))
    rows += [
        ("timezone", "Timezone", c.timezone),
        (
            "style",
            "Image style",
            styles.get(c.image_style, {}).get("label", c.image_style),
        ),
...
def settings_table(rows):
    ...
    options = [
        (f"{name:<{width}} | {value}" if value else name, key)
        for key, name, value in rows
    ]
    lead = wiz.header_indent()
    header = (
        lead
        + f'{"Setting":<{width}} | Currently\n'
        + lead
        + "-" * width
        + "-+-"
        + "-" * max(9, max(len(value) for _, _, value in rows))
    )
        ...
        try:
            if action == "timeline":
                state = wiz.choose(
                    "Image timeline",
                    [
                        ("Show status and gallery location", "status"),
                        ("Enable one local image every 15 minutes", "on"),
                        ("Stop new images; keep 30-day cleanup", "off"),
                        ("Back", "back"),
                    ],
                    allow_write=False,
                    allow_skip=False,
                ...
                if state != "back":
                    cmd_timeline(argparse.Namespace(home=c.home, state=state))
                c = cc.load(c.home)
            elif action == "schedule":
                state = wiz.choose(
                    "Scheduled jobs",
                    [
                        ("Authorize and run them", "active"),
                        ("Pause them", "paused"),
                        ("Back", "back"),
                    ],
                    allow_write=False,
                ...
                if state != "back":
                    cmd_schedule(argparse.Namespace(home=c.home, state=state))
                c = cc.load(c.home)
            else:
                changes = {}
                if action == "quiet":
                    changes["quiet_start"] = strict_hhmm(
                        ask("Start of your do-not-disturb hours (HH:MM)", c.quiet_start)
                    )
                    changes["quiet_end"] = strict_hhmm(
                        ask("End of your do-not-disturb hours (HH:MM)", c.quiet_end)
                    )
                elif action == "outreach":
                    changes["outreach"] = ask(
                        "May they message you first?",
                        2,
                        [
                            ...
                            (
                                "updates_only",
                                "Only with something real to report or ask",
                            ),
                            ("never", "No, replies only"),
                        ],
                    )
            ...
            json.dumps(
                {
                    "agent": c.agent,
                    "home": str(c.home),
                    "quiet_hours": f"{c.quiet_start}-{c.quiet_end}",
                    "timezone": c.timezone,
                    "outreach": c.outreach,
                    "outreach_per_day": c.outreach_per_day or None,
