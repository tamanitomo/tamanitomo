        ...
        while True:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise OSError(
                        "Another launcher is still preparing dependencies. Try again after it finishes."
                    )
                time.sleep(0.2)
        try:
            yield
...
def bootstrap(args):
    if sys.version_info < (3, 11):
        raise SystemExit(
            "Tamanitomo needs Python 3.11+. Use the tamanitomo launcher to provision Python automatically."
        )
    ...
    with setup_lock():
        ...
        if not stamp.exists() and old_stamp.exists():
            try:
                old_stamp.replace(stamp)
            except OSError:
                stamp = old_stamp
        if (
            not stamp.exists()
            or stamp.read_text(encoding="utf-8").strip() != fingerprint
        ):
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"Setup could not finish: {exc}\nRun the launcher again to retry.",
            file=sys.stderr,
        )
        sys.exit(1)
