...
if not args or args[:2] == ["cron", "runs"]:
    ...
    data = (
        json.loads(path.read_text(encoding="utf-8")) if path.exists() else {"jobs": []}
    )
    data["jobs"].append(
        {
            ...
            "schedule": {"kind": "cron", "expr": args[2]},
            "enabled": "--paused" not in args,
            "next_run_at": (
                None
                if "--paused" in args
                else (
            ...
            "enabled_toolsets": (
                args[args.index("--toolsets") + 1].split(",")
                if "--toolsets" in args
                else None
            ),
        }
    ...
    print(
        "Welcome back. I was thinking about our garden plan. Shall we start with tea?"
    )
else:
    sys.exit(2)
