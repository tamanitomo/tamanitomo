...
- Rebuild Plans & calendar: a compact month picker with a dot on days that have something on your list, the chosen day beside it (her day, then your list with one add box and an optional note), and your whole list below as an agenda (overdue, coming up, any time, then done and removed). This replaces the stat strip, the second add form and the "All Scheduled Tasks" panel. Removing an item no longer goes through the long-running operation wrapper, which the drop endpoint never used.
- Home shows the companion's day for today and what is on your list today, in place of a second month grid.
- Rebuild the timeline so photos belong to the scene they were taken in. A scene is one card with its start, its length, mood, place and a strip of its photos; a night's sleep is one card instead of a row per rewrite with a photo wedged between each. Hidden photos say so instead of showing an unexplained grey box.
- Declare the night at a local-model wind-down. The hosted wind-down declared its own sleep window, but the local variant told the model its record was already saved, so nothing declared the night, every overnight photo gate read "awake", and one night produced ten photos of the companion asleep. The wake time is the end of quiet hours, or earlier if tomorrow's plan starts earlier; a night already declared is left alone.
- Offer a clothes-shopping trip among the day's ideas and in the wind-down plan once one is due (every 14 days, with the lifestyle on), with how to record it: the `shop` care action with up to three new pieces, each saying what it covers. Buying clothes was possible but only mentioned in the middle of whatever she was doing, and no companion had ever bought anything.
- Remove "fictional" from activity-idea titles and a persona description that is written into the SOUL.
...
- Judge the continuity hook by the command actually registered, not by the Python the doctor happens to run under, and treat `hooks_auto_accept` as consent. The doctor called a healthy hook unregistered and unapproved whenever it ran under a different interpreter.
